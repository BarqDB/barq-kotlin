...
// Configure how the Barq module is published
open class BarqPublishExtensions {
...
class BarqPublishPlugin : Plugin<Project> {
    ...
    private fun configureSubProject(project: Project, signBuild: Boolean) {
        ...
        with(project) {
            ...
            // Create the BarqPublish plugin. It must evaluate after all other plugins as it modifies their output.
            // Only allow configuration from sub projects as the top-level project is just a placeholder
            extensions.create<BarqPublishExtensions>("barqPublish")

            afterEvaluate {
                project.extensions.findByType<BarqPublishExtensions>()?.run {
        ...
        project.extensions.getByType<PublishingExtension>().apply {
            publications.withType<MavenPublication>().all {
                pom {
                    ...
                    url.set(Barq.projectUrl)
                    licenses {
                        license {
                            name.set(Barq.License.name)
                            url.set(Barq.License.url)
                        }
                    }
                    issueManagement {
                        system.set(Barq.IssueManagement.system)
                        url.set(Barq.IssueManagement.url)
                    }
                    scm {
                        connection.set(Barq.SCM.connection)
                        developerConnection.set(Barq.SCM.developerConnection)
                        url.set(Barq.SCM.url)
                    }
                    developers {
                        developers {
                            developer {
                                name.set(Barq.Developer.name)
                                email.set(Barq.Developer.email)
                                organization.set(Barq.Developer.organization)
                                organizationUrl.set(Barq.Developer.organizationUrl)
                            }
                        }
                    }
                }
