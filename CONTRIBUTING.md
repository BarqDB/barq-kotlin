...
Checkout repo:
```sh
git clone --recursive  https://github.com/BarqDB/barq-kotlin.git 
```
...
1) All "common" tests should be placed in the `packages/test-X/src/androidAndroidTest/kotlin/io/github/barqdb/test/shared` folder. They should be written using only common API's. I.e. use Kotlin Test, not JUnit. This `io.github.barqdb.shared` package should only contain tests we plan to eventually move to `commonTest`.


2) The `nativeDarwinTest` (macOS) shared tests would automatically be picked up from the `androidAndroidTest` as it is symlinked to `packages/test-X/src/androidAndroidTest/kotlin/io/github/barqdb/test/shared`.
...
All platform specific tests should be placed outside the `io.github.barqdb.test.shared` package, the default being `io.github.barqdb.test`.
...
- Set breakpoints, e.g.:
`breakpoint set --file barq_coordinator.cpp --line 288`
- Run ONLY the test you want:
`r --gtest_filter="io.github.barqdb.MigrationTests.deleteOnMigration"`
- Step into:
`s`
- Step over:
`n`
