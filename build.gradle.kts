...
fun readAndCacheVersion(): String {
    ...
    val match: MatchResult = regex.find(constants) ?: throw GradleException("Could not find current Barq version")
...
tasks {
    ...
    register<GradleBuild>("mavenCentralUpload") {
        description = "Push all Barq artifacts to Maven Central"
        group = "Publishing"
    ...
    val archiveDebugSymbols by register("archiveDebugSymbols", Zip::class) {
        archiveName = "barq-kotlin-jni-libs-unstripped-${currentVersion}.zip"
    ...
    val verifyS3Access by register("verifyS3Access", Task::class) {
        doLast {
            getPropertyValue("BARQ_S3_ACCESS_KEY", true)
            getPropertyValue("BARQ_S3_SECRET_KEY", true)
        }
    }
    ...
    val uploadDebugSymbols by register("uploadDebugSymbols", Task::class) {
        ...
        doLast {
            exec {
                val s3AccessKey = getPropertyValue("BARQ_S3_ACCESS_KEY")
                val s3SecretKey = getPropertyValue("BARQ_S3_SECRET_KEY")
                ...
                commandLine = listOf(
                        ...
                        "put",
                        "barq-kotlin-jni-libs-unstripped-${currentVersion}.zip",
                        "s3://static.barq.io/downloads/kotlin/"
                )
            }
        }
    }
    ...
    val updateS3VersionFile by register("updateS3VersionFile", Exec::class) {
        ...
        commandLine = listOf(
                ...
                "s3://static.barq.io/update/kotlin")
    }
