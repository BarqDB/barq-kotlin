 ...
 */
package io.github.barqdb.kotlin.benchmark
...
open class SetterTests {
     ...
     * BarqObject setter benchmark, we use a managed object created inside the transaction.
     *
    ...
    fun before() {
        config = BarqConfiguration.Builder(schema = setOf(Entity1::class))
            .directory("./build")
            .build()
        barq = Barq.open(config)
        managedObject = barq.writeBlocking {
            unmanagedObject = Entity1().apply {
                ...
                doubleField = 1.234
                objectField = Entity1()
                objectListField = barqListOf(Entity1(), Entity1(), Entity1())
            }
            copyToBarq(unmanagedObject)
        }
    }
    ...
    fun managedSetList5000(blackhole: Blackhole) {
        barq.writeBlocking {
            val listObj = copyToBarq(Entity1())
            val obj = findLatest(managedObject)!!
            repeat(5000) {
