...
PACKAGE_DIR="$BARQ_KOTLIN_PATH/packages/build/m2-buildrepo/io/github/barqdb/kotlin"
VERSION="$1"
OUTPUT_FILE="$2"
...
ANDROID_X86_SIZE=`wc -c ./unzipped/jni/x86/libbarqc.so | awk '{print $1}'`
ANDROID_X86_64_SIZE=`wc -c ./unzipped/jni/x86_64/libbarqc.so | awk '{print $1}'`
ANDROID_ARM64_V8A_SIZE=`wc -c ./unzipped/jni/arm64-v8a/libbarqc.so | awk '{print $1}'`
ANDROID_ARMEABI_V7A_SIZE=`wc -c ./unzipped/jni/armeabi-v7a/libbarqc.so | awk '{print $1}'`
cleanup_aar_tmp_data
...
calculate_jvm_native_size "$PACKAGE_DIR/cinterop-jvm/$VERSION//unzipped/jni/macos/libbarqc.dylib"
JVM_MACOS_SIZE=$RETURN_NATIVE_SIZE
calculate_jvm_native_size "$PACKAGE_DIR/cinterop-jvm/$VERSION//unzipped/jni/linux/libbarqc.so"
JVM_LINUX_SIZE=$RETURN_NATIVE_SIZE
calculate_jvm_native_size "$PACKAGE_DIR/cinterop-jvm/$VERSION//unzipped/jni/windows/barqc.dll"
JVM_WINDOWS_SIZE=$RETURN_NATIVE_SIZE
rm -r ./unzipped
cd "$STARTING_DIR"

# Output all metrics as a JSON object.
