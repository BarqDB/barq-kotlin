...
# 7. Upload Javadoc/KDoc to the Barq docs bucket.
# 8. Notify release channels about the new release.
set -e
...
usage() {
cat <<EOF
Usage: $0 <maven_central_user> <maven_central_key> <barq_s3_access_key> <barq_s3_secret_key> <docs_s3_access_key> <docs_s3_secret_key> <slack-webhook-releases-url> <slack-webhook-ci-url> <gradle-plugin-portal-key> <gradle-plugin-portal-secret> <gradle-assemble-build-params>
Usage: $0 verify
EOF
}
...
BARQ_KOTLIN_PATH="$HERE/.."
RELEASE_VERSION=""
MAVEN_CENTRAL_USER="$1"
MAVEN_CENTRAL_KEY="$2"
BARQ_S3_ACCESS_KEY="$3"
BARQ_S3_SECRET_KEY="$4"
...
abort_release() {
  # Reporting failures is done from Jenkins.
  exit 1
}
...
verify_changelog() {
  ...
  query="grep -c '^## $RELEASE_VERSION ([0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9])' $BARQ_KOTLIN_PATH/CHANGELOG.md"
...
upload_debug_symbols() {
  ...
  cd $BARQ_KOTLIN_PATH
  ./gradlew uploadReleaseMetaData -PBARQ_S3_ACCESS_KEY=$BARQ_S3_ACCESS_KEY -PBARQ_S3_SECRET_KEY=$BARQ_S3_SECRET_KEY
  cd $HERE
}
...
notify_slack_channels() {
  ...
  command="grep '## $RELEASE_VERSION' $BARQ_KOTLIN_PATH/CHANGELOG.md | cut -c 4- | sed -e 's/[.)(]//g' | sed -e 's/ /-/g'"
  tag=`eval $command`
  if [ -z "$tag" ]
  then
  ...
  link_to_changelog="https://github.com/BarqDB/barq-kotlin/blob/$current_commit/CHANGELOG.md#$tag"
  payload="{ \"username\": \"Barq CI\", \"icon_emoji\": \":rocket:\", \"text\": \"<$link_to_changelog|*Barq Kotlin $RELEASE_VERSION has been released*>\\nSee the Release Notes for more details. Note, it can take up to 10 minutes before the release is visible on Maven Central.\" }"
  echo $link_to_changelog
  echo "Pinging release channel"
  curl -X POST --data-urlencode "payload=${payload}" ${SLACK_WEBHOOK_RELEASES_URL}
  echo "Pinging CI channel"
