...
```
gh api -H 'Accept: application/vnd.github+json' /repos/BarqDB/barq-kotlin/actions/caches --paginate | jq -r '.actions_caches | .[].id' | xargs -I {} sh -c 'gh api --method DELETE -H "Accept: application/vnd.github+json" /repos/BarqDB/barq-kotlin/actions/caches/{} --silent'
```

of if you have the Github Actions CLI 2.42.0 or later from Homebrew:

```
gh cache delete -a --repo BarqDB/barq-kotlin
```


## See all caches

Access all Github Action caches using: https://github.com/BarqDB/barq-kotlin/actions/caches?query=sort%3Asize-desc
...
2. Goto Github.com > Actions > Press the PR build > Read the action ID from the URL > Manually enter the following URL: https://github.com/BarqDB/barq-kotlin/runs/<id>
