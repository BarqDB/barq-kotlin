...
public class OkHttpWebsocketClient(
    ...
    override fun onMessage(webSocket: WebSocket, bytes: ByteString) {
        ...
        runIfObserverNotClosed {
            ...
            if (shouldClose) {
                webSocket.close(
                    WebsocketErrorCode.BARQ_ERR_WEBSOCKET_OK.nativeValue,
    ...
    override fun onClosed(webSocket: WebSocket, code: Int, reason: String) {
        ...
        runIfObserverNotClosed {
            ...
            // via a 401 HTTP response is not possible).
            ...
            WebsocketErrorCode.of(code)?.let { errorCode ->
                ...
                observer.onClose(
                    true,
                    WebsocketErrorCode.BARQ_ERR_WEBSOCKET_FATAL_ERROR,
    ...
    override fun send(message: ByteArray, handlerCallback: BarqWebsocketHandlerCallbackPointer) {
                ...
                try {
                    if (!isClosed.get()) { // double check that the websocket is still open before sending.
                        ...
                        runCallback(
                            ...
                            WebsocketCallbackResult.BARQ_ERR_SYNC_SOCKET_SUCCESS,
                            ""
                        )
                    } else {
                        runCallback(
                            handlerCallback,
                            observerIsClosed.get(), // if the Core observer is closed we run this callback as cancelled (to free underlying resources)
                            WebsocketCallbackResult.BARQ_ERR_SYNC_SOCKET_CONNECTION_CLOSED,
                    ...
                    runCallback(
                        handlerCallback,
                        observerIsClosed.get(), // if the Core observer is closed we run this callback as cancelled (to free underlying resources)
                        WebsocketCallbackResult.BARQ_ERR_SYNC_SOCKET_RUNTIME,
    ...
    override fun close() {
        ...
        if (::webSocket.isInitialized) {
            scope.launch {
                if (!isClosed.get()) {
                    webSocket.close(
                        WebsocketErrorCode.BARQ_ERR_WEBSOCKET_OK.nativeValue, "client closed websocket"
                    )
                }
            }
        }
...
public actual fun platformWebsocketClient(
    ...
    return OkHttpWebsocketClient(
        ...
        transport.engine,
        transport.scope
    ) { handlerCallback: BarqWebsocketHandlerCallbackPointer, cancelled: Boolean, status: WebsocketCallbackResult, reason: String ->
