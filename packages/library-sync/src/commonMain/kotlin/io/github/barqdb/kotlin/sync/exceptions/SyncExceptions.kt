...
import io.github.barqdb.kotlin.internal.asPrimitiveBarqAnyOrElse
 ...
 * [io.github.barqdb.kotlin.sync.SyncConfiguration.Builder.errorHandler] and the the exact reason
 * must be found in [Throwable.message].
 *
 * @see io.github.barqdb.kotlin.sync.SyncConfiguration.Builder.errorHandler
 */
public open class SyncException internal constructor(message: String?, isFatal: Boolean) : BarqException(message) {
    /**
     ...
     * resolved.
     *
     * It is still possible to use the Barq locally after receiving an error where this flag is
     * true. However, this must be done with caution as data written to the barq after this point
 ...
 *
 * Generally, errors of this kind are due to incompatible client/server versions or bugs in the
 * library or on the server, and the only fix would be installing a new version of the app with a
 * new version of Barq.
 *
 * It is still possible to use the Barq locally after this error occurs. However, this must be
 * done with caution as data written to the barq after this point risk getting lost as
 ...
 * [io.github.barqdb.kotlin.sync.SubscriptionSet].
 */
