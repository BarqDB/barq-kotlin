...
import io.github.barqdb.kotlin.internal.util.DispatcherHolder
...
public class BarqWebSocketTransport(
    public val timeoutMs: Long
) : WebSocketTransport {
    // we need a single thread dispatcher to act like an event loop
    private val dispatcherHolder: DispatcherHolder = CoroutineDispatcherFactory.managed("BarqWebsocketTransport").create()
    ...
    override fun post(handlerCallback: BarqWebsocketHandlerCallbackPointer) {
        scope.launch {
    ...
    override fun createTimer(
        ...
        val atomicCallback: AtomicRef<BarqWebsocketHandlerCallbackPointer?> =
