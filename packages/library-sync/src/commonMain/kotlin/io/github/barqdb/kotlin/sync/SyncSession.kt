...
import io.github.barqdb.kotlin.sync.SyncSession.ErrorHandler
import io.github.barqdb.kotlin.sync.SyncSession.State.ACTIVE
import io.github.barqdb.kotlin.sync.SyncSession.State.DYING
import kotlinx.coroutines.flow.Flow
import kotlin.time.Duration

/**
 * A session controls how data is synchronized between a single Barq on the device and the server.
 *
 * A `SyncSession` is created by opening a Barq instance using a [SyncConfiguration]. Once a
 * session has been created, it will continue to exist until the app is closed or the [Barq] is
 * closed.
 *
 * A session is controlled by Barq, but can provide additional information in case of errors.
 * These errors are passed along in the [ErrorHandler].
 *
 * When creating a session, Barq will establish a connection to the server. This connection is
 * controlled by Barq and might be shared between multiple sessions.
 *
...
public interface SyncSession {
    ...
    /**
     * The [User] used to authenticate the session on the Barq sync server.
     */
     ...
     * applied to the Barq or the specified timeout is hit. This will involve network access, so
    ...
    /**
     * Pauses synchronization until the Barq is closed and re-opened again.
     *
    ...
    /**
     * Attempts to resume the session and activate synchronization.
     *
     * This happens automatically when opening the Barq, so doing it manually should only
     ...
     * visible in the barq. To wait for data being integrated and visible use
     ...
     *
     * The flow will be completed if the barq is closed.
     *
    ...
    public fun interface ErrorHandler {
         ...
         * inside an error handler, as the exception will be caught, logged, and ignored by Barq.
     ...
     *
     * A Barq will automatically synchronize data with the server if the session is either [ACTIVE]
     * or [DYING] and is connected.
     */
    public enum class State {
        /**
         * This is the initial state. The session is closed. No data is being synchronized. The session
         * will automatically transition to [ACTIVE] when a Barq is opened.
         */
        INACTIVE,

        /**
         * The Barq is open and data will be synchronized between the device and the server
         * if the underlying connection is connected.
         *
         * The session will remain in this state until the session is either paused, after
         * which the session becomes [PAUSED] or the barq is closed, in which case it will
         * become [DYING].
         */
        ACTIVE,

        /**
         * The Barq is open and has a connection to the server, but no data is allowed to be
        ...
        /**
         * The Barq was closed, but still contains data that needs to be synchronized to the server.
         * The session will attempt to upload all local data before going [INACTIVE].
         */
        DYING,

        /**
         * The user is attempting to synchronize data but needs a valid access token to do so. Barq
