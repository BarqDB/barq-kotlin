...
public interface AutomaticClientResetStrategy : SyncClientResetStrategy {
     ...
     * of the barq that will be reset.
     *
     * @param barq frozen [TypedBarq] in its state before the reset.
     */
    public fun onBeforeReset(barq: TypedBarq)
 ...
 *
 * A synced barq may need to be reset because the Device Sync encountered an error and had to be
 ...
 *
 * The manual reset process is as follows: the local copy of the barq is copied into a recovery
 * directory for safekeeping and then deleted from the original location. The next time the barq
 * for that URL is opened it will automatically be re-downloaded from the server, and can be used as
 * usual.
 *
 * Data written to the barq after the local copy of itself diverged from the backup remote copy
 * will be present in the local recovery copy of the Barq file. The re-downloaded barq will
 * initially contain only the data present at the time the barq was backed up on the server.
 *
 * The client reset process can be initiated in one of two ways:
 *
 *  1. Run [ClientResetRequiredException.executeClientReset] manually. All Barq instances must be
 ...
 *  all Barq instances have been closed and re-opened. This will most likely be when the app is
 *  restarted.
 *
 * **WARNING:**
 * Any writes to the Barq file between this callback and Client Reset has been executed, will not
 * be synchronized to the server. Those changes will only be present in the backed up file. It is
 * therefore recommended to close all open Barq instances as soon as possible.
 */
public interface ManuallyRecoverUnsyncedChangesStrategy : SyncClientResetStrategy {
     ...
     * possible as any further changes to the barq will not be synchronized with the server and
     * must be moved manually from the backup barq to the new one.
     *
 ...
 * otherwise keeps the barq open. Any changes will be reported through the normal collection and
 ...
 * [onBeforeReset] callback is invoked, providing an instance of the barq before the reset. Once
 * the reset has concluded, [onAfterReset] will be invoked with both the instance before (read only)
 * an instance of the final barq (mutable).
 *
...
public interface DiscardUnsyncedChangesStrategy : AutomaticClientResetStrategy {
    /**
     * Callback invoked once the Client Reset happens. It receives two Barq instances: a frozen one
     ...
     *
     * @param before frozen [TypedBarq] barq before the reset.
     * @param after [MutableBarq] barq after the reset.
     */
    public fun onAfterReset(before: TypedBarq, after: MutableBarq)
}

/**
 * Strategy that attempts to automatically recover any unsynced changes during a Client Reset.
 *
 * A synced barq may need to be reset because the Barq sync server encountered an error and had
...
public interface RecoverOrDiscardUnsyncedChangesStrategy : AutomaticClientResetStrategy {
     ...
     * It provides two Barq instances, a frozen one displaying the state before the reset and a
     * mutable Barq with the current state.
     *
     * @param before frozen [TypedBarq] barq before the reset.
     * @param after [MutableBarq] barq after the reset.
     */
    public fun onAfterRecovery(before: TypedBarq, after: MutableBarq)

    /**
     * Callback invoked once the Client Reset has discarded the unsynced changes because it couldn't
     * recover them. It provides two Barq instances, a frozen one displaying the state before the
     * reset and a regular Barq displaying the current state that can be used to recover objects
     * from the reset.
     *
     * @param before frozen [TypedBarq] barq before the reset.
     * @param after [MutableBarq] barq after the reset.
     */
    public fun onAfterDiscard(before: TypedBarq, after: MutableBarq)
}
