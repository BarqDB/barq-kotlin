...
/**
 * Enum defining the behaviour of when [BarqQuery.subscribe] and [BarqResults.subscribe] will
 ...
 *
 * @see io.github.barqdb.kotlin.sync.ext.subscribe
 */
public enum class WaitForSync {
    /**
     * This mode will wait for the server data the first time a subscription is created before
     * running the local query. Later calls to [io.github.barqdb.kotlin.sync.ext.subscribe] will detect
    ...
    FIRST_TIME,
    /**
     * With this mode enabled, Barq will always download the latest server state before running
    ...
    ALWAYS,
    /**
     * With this mode enabled, Barq will always query the local database first while any server
