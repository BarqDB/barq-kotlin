...
/**
 * Class responsible for adding Barq specific logic to the default object methods like:
 * - toString()
 * - hashCode()
 * - equals()
 *
 ...
 * methods they will not pick up changes in the BarqObjectHelper.
 *
 * This should only impact us as SDK developers though, but it does mean that changes to
 * BarqObjectHelper methods will require a clean build to take effect.
 */
class BarqModelDefaultMethodGeneration(private val pluginContext: IrPluginContext) {

    private val barqObjectHelper: IrClass = pluginContext.lookupClassOrThrow(ClassIds.BARQ_OBJECT_HELPER)
    private val barqToString: IrSimpleFunction = barqObjectHelper.lookupFunction(Name.identifier("barqToString"))
    private val barqEquals: IrSimpleFunction = barqObjectHelper.lookupFunction(Name.identifier("barqEquals"))
    private val barqHashCode: IrSimpleFunction = barqObjectHelper.lookupFunction(Name.identifier("barqHashCode"))
     ...
     *
     * These methods are created by an earlier step by the Barq compiler plugin and are
        ...
        function.body = pluginContext.blockBody(function.symbol) {
            +irReturn(
                IrCallImpl(
                    ...
                    symbol = barqEquals.symbol,
                    typeArgumentsCount = 0,
                    valueArgumentsCount = 2
                ).apply {
                    dispatchReceiver = irGetObject(barqObjectHelper.symbol)
