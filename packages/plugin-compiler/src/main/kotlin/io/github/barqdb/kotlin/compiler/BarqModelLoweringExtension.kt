...
import io.github.barqdb.kotlin.compiler.ClassIds.MODEL_OBJECT_ANNOTATION
...
class BarqModelLoweringExtension : IrGenerationExtension {
    override fun generate(moduleFragment: IrModuleFragment, pluginContext: IrPluginContext) {
        BarqModelLowering(pluginContext).lowerFromModuleFragment(moduleFragment)
    }
}

private class BarqModelLowering(private val pluginContext: IrPluginContext) : ClassLoweringPass {
    ...
    fun lowerFromModuleFragment(
        ...
        try {
            ...
            throw CompilationException(
                "Internal error in barq lowering : ${this::class.qualifiedName}: ${e.message}",
                it,
                null,
                cause = e
            ).apply {
    ...
    override fun lower(irClass: IrClass) {
        if (irClass.isBaseBarqObject) {
            // Throw error with classes that we do not support
            if (irClass.isData) {
                error("Data class '${irClass.kotlinFqName}' is not currently supported. Only normal classes can inherit from 'BarqObject' or 'EmbeddedBarqObject'.")
            }
            if (irClass.isEnumClass) {
                error("Enum class '${irClass.kotlinFqName}' is not supported. Only normal classes can inherit from 'BarqObject' or 'EmbeddedBarqObject'.")
            }
            if (irClass.isObject) {
                error("Object declarations are not supported. Only normal classes can inherit from 'BarqObject' or 'EmbeddedBarqObject'.")
            }
            if (irClass.isAnonymousObject) {
                error("Anonymous objects are not supported. Only normal classes can inherit from 'BarqObject' or 'EmbeddedBarqObject'.")
            }
            ...
            // add super type BarqObjectInternal and BarqObjectInterop
            val barqObjectInternalInterface: IrClassSymbol =
                pluginContext.lookupClassOrThrow(BARQ_OBJECT_INTERNAL_INTERFACE).symbol
            irClass.superTypes += barqObjectInternalInterface.defaultType

            // Generate BarqObjectInternal properties overrides
            val generator = BarqModelSyntheticPropertiesGeneration(pluginContext)
            generator.addBarqObjectInternalProperties(irClass)
            ...
            val methodGenerator = BarqModelDefaultMethodGeneration(pluginContext)
            methodGenerator.addDefaultMethods(irClass)

            // Add body for synthetic companion methods
            val companion = irClass.companionObject() ?: fatalError("BarqObject without companion: ${irClass.kotlinFqName}")
            ...
            if (irClass.isCompanion && irClass.parentAsClass.isBaseBarqObject) {
                val barqModelCompanion: IrClassSymbol =
                    pluginContext.lookupClassOrThrow(BARQ_MODEL_COMPANION).symbol
                irClass.superTypes += barqModelCompanion.defaultType
            }
        }
    }
}
