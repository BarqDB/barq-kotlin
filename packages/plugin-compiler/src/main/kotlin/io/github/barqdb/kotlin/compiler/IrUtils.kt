...
import io.github.barqdb.kotlin.compiler.ClassIds.BASE_BARQ_OBJECT_INTERFACE
import io.github.barqdb.kotlin.compiler.ClassIds.EMBEDDED_OBJECT_INTERFACE
import io.github.barqdb.kotlin.compiler.ClassIds.KOTLIN_COLLECTIONS_LISTOF
import io.github.barqdb.kotlin.compiler.ClassIds.PERSISTED_NAME_ANNOTATION
import io.github.barqdb.kotlin.compiler.ClassIds.BARQ_OBJECT_INTERFACE
import io.github.barqdb.kotlin.compiler.FqNames.PACKAGE_TYPES
import io.github.barqdb.kotlin.compiler.Names.ASYMMETRIC_BARQ_OBJECT
import io.github.barqdb.kotlin.compiler.Names.EMBEDDED_BARQ_OBJECT
import io.github.barqdb.kotlin.compiler.Names.BARQ_OBJECT
...
val ClassDescriptor.isBarqObjectCompanion
    get() = isCompanionObject && (containingDeclaration as ClassDescriptor).isBaseBarqObject

val barqObjectInterfaceFqNames = setOf(BARQ_OBJECT_INTERFACE)
val barqEmbeddedObjectInterfaceFqNames = setOf(EMBEDDED_OBJECT_INTERFACE)
val barqAsymmetricObjectInterfaceFqNames = setOf(ASYMMETRIC_OBJECT_INTERFACE)
val anyBarqObjectInterfacesFqNames = barqObjectInterfaceFqNames + barqEmbeddedObjectInterfaceFqNames + barqAsymmetricObjectInterfaceFqNames
...
inline fun PsiElement.hasInterface(interfaces: Set<String>): Boolean {
    var hasBarqObjectAsSuperType = false
    this.acceptChildren(object : PsiElementVisitor() {
        override fun visitElement(element: PsiElement) {
            if (element.node.elementType == SUPER_TYPE_LIST) {
                // Check supertypes for classes with Embbeded/BarqObject as generics and remove
                // them from the string so as to avoid erroneously processing said classes which
                // implement these types as implementing Embedded/BarqObject. Doing so would
                ...
                val elementNodeText = element.node.text
                    ...
                    .filter {
                        !(
                            it.contains("<BarqObject>") ||
                                it.contains("<io.github.barqdb.kotlin.types.BarqObject>") ||
                                it.contains("<EmbeddedBarqObject>") ||
                                it.contains("<io.github.barqdb.kotlin.types.EmbeddedBarqObject>")
                            )
                    }.joinToString(",") // Re-sanitize again
                hasBarqObjectAsSuperType = elementNodeText.findAnyOf(interfaces) != null
            }
        }
    })

    return hasBarqObjectAsSuperType
}

@Suppress("NOTHING_TO_INLINE")
inline fun ClassDescriptor.hasInterfacePsi(interfaces: Set<String>): Boolean {
    // Using PSI to find super types to avoid cyclic reference (see https://github.com/BarqDB/barq-kotlin/issues/339)
    return this.findPsi()?.hasInterface(interfaces) ?: false
}

// Do to the way PSI works, it can be a bit tricky to uniquely identify when the Barq Kotlin
// BarqObject interface is used. For that reason, once we have determined a match for BarqObject,
// We also need to ensure we didn't accidentally matched on the Barq Java BarqObject abstract
// type. Fortunately that is visible in the PSI as `BarqObject()` (Java, abstract class) vs.
// `BarqObject` (Kotlin, interface).
val barqObjectPsiNames = setOf("BarqObject", "io.github.barqdb.kotlin.types.BarqObject")
val embeddedBarqObjectPsiNames = setOf("EmbeddedBarqObject", "io.github.barqdb.kotlin.types.EmbeddedBarqObject")
val asymmetricBarqObjectPsiNames = setOf("AsymmetricBarqObject", "io.github.barqdb.kotlin.types.AsymmetricBarqObject")
val barqJavaObjectPsiNames = setOf("io.github.barqdb.BarqObject()", "BarqObject()")
val ClassDescriptor.isBarqObject: Boolean
    get() = this.hasInterfacePsi(barqObjectPsiNames) && !this.hasInterfacePsi(barqJavaObjectPsiNames)
val ClassDescriptor.isEmbeddedBarqObject: Boolean
    get() = this.hasInterfacePsi(embeddedBarqObjectPsiNames)
val ClassDescriptor.isBaseBarqObject: Boolean
    get() = this.hasInterfacePsi(barqObjectPsiNames + embeddedBarqObjectPsiNames + asymmetricBarqObjectPsiNames) && !this.hasInterfacePsi(barqJavaObjectPsiNames)

val barqObjectTypes: Set<Name> = setOf(BARQ_OBJECT, EMBEDDED_BARQ_OBJECT, ASYMMETRIC_BARQ_OBJECT)
val barqObjectClassIds = barqObjectTypes.map { name -> ClassId(PACKAGE_TYPES, name) }

// This is the K2 equivalent of our PSI hack to determine if a symbol has a BarqObject base class.
...
val FirClassSymbol<*>.isBaseBarqObject: Boolean
    get() = this.classKind == ClassKind.CLASS &&
        this.fir.superTypeRefs.any { typeRef ->
            when (typeRef) {
                // In SUPERTYPES stage
                is FirUserTypeRef -> {
                    typeRef.qualifier.last().name in barqObjectTypes &&
                        // Disregard constructor invocations as that means that it is a Barq Java class
                        !(
                ...
                is FirResolvedTypeRef -> typeRef.type.classId in barqObjectClassIds
                else -> false
            }
        }
...
val IrClass.isBaseBarqObject
    get() = superTypes.any { it.classId in anyBarqObjectInterfacesFqNames }

val IrClass.isBarqObject
    get() = superTypes.any { it.classId == BASE_BARQ_OBJECT_INTERFACE }

val IrClass.isEmbeddedBarqObject: Boolean
    get() = superTypes.any { it.classId == EMBEDDED_OBJECT_INTERFACE }

val IrClass.isAsymmetricBarqObject: Boolean
    get() = superTypes.any { it.classId == ASYMMETRIC_OBJECT_INTERFACE }

val IrType.classId: ClassId?
    get() = this.getClass()?.classId

val IrType.isEmbeddedBarqObject: Boolean
    get() = superTypes().any { it.classId == EMBEDDED_OBJECT_INTERFACE }

val IrType.isAsymmetricBarqObject: Boolean
    get() = superTypes().any { it.classId == ASYMMETRIC_OBJECT_INTERFACE }
...
/**
 * This matches BarqEnums.CollectionType.
 */
enum class CollectionType(val description: String) {
    NONE("None"),
    LIST("BarqList"),
    SET("BarqSet"),
    DICTIONARY("BarqDictionary");
}

/**
 * This matches BarqEnums.PropertyType.
 */
...
//  see (https://github.com/BarqDB/barq-kotlin/issues/238)
data class SchemaProperty(
    ...
    val isComputed = propertyType == PropertyType.BARQ_PROPERTY_TYPE_LINKING_OBJECTS
