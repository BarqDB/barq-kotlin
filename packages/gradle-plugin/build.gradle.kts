...
pluginBundle {
    website = "https://github.com/BarqDB/barq-kotlin"
    vcsUrl = "https://github.com/BarqDB/barq-kotlin"
    tags = listOf("Barq", "Database", "Kotlin", "Mobile", "Multiplatform", "Android", "KMM")

    mavenCoordinates {
        groupId = Barq.group
        artifactId = Barq.gradlePluginId
...
gradlePlugin {
    plugins {
        create("BarqPlugin") {
            id = Barq.pluginPortalId
            displayName = "Barq Kotlin Plugin"
            description = "Gradle plugin for the Barq Kotlin SDK, supporting Android and Multiplatform. " +
                "Barq is a mobile database: Build better apps faster."
            implementationClass = "io.github.barqdb.kotlin.gradle.BarqPlugin"
        }
...
barqPublish {
    pom {
        name = "Gradle Plugin"
        description = "Gradle plugin for Barq Kotlin. Barq is a mobile database: Build better apps faster."
    }
}
...
val versionConstants: Task = tasks.create("versionConstants") {
    ...
    doLast {
        val versionFile = file("$outputDir/io/github/barqdb/kotlin/gradle/version.kt")
