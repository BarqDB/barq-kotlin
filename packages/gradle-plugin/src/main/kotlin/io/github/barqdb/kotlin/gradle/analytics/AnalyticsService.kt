 ...
 */
package io.github.barqdb.kotlin.gradle.analytics

import io.github.barqdb.kotlin.gradle.BarqCompilerSubplugin
import io.github.barqdb.kotlin.gradle.analytics.AnalyticsService.Companion.unknown
import io.github.barqdb.kotlin.gradle.gradle75
import io.github.barqdb.kotlin.gradle.gradleVersion
...
// Asynchronously submits build information to Barq when the gradle compile task run
//
...
// *will* help us understand what Barq version you use, what host OS you use,
...
// long long time. Since Barq is a free product without an email signup, we
// feel this is a necessary step so we can collect relevant data to build a
// better product for you.
//
// Currently the following information is reported:
// - What version of Barq is being used
...
// [barq-analytics] if settings the system environment variable
//   BARQ_PRINT_ANALYTICS=true
// Collection and submission of data can be fully disabled by setting the system environment variable
//   BARQ_DISABLE_ANALYTICS=true

private const val EVENT_NAME = "Run"
// Barq does not phone home. See submit() below, which is now a local-only no-op.
...
abstract class AnalyticsService : BuildService<ProjectConfiguration> {
    ...
    init {
        ...
        projectInfo.add(
            ...
            JsonObject().apply {
                ...
                add("Barq Version", JsonPrimitive(BarqCompilerSubplugin.version))
                add("Core Version", JsonPrimitive(BarqCompilerSubplugin.coreVersion))
                add(
                    "Sync Enabled",
    ...
    internal fun print(json: String) {
        info("[barq-analytics] Payload: $json")
    }

    @Suppress("TooGenericExceptionCaught", "UNUSED_PARAMETER")
    internal fun submit(json: String) {
        // Barq does not phone home. Network analytics were removed, so nothing leaves
        // the machine.
        debug("Analytics disabled in Barq; payload not submitted.")
    }
    ...
    companion object {
        internal val LOGGER: Logger = Logging.getLogger("barq-analytics")
