...
//               barq-kotlin/packages> CONFIGURATION=Release ./gradlew capiIosArm64
//               * to force build a debug (default BTW) use
//               barq-kotlin/packages> CONFIGURATION=Debug ./gradlew capiIosArm64
//               default is 'Release'
...
fun checkIfBuildingNativeLibs(task: Task, action: Task.() -> Unit) {
    // Whether or not to build the underlying native Barq Libs. Generally these are only
    // needed at runtime and thus can be ignored when only building the layers on top
    if (project.extra.properties["barq.kotlin.buildBarqCore"] == "true") {
        action(task)
    } else {
...
val nativeLibraryIncludesMacosUniversalRelease = includeBinaries(
    listOf(
        "object-store/c_api/Release/libbarq-ffi-static.a",
        "Release/libbarq.a",
        "parser/Release/libbarq-parser.a",
        "object-store/Release/libbarq-object-store.a",
        "sync/Release/libbarq-sync.a"
    ).map { "$absoluteCorePath/build-macos_universal/src/barq/$it" }
)
val nativeLibraryIncludesMacosUniversalDebug = includeBinaries(
    listOf(
        "object-store/c_api/Debug/libbarq-ffi-static-dbg.a",
        "Debug/libbarq-dbg.a",
        "parser/Debug/libbarq-parser-dbg.a",
        "object-store/Debug/libbarq-object-store-dbg.a",
        "sync/Debug/libbarq-sync-dbg.a"
    ).map { "$absoluteCorePath/build-macos_universal-dbg/src/barq/$it" }
)
val releaseLibs = listOf(
    "libbarq-ffi-static.a",
    "libbarq.a",
    "libbarq-parser.a",
...
kotlin {
    ...
    iosX64 { // Shortcut for both iosArm64 and iosX64
        compilations.getByName("main") {
            cinterops.create("barq_wrapper") {
                definitionFile.set(project.file("src/native/barq.def"))
                packageName = "barq_wrapper"
...
android {
    namespace = "io.github.barqdb.kotlin.internal.interop"
...
if (HOST_OS.isMacOs()) {
    ...
    tasks.named("cinteropBarq_wrapperIosArm64") {
        checkIfBuildingNativeLibs(this) {
            dependsOn(capiIosArm64)
        }
    }

    tasks.named("cinteropBarq_wrapperIosX64") {
        checkIfBuildingNativeLibs(this) {
            dependsOn(capiSimulatorX64)
        }
    }

    tasks.named("cinteropBarq_wrapperIosSimulatorArm64") {
        checkIfBuildingNativeLibs(this) {
            dependsOn(capiSimulatorArm64)
        }
    }

    tasks.named("cinteropBarq_wrapperMacosX64") {
        checkIfBuildingNativeLibs(this) {
            dependsOn(capiMacosUniversal)
        }
    }

    tasks.named("cinteropBarq_wrapperMacosArm64") {
        checkIfBuildingNativeLibs(this) {
            dependsOn(capiMacosUniversal)
        }
    }
...
val copyJVMSharedLibs: TaskProvider<Task> by tasks.registering {
    val copyJvmABIs = project.hasProperty("barq.kotlin.copyNativeJvmLibs")
            && (project.property("barq.kotlin.copyNativeJvmLibs") as String).isNotEmpty()
    logger.info("Copy native Barq JVM libraries: $copyJvmABIs")
    if (copyJvmABIs) {
        val archs = (project.property("barq.kotlin.copyNativeJvmLibs") as String)
            .split(",")
            .map { it.trim() }
            .map { it.toLowerCase() }
        ...
        archs.forEach { arch ->
            when(arch) {
                "linux" -> {
                    ...
                    project.file("$buildDir/barqLinuxBuild/libbarqc.so")
                ...
                "macos" -> {
                    ...
                    project.file("$buildDir/barqMacOsBuild/libbarqc.dylib")
                        .copyTo(project.file("$jvmJniPath/macos/libbarqc.dylib"), overwrite = true)
                    outputs.file(project.file("$jvmJniPath/macos/libbarqc.dylib"))
                }
                "windows" -> {
                    // copy Window pre-built binaries
                    project.file("$buildDir/barqWindowsBuild/Release/barqc.dll")
                        .copyTo(project.file("$jvmJniPath/windows/barqc.dll"), overwrite = true)
                    outputs.file(project.file("$jvmJniPath/windows/barqc.dll"))
                }
                else -> throw IllegalArgumentException("Unsupported platform for barq.kotlin.copyNativeJvmLibs: $arch")
            }
        }
    }
}
...
fun getSharedCMakeFlags(buildType: BuildType, ccache: Boolean = true): Array<String> {
    ...
    with(args) {
        ...
        add("-DBARQ_ENABLE_SYNC=1")
        add("-DBARQ_NO_TESTS=1")
        add("-DBARQ_BUILD_LIB_ONLY=true")
        add("-DBARQ_CORE_SUBMODULE_BUILD=true")
...
fun Task.buildSharedLibrariesForJVMMacOs() {
    ...
    val directory = "$buildDir/barqMacOsBuild"
...
fun Task.buildSharedLibrariesForJVMWindows() {
    group = "Build"
    description = "Compile dynamic libraries loaded by the JVM fat jar for supported platforms."
    val directory = "$buildDir/barqWindowsBuild"
    ...
    doLast {
        ...
        File("$directory/Release/barqc.dll")
            .copyTo(project.file("$jvmJniPath/windows/barqc.dll"), overwrite = true)
    }
...
fun Task.build_C_API_Macos_Universal(buildVariant: BuildType) {
    ...
    doLast {
        ...
        exec {
            // See https://github.com/BarqDB/barq-core/blob/master/tools/build-cocoa.sh#L47
        ...
        exec {
            ...
            commandLine(
                ...
                "-configuration",
                "${buildVariant.type}",
                "-UseModernBuildSystem=NO", // TODO remove flag when https://github.com/BarqDB/barq-kotlin/issues/141 is fixed
                "DISABLE_MANUAL_TARGET_ORDER_BUILD_WARNING=YES",
            )
        }
    }
    inputs.dir(project.file("$absoluteCorePath/src"))
    outputs.file(project.file("$directory/src/barq/object-store/c_api/$buildVariant/libbarq-ffi-static.a"))
    outputs.file(project.file("$directory/src/barq/$buildVariant/libbarq.a"))
    outputs.file(project.file("$directory/src/barq/object-store/c_api/$buildVariant/libbarq-ffi-static.a"))
    outputs.file(project.file("$directory/src/barq/object-store/$buildVariant/libbarq-object-store.a"))
    outputs.file(project.file("$directory/src/barq/sync/$buildVariant/libbarq-sync.a"))
}
...
fun Task.build_C_API_Simulator(arch: String, buildType: BuildType) {
    ...
    inputs.dir(project.file("$absoluteCorePath/src"))
    outputs.file(project.file("$directory/lib/libbarq-ffi-static${buildType.buildDirSuffix}.a"))
    outputs.file(project.file("$directory/lib/libbarq${buildType.buildDirSuffix}.a"))
    outputs.file(project.file("$directory/lib/libbarq-parser${buildType.buildDirSuffix}.a"))
    outputs.file(project.file("$directory/lib/libbarq-object-store${buildType.buildDirSuffix}.a"))
    outputs.file(project.file("$directory/lib/libbarq-sync${buildType.buildDirSuffix}.a"))
}
...
afterEvaluate {
    ...
    tasks.named("generateJsonModelDebug") {
        inputs.files(tasks.getByPath(":jni-swig-stub:barqWrapperJvm").outputs)
    }
...
barqPublish {
    pom {
        name = "C Interop"
        description =
            "Wrapper for interacting with Barq Kotlin native code. This artifact is not " +
...
val generateSdkVersionConstant: Task = tasks.create("generateSdkVersionConstant") {
    ...
    doLast {
        val versionFile = file("$outputDir/io/github/barqdb/kotlin/internal/Version.kt")
