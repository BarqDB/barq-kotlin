/*
 * Copyright 2020 Realm Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package io.github.barqdb.kotlin.internal.interop

import io.github.barqdb.kotlin.internal.interop.Constants.ENCRYPTION_KEY_LENGTH
import io.github.barqdb.kotlin.internal.interop.sync.ApiKeyWrapper
import io.github.barqdb.kotlin.internal.interop.sync.CoreConnectionState
import io.github.barqdb.kotlin.internal.interop.sync.CoreSubscriptionSetState
import io.github.barqdb.kotlin.internal.interop.sync.CoreSyncSessionState
import io.github.barqdb.kotlin.internal.interop.sync.CoreUserState
import io.github.barqdb.kotlin.internal.interop.sync.JVMSyncSessionTransferCompletionCallback
import io.github.barqdb.kotlin.internal.interop.sync.NetworkTransport
import io.github.barqdb.kotlin.internal.interop.sync.ProgressDirection
import io.github.barqdb.kotlin.internal.interop.sync.SyncSessionResyncMode
import io.github.barqdb.kotlin.internal.interop.sync.WebSocketTransport
import io.github.barqdb.kotlin.internal.interop.sync.WebsocketCallbackResult
import io.github.barqdb.kotlin.internal.interop.sync.WebsocketErrorCode
import kotlinx.coroutines.CoroutineDispatcher
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.launch
import io.github.barqdb.kotlin.types.ObjectId

// FIXME API-CLEANUP Rename io.github.barqdb.interop. to something with platform?
//  https://github.com/BarqDB/barq-kotlin/issues/56

actual val INVALID_CLASS_KEY: ClassKey by lazy { ClassKey(barqc.getBARQ_INVALID_CLASS_KEY()) }
actual val INVALID_PROPERTY_KEY: PropertyKey by lazy { PropertyKey(barqc.getBARQ_INVALID_PROPERTY_KEY()) }

// The value to pass to JNI functions that accept longs as replacements for pointers and need
// to represent null.
const val NULL_POINTER_VALUE = 0L

/**
 * JVM/Android interop implementation.
 *
 * NOTE: All methods that return a boolean to indicate an exception are being checked automatically in JNI.
 * So there is no need to verify the return value in the JVM interop layer.
 */
@Suppress("LargeClass", "FunctionNaming", "TooGenericExceptionCaught")
actual object BarqInterop {

    actual fun barq_value_get(value: BarqValue): Any? = value.value

    actual fun barq_get_version_id(barq: BarqPointer): Long {
        val version = barq_version_id_t()
        val found = BooleanArray(1)
        barqc.barq_get_version_id(barq.cptr(), found, version)
        return if (found[0]) {
            version.version
        } else {
            throw IllegalStateException("No VersionId was available. Reading the VersionId requires a valid read transaction.")
        }
    }

    actual fun barq_get_library_version(): String {
        return barqc.barq_get_library_version()
    }

    actual fun barq_decimal128_from_string(value: String): ULongArray {
        val decimal = barq_decimal128_t()
        barqc.barq_decimal128_from_string(value, decimal)
        return decimal.w.toULongArray()
    }

    actual fun barq_decimal128_to_string(low: ULong, high: ULong): String {
        val decimal = barq_decimal128_t().apply {
            w = longArrayOf(low.toLong(), high.toLong())
        }
        return barqc.barq_decimal128_to_string(decimal)
    }

    actual fun barq_get_num_versions(barq: BarqPointer): Long {
        val result = LongArray(1)
        barqc.barq_get_num_versions(barq.cptr(), result)
        return result.first()
    }

    actual fun barq_refresh(barq: BarqPointer) {
        // Only returns `true` if the version changed, `false` if the version
        // was already at the latest. Errors will be represented by the actual
        // return value, so just ignore this out parameter.
        val didRefresh = booleanArrayOf(false)
        barqc.barq_refresh(barq.cptr(), didRefresh)
    }

    actual fun barq_schema_new(schema: List<Pair<ClassInfo, List<PropertyInfo>>>): BarqSchemaPointer {
        val count = schema.size
        val cclasses = barqc.new_classArray(count)
        val cproperties = barqc.new_propertyArrayArray(count)

        for ((i, entry) in schema.withIndex()) {
            val (clazz, properties) = entry

            val computedCount = properties.count { it.isComputed }

            // Class
            val cclass = barq_class_info_t().apply {
                name = clazz.name
                primary_key = clazz.primaryKey
                num_properties = (properties.size - computedCount).toLong()
                num_computed_properties = computedCount.toLong()
                key = INVALID_CLASS_KEY.key
                flags = clazz.flags
            }
            // Properties
            val classProperties = barqc.new_propertyArray(properties.size)
            for ((j, property) in properties.withIndex()) {
                val cproperty = barq_property_info_t().apply {
                    name = property.name
                    public_name = property.publicName
                    type = property.type.nativeValue
                    collection_type = property.collectionType.nativeValue
                    link_target = property.linkTarget
                    link_origin_property_name = property.linkOriginPropertyName
                    key = INVALID_PROPERTY_KEY.key
                    flags = property.flags
                }
                barqc.propertyArray_setitem(classProperties, j, cproperty)
            }
            barqc.classArray_setitem(cclasses, i, cclass)
            barqc.propertyArrayArray_setitem(cproperties, i, classProperties)
        }
        try {
            return LongPointerWrapper(barqc.barq_schema_new(cclasses, count.toLong(), cproperties))
        } finally {
            // Clean up classes
            for (classIndex in 0 until count) {
                val classInfo = barqc.classArray_getitem(cclasses, classIndex)

                // Clean properties
                val propertyArray = barqc.propertyArrayArray_getitem(cproperties, classIndex)

                val propertyCount = classInfo.getNum_properties() + classInfo.getNum_computed_properties()
                for (propertyIndex in 0 until propertyCount) {
                    val property = barqc.propertyArray_getitem(propertyArray, propertyIndex.toInt())

                    barqc.barq_property_info_t_cleanup(property)
                    property.delete()
                }

                barqc.delete_propertyArray(propertyArray)

                // end clean properties

                barqc.barq_class_info_t_cleanup(classInfo)
                classInfo.delete()
            }

            barqc.delete_propertyArrayArray(cproperties)
            barqc.delete_classArray(cclasses)
        }
    }

    actual fun barq_config_new(): BarqConfigurationPointer {
        return LongPointerWrapper(barqc.barq_config_new())
    }

    actual fun barq_config_set_path(config: BarqConfigurationPointer, path: String) {
        barqc.barq_config_set_path((config as LongPointerWrapper).ptr, path)
    }

    actual fun barq_config_set_schema_mode(config: BarqConfigurationPointer, mode: SchemaMode) {
        barqc.barq_config_set_schema_mode((config as LongPointerWrapper).ptr, mode.nativeValue)
    }

    actual fun barq_config_set_schema_version(config: BarqConfigurationPointer, version: Long) {
        barqc.barq_config_set_schema_version((config as LongPointerWrapper).ptr, version)
    }

    actual fun barq_config_set_schema(config: BarqConfigurationPointer, schema: BarqSchemaPointer) {
        barqc.barq_config_set_schema((config as LongPointerWrapper).ptr, (schema as LongPointerWrapper).ptr)
    }

    actual fun barq_config_set_max_number_of_active_versions(config: BarqConfigurationPointer, maxNumberOfVersions: Long) {
        barqc.barq_config_set_max_number_of_active_versions(config.cptr(), maxNumberOfVersions)
    }

    actual fun barq_config_set_encryption_key(config: BarqConfigurationPointer, encryptionKey: ByteArray) {
        barqc.barq_config_set_encryption_key(config.cptr(), encryptionKey, encryptionKey.size.toLong())
    }

    actual fun barq_config_get_encryption_key(config: BarqConfigurationPointer): ByteArray? {
        val key = ByteArray(ENCRYPTION_KEY_LENGTH)
        val keyLength: Long = barqc.barq_config_get_encryption_key(config.cptr(), key)

        if (keyLength == ENCRYPTION_KEY_LENGTH.toLong()) {
            return key
        }
        return null
    }

    actual fun barq_config_set_should_compact_on_launch_function(
        config: BarqConfigurationPointer,
        callback: CompactOnLaunchCallback
    ) {
        barqc.barq_config_set_should_compact_on_launch_function(config.cptr(), callback)
    }

    actual fun barq_config_set_migration_function(config: BarqConfigurationPointer, callback: MigrationCallback) {
        barqc.barq_config_set_migration_function(config.cptr(), callback)
    }

    actual fun barq_config_set_automatic_backlink_handling(config: BarqConfigurationPointer, enabled: Boolean) {
        barqc.barq_config_set_automatic_backlink_handling(config.cptr(), enabled)
    }

    actual fun barq_config_set_data_initialization_function(config: BarqConfigurationPointer, callback: DataInitializationCallback) {
        barqc.barq_config_set_data_initialization_function(config.cptr(), callback)
    }

    actual fun barq_config_set_in_memory(config: BarqConfigurationPointer, inMemory: Boolean) {
        barqc.barq_config_set_in_memory(config.cptr(), inMemory)
    }

    actual fun barq_create_scheduler(): BarqSchedulerPointer =
        LongPointerWrapper(barqc.barq_create_generic_scheduler())

    actual fun barq_create_scheduler(dispatcher: CoroutineDispatcher): BarqSchedulerPointer =
        LongPointerWrapper(barqc.barq_create_scheduler(JVMScheduler(dispatcher)))

    actual fun barq_open(
        config: BarqConfigurationPointer,
        scheduler: BarqSchedulerPointer,
    ): Pair<LiveBarqPointer, Boolean> {
        // Configure callback to track if the file was created as part of opening
        var fileCreated = false
        val callback = DataInitializationCallback {
            fileCreated = true
        }
        barq_config_set_data_initialization_function(config, callback)

        barqc.barq_config_set_scheduler(config.cptr(), scheduler.cptr())
        val barqPtr = LongPointerWrapper<LiveBarqT>(barqc.barq_open(config.cptr()))

        // Ensure that we can read version information, etc.
        barq_begin_read(barqPtr)
        return Pair(barqPtr, fileCreated)
    }

    actual fun barq_open_synchronized(config: BarqConfigurationPointer): BarqAsyncOpenTaskPointer {
        return LongPointerWrapper(barqc.barq_open_synchronized(config.cptr()))
    }

    actual fun barq_async_open_task_start(task: BarqAsyncOpenTaskPointer, callback: AsyncOpenCallback) {
        barqc.barq_async_open_task_start(task.cptr(), callback)
    }

    actual fun barq_async_open_task_cancel(task: BarqAsyncOpenTaskPointer) {
        barqc.barq_async_open_task_cancel(task.cptr())
    }

    actual fun barq_add_barq_changed_callback(barq: LiveBarqPointer, block: () -> Unit): BarqCallbackTokenPointer {
        return LongPointerWrapper(
            barqc.barq_add_barq_changed_callback(barq.cptr(), block),
            managed = false
        )
    }

    actual fun barq_add_schema_changed_callback(barq: LiveBarqPointer, block: (BarqSchemaPointer) -> Unit): BarqCallbackTokenPointer {
        return LongPointerWrapper(
            barqc.barq_add_schema_changed_callback(barq.cptr(), block),
            managed = false
        )
    }

    actual fun barq_freeze(liveBarq: LiveBarqPointer): FrozenBarqPointer {
        return LongPointerWrapper(barqc.barq_freeze(liveBarq.cptr()))
    }

    actual fun barq_is_frozen(barq: BarqPointer): Boolean {
        return barqc.barq_is_frozen(barq.cptr())
    }

    actual fun barq_close(barq: BarqPointer) {
        barqc.barq_close(barq.cptr())
    }

    actual fun barq_delete_files(path: String) {
        val deleted = booleanArrayOf(false)
        barqc.barq_delete_files(path, deleted)

        if (!deleted[0]) {
            throw IllegalStateException("It's not allowed to delete the file associated with an open Barq. Remember to call 'close()' on the instances of the barq before deleting its file: $path")
        }
    }

    actual fun barq_compact(barq: BarqPointer): Boolean {
        val compacted = booleanArrayOf(false)
        barqc.barq_compact(barq.cptr(), compacted)
        return compacted.first()
    }

    actual fun barq_convert_with_config(
        barq: BarqPointer,
        config: BarqConfigurationPointer,
        mergeWithExisting: Boolean
    ) {
        barqc.barq_convert_with_config(barq.cptr(), config.cptr(), mergeWithExisting)
    }

    actual fun barq_schema_validate(schema: BarqSchemaPointer, mode: SchemaValidationMode): Boolean {
        return barqc.barq_schema_validate(schema.cptr(), mode.nativeValue.toLong())
    }

    actual fun barq_get_schema(barq: BarqPointer): BarqSchemaPointer {
        return LongPointerWrapper(barqc.barq_get_schema(barq.cptr()))
    }

    actual fun barq_get_schema_version(barq: BarqPointer): Long {
        return barqc.barq_get_schema_version(barq.cptr())
    }

    actual fun barq_get_num_classes(barq: BarqPointer): Long {
        return barqc.barq_get_num_classes(barq.cptr())
    }

    actual fun barq_get_class_keys(barq: BarqPointer): List<ClassKey> {
        val count = barq_get_num_classes(barq)
        val keys = LongArray(count.toInt())
        val outCount = longArrayOf(0)
        barqc.barq_get_class_keys(barq.cptr(), keys, count, outCount)
        if (count != outCount[0]) {
            error("Invalid schema: Insufficient keys; got ${outCount[0]}, expected $count")
        }
        return keys.map { ClassKey(it) }
    }

    actual fun barq_find_class(barq: BarqPointer, name: String): ClassKey? {
        val info = barq_class_info_t()
        val found = booleanArrayOf(false)
        barqc.barq_find_class(barq.cptr(), name, found, info)
        return if (found[0]) {
            ClassKey(info.key)
        } else {
            null
        }
    }

    actual fun barq_get_class(barq: BarqPointer, classKey: ClassKey): ClassInfo {
        val info = barq_class_info_t()
        barqc.barq_get_class(barq.cptr(), classKey.key, info)
        return with(info) {
            ClassInfo(name, primary_key, num_properties, num_computed_properties, ClassKey(key), flags)
        }
    }
    actual fun barq_get_class_properties(barq: BarqPointer, classKey: ClassKey, max: Long): List<PropertyInfo> {
        val properties = barqc.new_propertyArray(max.toInt())
        val outCount = longArrayOf(0)
        barqc.barq_get_class_properties(barq.cptr(), classKey.key, properties, max, outCount)
        try {
            return if (outCount[0] > 0) {
                (0 until outCount[0]).map { i ->
                    with(barqc.propertyArray_getitem(properties, i.toInt())) {
                        PropertyInfo(
                            name,
                            public_name,
                            PropertyType.from(type),
                            CollectionType.from(collection_type),
                            link_target,
                            link_origin_property_name,
                            PropertyKey(key),
                            flags
                        )
                    }
                }
            } else {
                emptyList()
            }
        } finally {
            barqc.delete_propertyArray(properties)
        }
    }

    internal actual fun barq_release(p: BarqNativePointer) {
        barqc.barq_release(p.cptr())
    }

    actual fun barq_equals(p1: BarqNativePointer, p2: BarqNativePointer): Boolean {
        return barqc.barq_equals(p1.cptr(), p2.cptr())
    }

    actual fun barq_is_closed(barq: BarqPointer): Boolean {
        return barqc.barq_is_closed(barq.cptr())
    }

    actual fun barq_begin_read(barq: BarqPointer) {
        barqc.barq_begin_read(barq.cptr())
    }

    actual fun barq_begin_write(barq: LiveBarqPointer) {
        barqc.barq_begin_write(barq.cptr())
    }

    actual fun barq_commit(barq: LiveBarqPointer) {
        barqc.barq_commit(barq.cptr())
    }

    actual fun barq_rollback(barq: LiveBarqPointer) {
        barqc.barq_rollback(barq.cptr())
    }

    actual fun barq_is_in_transaction(barq: BarqPointer): Boolean {
        return barqc.barq_is_writable(barq.cptr())
    }

    actual fun barq_update_schema(barq: LiveBarqPointer, schema: BarqSchemaPointer) {
        barqc.barq_update_schema(barq.cptr(), schema.cptr())
    }

    actual fun barq_object_create(barq: LiveBarqPointer, classKey: ClassKey): BarqObjectPointer {
        return LongPointerWrapper(barqc.barq_object_create(barq.cptr(), classKey.key))
    }

    actual fun barq_object_create_with_primary_key(
        barq: LiveBarqPointer,
        classKey: ClassKey,
        primaryKeyTransport: BarqValue
    ): BarqObjectPointer {
        return LongPointerWrapper(
            barqc.barq_object_create_with_primary_key(
                barq.cptr(),
                classKey.key,
                primaryKeyTransport.value
            )
        )
    }

    actual fun barq_object_get_or_create_with_primary_key(
        barq: LiveBarqPointer,
        classKey: ClassKey,
        primaryKeyTransport: BarqValue
    ): BarqObjectPointer {
        val created = booleanArrayOf(false)
        return LongPointerWrapper(
            barqc.barq_object_get_or_create_with_primary_key(
                barq.cptr(),
                classKey.key,
                primaryKeyTransport.value,
                created
            )
        )
    }

    actual fun barq_object_is_valid(obj: BarqObjectPointer): Boolean {
        return barqc.barq_object_is_valid(obj.cptr())
    }

    actual fun barq_object_get_key(obj: BarqObjectPointer): ObjectKey {
        return ObjectKey(barqc.barq_object_get_key(obj.cptr()))
    }

    actual fun barq_object_resolve_in(obj: BarqObjectPointer, barq: BarqPointer): BarqObjectPointer? {
        val objectPointer = longArrayOf(0)
        barqc.barq_object_resolve_in(obj.cptr(), barq.cptr(), objectPointer)
        return if (objectPointer[0] != 0L) {
            LongPointerWrapper(objectPointer[0])
        } else {
            null
        }
    }

    actual fun barq_object_as_link(obj: BarqObjectPointer): Link {
        val link: barq_link_t = barqc.barq_object_as_link(obj.cptr())
        return Link(ClassKey(link.target_table), link.target)
    }

    actual fun barq_object_get_table(obj: BarqObjectPointer): ClassKey {
        return ClassKey(barqc.barq_object_get_table(obj.cptr()))
    }

    actual fun barq_get_col_key(
        barq: BarqPointer,
        classKey: ClassKey,
        col: String
    ): PropertyKey {
        return PropertyKey(propertyInfo(barq, classKey, col).key)
    }

    actual fun barq_add_vector_index(
        barq: LiveBarqPointer,
        classKey: ClassKey,
        columnKey: PropertyKey,
        config: VectorIndexConfig
    ) {
        val cconfig = barq_vector_index_config_t().apply {
            metric = config.metric
            encoding = config.encoding
            dimensions = config.dimensions
            m = config.m
            ef_construction = config.efConstruction
            ef_search = config.efSearch
        }
        barqc.barq_add_vector_index(barq.cptr(), classKey.key, columnKey.key, cconfig)
    }

    actual fun barq_remove_vector_index(
        barq: LiveBarqPointer,
        classKey: ClassKey,
        columnKey: PropertyKey
    ) {
        barqc.barq_remove_vector_index(barq.cptr(), classKey.key, columnKey.key)
    }

    actual fun barq_has_vector_index(
        barq: BarqPointer,
        classKey: ClassKey,
        columnKey: PropertyKey
    ): Boolean {
        val hasIndex = booleanArrayOf(false)
        barqc.barq_has_vector_index(barq.cptr(), classKey.key, columnKey.key, hasIndex)
        return hasIndex[0]
    }

    actual fun barq_get_vector_index_config(
        barq: BarqPointer,
        classKey: ClassKey,
        columnKey: PropertyKey
    ): VectorIndexConfig {
        val cconfig = barq_vector_index_config_t()
        barqc.barq_get_vector_index_config(barq.cptr(), classKey.key, columnKey.key, cconfig)
        return VectorIndexConfig(
            metric = cconfig.metric,
            encoding = cconfig.encoding,
            dimensions = cconfig.dimensions,
            m = cconfig.m,
            efConstruction = cconfig.ef_construction,
            efSearch = cconfig.ef_search
        )
    }

    actual fun barq_results_knn_search(
        results: BarqResultsPointer,
        columnKey: PropertyKey,
        queryVector: FloatArray,
        k: Long,
        ef: Long,
        exact: Boolean
    ): BarqResultsPointer {
        return LongPointerWrapper(
            barqc.barq_results_knn_search(
                results.cptr(),
                columnKey.key,
                queryVector,
                queryVector.size.toLong(),
                k,
                ef,
                exact
            )
        )
    }

    actual fun MemAllocator.barq_get_value(
        obj: BarqObjectPointer,
        key: PropertyKey
    ): BarqValue {
        val struct = allocBarqValueT()
        barqc.barq_get_value((obj as LongPointerWrapper).ptr, key.key, struct)
        return BarqValue(struct)
    }

    actual fun barq_set_value(
        obj: BarqObjectPointer,
        key: PropertyKey,
        value: BarqValue,
        isDefault: Boolean
    ) {
        barqc.barq_set_value(obj.cptr(), key.key, value.value, isDefault)
    }

    actual fun barq_set_embedded(obj: BarqObjectPointer, key: PropertyKey): BarqObjectPointer {
        return LongPointerWrapper(barqc.barq_set_embedded(obj.cptr(), key.key))
    }

    actual fun barq_set_list(obj: BarqObjectPointer, key: PropertyKey): BarqListPointer {
        barqc.barq_set_list(obj.cptr(), key.key)
        return barq_get_list(obj, key)
    }
    actual fun barq_set_dictionary(obj: BarqObjectPointer, key: PropertyKey): BarqMapPointer {
        barqc.barq_set_dictionary(obj.cptr(), key.key)
        return barq_get_dictionary(obj, key)
    }

    actual fun barq_object_add_int(obj: BarqObjectPointer, key: PropertyKey, value: Long) {
        barqc.barq_object_add_int(obj.cptr(), key.key, value)
    }

    actual fun <T> barq_object_get_parent(
        obj: BarqObjectPointer,
        block: (ClassKey, BarqObjectPointer) -> T
    ): T {
        val objectPointerArray = longArrayOf(0)
        val classKeyPointerArray = longArrayOf(0)

        barqc.barq_object_get_parent(
            /* object = */ obj.cptr(),
            /* parent = */ objectPointerArray,
            /* class_key = */ classKeyPointerArray
        )

        val classKey = ClassKey(classKeyPointerArray[0])
        val objectPointer = LongPointerWrapper<BarqObjectT>(objectPointerArray[0])

        return block(classKey, objectPointer)
    }

    actual fun barq_get_list(obj: BarqObjectPointer, key: PropertyKey): BarqListPointer {
        return LongPointerWrapper(
            barqc.barq_get_list(
                (obj as LongPointerWrapper).ptr,
                key.key
            )
        )
    }

    actual fun barq_get_backlinks(obj: BarqObjectPointer, sourceClassKey: ClassKey, sourcePropertyKey: PropertyKey): BarqResultsPointer {
        return LongPointerWrapper(
            barqc.barq_get_backlinks(
                (obj as LongPointerWrapper).ptr,
                sourceClassKey.key,
                sourcePropertyKey.key
            )
        )
    }

    actual fun barq_list_size(list: BarqListPointer): Long {
        val size = LongArray(1)
        barqc.barq_list_size(list.cptr(), size)
        return size[0]
    }

    actual fun MemAllocator.barq_list_get(
        list: BarqListPointer,
        index: Long
    ): BarqValue {
        val struct = allocBarqValueT()
        barqc.barq_list_get(list.cptr(), index, struct)
        return BarqValue(struct)
    }

    actual fun barq_list_find(list: BarqListPointer, value: BarqValue): Long {
        val index = LongArray(1)
        val found = BooleanArray(1)
        barqc.barq_list_find(list.cptr(), value.value, index, found)
        return if (found[0]) {
            index[0]
        } else {
            INDEX_NOT_FOUND
        }
    }

    actual fun barq_list_get_list(list: BarqListPointer, index: Long): BarqListPointer =
        LongPointerWrapper(barqc.barq_list_get_list(list.cptr(), index))

    actual fun barq_list_get_dictionary(list: BarqListPointer, index: Long): BarqMapPointer =
        LongPointerWrapper(barqc.barq_list_get_dictionary(list.cptr(), index))

    actual fun barq_list_add(list: BarqListPointer, index: Long, transport: BarqValue) {
        barqc.barq_list_insert(list.cptr(), index, transport.value)
    }

    actual fun barq_list_insert_embedded(list: BarqListPointer, index: Long): BarqObjectPointer {
        return LongPointerWrapper(barqc.barq_list_insert_embedded(list.cptr(), index))
    }
    actual fun barq_list_insert_list(list: BarqListPointer, index: Long): BarqListPointer {
        return LongPointerWrapper(barqc.barq_list_insert_list(list.cptr(), index))
    }
    actual fun barq_list_insert_dictionary(list: BarqListPointer, index: Long): BarqMapPointer {
        return LongPointerWrapper(barqc.barq_list_insert_dictionary(list.cptr(), index))
    }
    actual fun barq_list_set_list(list: BarqListPointer, index: Long): BarqListPointer {
        return LongPointerWrapper(barqc.barq_list_set_list(list.cptr(), index))
    }
    actual fun barq_list_set_dictionary(list: BarqListPointer, index: Long): BarqMapPointer {
        return LongPointerWrapper(barqc.barq_list_set_dictionary(list.cptr(), index))
    }

    actual fun barq_list_set(
        list: BarqListPointer,
        index: Long,
        inputTransport: BarqValue
    ) {
        barqc.barq_list_set(list.cptr(), index, inputTransport.value)
    }

    actual fun MemAllocator.barq_list_set_embedded(
        list: BarqListPointer,
        index: Long
    ): BarqValue {
        val struct = allocBarqValueT()

        // Returns the new object as a Link to follow convention of other getters and allow to
        // reuse the converter infrastructure
        val embedded = barqc.barq_list_set_embedded(list.cptr(), index)
        val link: barq_link_t = barqc.barq_object_as_link(embedded)
        return BarqValue(
            struct.apply {
                this.type = barq_value_type_e.BARQ_TYPE_LINK
                this.link = link
            }
        )
    }

    actual fun barq_list_clear(list: BarqListPointer) {
        barqc.barq_list_clear(list.cptr())
    }

    actual fun barq_list_remove_all(list: BarqListPointer) {
        barqc.barq_list_remove_all(list.cptr())
    }

    actual fun barq_list_erase(list: BarqListPointer, index: Long) {
        barqc.barq_list_erase(list.cptr(), index)
    }

    actual fun barq_list_resolve_in(
        list: BarqListPointer,
        barq: BarqPointer
    ): BarqListPointer? {
        val listPointer = longArrayOf(0)
        barqc.barq_list_resolve_in(list.cptr(), barq.cptr(), listPointer)
        return if (listPointer[0] != 0L) {
            LongPointerWrapper(listPointer[0])
        } else {
            null
        }
    }

    actual fun barq_list_is_valid(list: BarqListPointer): Boolean {
        return barqc.barq_list_is_valid(list.cptr())
    }

    actual fun barq_get_set(obj: BarqObjectPointer, key: PropertyKey): BarqSetPointer {
        return LongPointerWrapper(barqc.barq_get_set(obj.cptr(), key.key))
    }

    actual fun barq_set_size(set: BarqSetPointer): Long {
        val size = LongArray(1)
        barqc.barq_set_size(set.cptr(), size)
        return size[0]
    }

    actual fun barq_set_clear(set: BarqSetPointer) {
        barqc.barq_set_clear(set.cptr())
    }

    actual fun barq_set_insert(set: BarqSetPointer, transport: BarqValue): Boolean {
        val size = LongArray(1)
        val inserted = BooleanArray(1)
        barqc.barq_set_insert(set.cptr(), transport.value, size, inserted)
        return inserted[0]
    }

    // See comment in darwin implementation as to why we don't return null just like we do in other
    // functions.
    actual fun MemAllocator.barq_set_get(
        set: BarqSetPointer,
        index: Long
    ): BarqValue {
        val struct = allocBarqValueT()
        barqc.barq_set_get(set.cptr(), index, struct)
        return BarqValue(struct)
    }

    actual fun barq_set_find(set: BarqSetPointer, transport: BarqValue): Boolean {
        val index = LongArray(1)
        val found = BooleanArray(1)
        barqc.barq_set_find(set.cptr(), transport.value, index, found)
        return found[0]
    }

    actual fun barq_set_erase(set: BarqSetPointer, transport: BarqValue): Boolean {
        val erased = BooleanArray(1)
        barqc.barq_set_erase(set.cptr(), transport.value, erased)
        return erased[0]
    }

    actual fun barq_set_remove_all(set: BarqSetPointer) {
        barqc.barq_set_remove_all(set.cptr())
    }

    actual fun barq_set_resolve_in(set: BarqSetPointer, barq: BarqPointer): BarqSetPointer? {
        val setPointer = longArrayOf(0)
        barqc.barq_set_resolve_in(set.cptr(), barq.cptr(), setPointer)
        return if (setPointer[0] != 0L) {
            LongPointerWrapper(setPointer[0])
        } else {
            null
        }
    }

    actual fun barq_set_is_valid(set: BarqSetPointer): Boolean {
        return barqc.barq_set_is_valid(set.cptr())
    }

    actual fun barq_get_dictionary(
        obj: BarqObjectPointer,
        key: PropertyKey
    ): BarqMapPointer {
        val ptr = barqc.barq_get_dictionary(obj.cptr(), key.key)
        return LongPointerWrapper(ptr)
    }

    actual fun barq_dictionary_clear(dictionary: BarqMapPointer) {
        barqc.barq_dictionary_clear(dictionary.cptr())
    }

    actual fun barq_dictionary_size(dictionary: BarqMapPointer): Long {
        val size = LongArray(1)
        barqc.barq_dictionary_size(dictionary.cptr(), size)
        return size[0]
    }

    actual fun barq_dictionary_to_results(
        dictionary: BarqMapPointer
    ): BarqResultsPointer {
        return LongPointerWrapper(barqc.barq_dictionary_to_results(dictionary.cptr()))
    }

    actual fun MemAllocator.barq_dictionary_find(
        dictionary: BarqMapPointer,
        mapKey: BarqValue
    ): BarqValue {
        val found = BooleanArray(1)
        val struct = allocBarqValueT()
        // Core will always return a barq_value_t, even if the value was not found, in which case
        // the type of the struct will be BARQ_TYPE_NULL. This way we signal our converters not to
        // worry about nullability and just translate the struct types to their corresponding Kotlin
        // types.
        barqc.barq_dictionary_find(dictionary.cptr(), mapKey.value, struct, found)
        return BarqValue(struct)
    }

    actual fun barq_dictionary_find_list(
        dictionary: BarqMapPointer,
        mapKey: BarqValue
    ): BarqListPointer {
        return LongPointerWrapper(barqc.barq_dictionary_get_list(dictionary.cptr(), mapKey.value))
    }
    actual fun barq_dictionary_find_dictionary(
        dictionary: BarqMapPointer,
        mapKey: BarqValue
    ): BarqMapPointer {
        return LongPointerWrapper(barqc.barq_dictionary_get_dictionary(dictionary.cptr(), mapKey.value))
    }

    actual fun MemAllocator.barq_dictionary_get(
        dictionary: BarqMapPointer,
        pos: Int
    ): Pair<BarqValue, BarqValue> {
        val keyTransport = allocBarqValueT()
        val valueTransport = allocBarqValueT()
        barqc.barq_dictionary_get(dictionary.cptr(), pos.toLong(), keyTransport, valueTransport)
        return Pair(BarqValue(keyTransport), BarqValue(valueTransport))
    }

    actual fun MemAllocator.barq_dictionary_insert(
        dictionary: BarqMapPointer,
        mapKey: BarqValue,
        value: BarqValue
    ): Pair<BarqValue, Boolean> {
        val previousValue = barq_dictionary_find(dictionary, mapKey)
        val index = LongArray(1)
        val inserted = BooleanArray(1)
        barqc.barq_dictionary_insert(dictionary.cptr(), mapKey.value, value.value, index, inserted)
        return Pair(previousValue, inserted[0])
    }

    actual fun MemAllocator.barq_dictionary_erase(
        dictionary: BarqMapPointer,
        mapKey: BarqValue
    ): Pair<BarqValue, Boolean> {
        val previousValue = barq_dictionary_find(dictionary, mapKey)
        val erased = BooleanArray(1)
        barqc.barq_dictionary_erase(dictionary.cptr(), mapKey.value, erased)
        return Pair(previousValue, erased[0])
    }

    actual fun barq_dictionary_contains_key(
        dictionary: BarqMapPointer,
        mapKey: BarqValue
    ): Boolean {
        val found = BooleanArray(1)
        barqc.barq_dictionary_contains_key(dictionary.cptr(), mapKey.value, found)
        return found[0]
    }

    actual fun barq_dictionary_contains_value(
        dictionary: BarqMapPointer,
        value: BarqValue
    ): Boolean {
        val index = LongArray(1)
        barqc.barq_dictionary_contains_value(dictionary.cptr(), value.value, index)
        return index[0] != -1L
    }

    actual fun MemAllocator.barq_dictionary_insert_embedded(
        dictionary: BarqMapPointer,
        mapKey: BarqValue
    ): BarqValue {
        val struct = allocBarqValueT()

        // Returns the new object as a Link to follow convention of other getters and allow to
        // reuse the converter infrastructure
        val embedded = barqc.barq_dictionary_insert_embedded(dictionary.cptr(), mapKey.value)
        val link: barq_link_t = barqc.barq_object_as_link(embedded)
        return BarqValue(
            struct.apply {
                this.type = barq_value_type_e.BARQ_TYPE_LINK
                this.link = link
            }
        )
    }

    actual fun barq_dictionary_insert_list(dictionary: BarqMapPointer, mapKey: BarqValue): BarqListPointer {
        return LongPointerWrapper(barqc.barq_dictionary_insert_list(dictionary.cptr(), mapKey.value))
    }

    actual fun barq_dictionary_insert_dictionary(dictionary: BarqMapPointer, mapKey: BarqValue): BarqMapPointer {
        return LongPointerWrapper(barqc.barq_dictionary_insert_dictionary(dictionary.cptr(), mapKey.value))
    }

    actual fun barq_dictionary_get_keys(dictionary: BarqMapPointer): BarqResultsPointer {
        val size = LongArray(1)
        val keysPointer = longArrayOf(0)
        barqc.barq_dictionary_get_keys(dictionary.cptr(), size, keysPointer)
        return if (keysPointer[0] != 0L) {
            LongPointerWrapper(keysPointer[0])
        } else {
            throw IllegalArgumentException("There was an error retrieving the dictionary keys.")
        }
    }

    actual fun barq_dictionary_resolve_in(
        dictionary: BarqMapPointer,
        barq: BarqPointer
    ): BarqMapPointer? {
        val dictionaryPointer = longArrayOf(0)
        barqc.barq_set_resolve_in(dictionary.cptr(), barq.cptr(), dictionaryPointer)
        return if (dictionaryPointer[0] != 0L) {
            LongPointerWrapper(dictionaryPointer[0])
        } else {
            null
        }
    }

    actual fun barq_dictionary_is_valid(dictionary: BarqMapPointer): Boolean {
        return barqc.barq_dictionary_is_valid(dictionary.cptr())
    }

    actual fun barq_create_key_paths_array(barq: BarqPointer, clazz: ClassKey, keyPaths: List<String>): BarqKeyPathArrayPointer {
        val ptr = barqc.barq_create_key_path_array(barq.cptr(), clazz.key, keyPaths.size.toLong(), keyPaths.toTypedArray())
        return LongPointerWrapper(ptr)
    }

    actual fun barq_object_add_notification_callback(
        obj: BarqObjectPointer,
        keyPaths: BarqKeyPathArrayPointer?,
        callback: Callback<BarqChangesPointer>
    ): BarqNotificationTokenPointer {

        return LongPointerWrapper(
            barqc.register_notification_cb(
                obj.cptr(),
                CollectionType.BARQ_COLLECTION_TYPE_NONE.nativeValue,
                keyPaths?.cptr() ?: NULL_POINTER_VALUE,
                object : NotificationCallback {
                    override fun onChange(pointer: Long) {
                        callback.onChange(LongPointerWrapper(barqc.barq_clone(pointer), true))
                    }
                }
            ),
            managed = false
        )
    }

    actual fun barq_results_add_notification_callback(
        results: BarqResultsPointer,
        keyPaths: BarqKeyPathArrayPointer?,
        callback: Callback<BarqChangesPointer>
    ): BarqNotificationTokenPointer {
        return LongPointerWrapper(
            barqc.register_results_notification_cb(
                results.cptr(),
                keyPaths?.cptr() ?: NULL_POINTER_VALUE,
                object : NotificationCallback {
                    override fun onChange(pointer: Long) {
                        callback.onChange(LongPointerWrapper(barqc.barq_clone(pointer), true))
                    }
                }
            ),
            managed = false
        )
    }

    actual fun barq_list_add_notification_callback(
        list: BarqListPointer,
        keyPaths: BarqKeyPathArrayPointer?,
        callback: Callback<BarqChangesPointer>
    ): BarqNotificationTokenPointer {
        return LongPointerWrapper(
            barqc.register_notification_cb(
                list.cptr(),
                CollectionType.BARQ_COLLECTION_TYPE_LIST.nativeValue,
                keyPaths?.cptr() ?: NULL_POINTER_VALUE,
                object : NotificationCallback {
                    override fun onChange(pointer: Long) {
                        callback.onChange(LongPointerWrapper(barqc.barq_clone(pointer), true))
                    }
                }
            ),
            managed = false
        )
    }

    actual fun barq_set_add_notification_callback(
        set: BarqSetPointer,
        keyPaths: BarqKeyPathArrayPointer?,
        callback: Callback<BarqChangesPointer>
    ): BarqNotificationTokenPointer {
        return LongPointerWrapper(
            barqc.register_notification_cb(
                set.cptr(),
                CollectionType.BARQ_COLLECTION_TYPE_SET.nativeValue,
                keyPaths?.cptr() ?: NULL_POINTER_VALUE,
                object : NotificationCallback {
                    override fun onChange(pointer: Long) {
                        callback.onChange(LongPointerWrapper(barqc.barq_clone(pointer), true))
                    }
                }
            ),
            managed = false
        )
    }

    actual fun barq_dictionary_add_notification_callback(
        map: BarqMapPointer,
        keyPaths: BarqKeyPathArrayPointer?,
        callback: Callback<BarqChangesPointer>
    ): BarqNotificationTokenPointer {
        return LongPointerWrapper(
            barqc.register_notification_cb(
                map.cptr(),
                CollectionType.BARQ_COLLECTION_TYPE_DICTIONARY.nativeValue,
                keyPaths?.cptr() ?: NULL_POINTER_VALUE,
                object : NotificationCallback {
                    override fun onChange(pointer: Long) {
                        callback.onChange(LongPointerWrapper(barqc.barq_clone(pointer), true))
                    }
                }
            ),
            managed = false
        )
    }

    actual fun barq_object_changes_get_modified_properties(change: BarqChangesPointer): List<PropertyKey> {
        val propertyCount = barqc.barq_object_changes_get_num_modified_properties(change.cptr())

        val keys = LongArray(propertyCount.toInt())
        barqc.barq_object_changes_get_modified_properties(change.cptr(), keys, propertyCount)
        return keys.map { PropertyKey(it) }
    }

    private fun initIndicesArray(size: LongArray): LongArray = LongArray(size[0].toInt())
    @Suppress("UnusedPrivateMember")
    private fun initRangeArray(size: LongArray): Array<LongArray> = Array(size[0].toInt()) { LongArray(2) }

    actual fun <T, R> barq_collection_changes_get_indices(change: BarqChangesPointer, builder: CollectionChangeSetBuilder<T, R>) {
        val insertionCount = LongArray(1)
        val deletionCount = LongArray(1)
        val modificationCount = LongArray(1)
        val movesCount = LongArray(1)
        // Not exposed in SDK yet, but could be used to provide optimized notifications when
        // collections are cleared.
        //  https://github.com/BarqDB/barq-kotlin/issues/1498
        val collectionWasCleared = BooleanArray(1)
        val collectionWasDeleted = BooleanArray(1)

        barqc.barq_collection_changes_get_num_changes(
            change.cptr(),
            deletionCount,
            insertionCount,
            modificationCount,
            movesCount,
            collectionWasCleared,
            collectionWasDeleted,
        )

        val insertionIndices: LongArray = initIndicesArray(insertionCount)
        val modificationIndices: LongArray = initIndicesArray(modificationCount)
        val modificationIndicesAfter: LongArray = initIndicesArray(modificationCount)
        val deletionIndices: LongArray = initIndicesArray(deletionCount)
        val moves: barq_collection_move_t = barqc.new_collectionMoveArray(movesCount[0].toInt())

        barqc.barq_collection_changes_get_changes(
            change.cptr(),
            deletionIndices,
            deletionCount[0],
            insertionIndices,
            insertionCount[0],
            modificationIndices,
            modificationCount[0],
            modificationIndicesAfter,
            modificationCount[0],
            moves,
            movesCount[0]
        )

        builder.initIndicesArray(builder::insertionIndices, insertionIndices)
        builder.initIndicesArray(builder::deletionIndices, deletionIndices)
        builder.initIndicesArray(builder::modificationIndices, modificationIndices)
        builder.initIndicesArray(builder::modificationIndicesAfter, modificationIndicesAfter)
        builder.movesCount = movesCount[0].toInt()

        barqc.delete_collectionMoveArray(moves)
    }

    actual fun <T, R> barq_collection_changes_get_ranges(
        change: BarqChangesPointer,
        builder: CollectionChangeSetBuilder<T, R>
    ) {
        val insertRangesCount = LongArray(1)
        val deleteRangesCount = LongArray(1)
        val modificationRangesCount = LongArray(1)
        val movesCount = LongArray(1)

        barqc.barq_collection_changes_get_num_ranges(
            change.cptr(),
            deleteRangesCount,
            insertRangesCount,
            modificationRangesCount,
            movesCount
        )

        val insertionRanges: barq_index_range_t =
            barqc.new_indexRangeArray(insertRangesCount[0].toInt())
        val modificationRanges: barq_index_range_t =
            barqc.new_indexRangeArray(modificationRangesCount[0].toInt())
        val modificationRangesAfter: barq_index_range_t =
            barqc.new_indexRangeArray(modificationRangesCount[0].toInt())
        val deletionRanges: barq_index_range_t =
            barqc.new_indexRangeArray(deleteRangesCount[0].toInt())
        val moves: barq_collection_move_t = barqc.new_collectionMoveArray(movesCount[0].toInt())

        barqc.barq_collection_changes_get_ranges(
            change.cptr(),
            deletionRanges,
            deleteRangesCount[0],
            insertionRanges,
            insertRangesCount[0],
            modificationRanges,
            modificationRangesCount[0],
            modificationRangesAfter,
            modificationRangesCount[0],
            moves,
            movesCount[0]
        )

        builder.initRangesArray(builder::deletionRanges, deletionRanges, deleteRangesCount[0])
        builder.initRangesArray(builder::insertionRanges, insertionRanges, insertRangesCount[0])
        builder.initRangesArray(builder::modificationRanges, modificationRanges, modificationRangesCount[0])
        builder.initRangesArray(builder::modificationRangesAfter, modificationRangesAfter, modificationRangesCount[0])

        barqc.delete_indexRangeArray(insertionRanges)
        barqc.delete_indexRangeArray(modificationRanges)
        barqc.delete_indexRangeArray(modificationRangesAfter)
        barqc.delete_indexRangeArray(deletionRanges)
        barqc.delete_collectionMoveArray(moves)
    }

    actual fun <R> barq_dictionary_get_changes(
        change: BarqChangesPointer,
        builder: DictionaryChangeSetBuilder<R>
    ) {
        val deletions = longArrayOf(0)
        val insertions = longArrayOf(0)
        val modifications = longArrayOf(0)
        val collectionWasDeleted = BooleanArray(1)
        barqc.barq_dictionary_get_changes(
            change.cptr(),
            deletions,
            insertions,
            modifications,
            collectionWasDeleted,
        )

        val deletionStructs = barqc.new_valueArray(deletions[0].toInt())
        val insertionStructs = barqc.new_valueArray(insertions[0].toInt())
        val modificationStructs = barqc.new_valueArray(modifications[0].toInt())
        // Not exposed in SDK yet, but could be used to provide optimized notifications when
        // collections are cleared.
        //  https://github.com/BarqDB/barq-kotlin/issues/1498
        val collectionWasCleared = booleanArrayOf(false)
        barqc.barq_dictionary_get_changed_keys(
            change.cptr(),
            deletionStructs,
            deletions,
            insertionStructs,
            insertions,
            modificationStructs,
            modifications,
            collectionWasCleared,
        )

        // TODO optimize - integrate within mem allocator?
        // Get keys and release array of structs
        val deletedKeys = (0 until deletions[0]).map {
            barqc.valueArray_getitem(deletionStructs, it.toInt()).string
        }
        val insertedKeys = (0 until insertions[0]).map {
            barqc.valueArray_getitem(insertionStructs, it.toInt()).string
        }
        val modifiedKeys = (0 until modifications[0]).map {
            barqc.valueArray_getitem(modificationStructs, it.toInt()).string
        }
        barqc.delete_valueArray(deletionStructs)
        barqc.delete_valueArray(insertionStructs)
        barqc.delete_valueArray(modificationStructs)

        builder.initDeletions(deletedKeys.toTypedArray())
        builder.initInsertions(insertedKeys.toTypedArray())
        builder.initModifications(modifiedKeys.toTypedArray())
    }

    actual fun barq_sync_client_config_set_default_binding_thread_observer(syncClientConfig: BarqSyncClientConfigurationPointer, appId: String) {
        barqc.barq_sync_client_config_set_default_binding_thread_observer(
            syncClientConfig.cptr(),
            object : SyncThreadObserver {
                override fun threadName(): String {
                    return "SyncThread-$appId"
                }

                override fun onCreated() {
                    // We cannot set the name on JNI side as it would require access to JNIEnv before
                    // we attach it, so we set the thread name after it is created.
                    Thread.currentThread().name = threadName()
                }

                override fun onDestroyed() {
                    // Do nothing
                    // Thread is destroyed in the JNI side
                }

                @Suppress("TooGenericExceptionThrown")
                override fun onError(error: String) {
                    // TODO Wait for https://github.com/BarqDB/barq-core/issues/4194 to correctly
                    //  log errors. For now, just throw an Error as exceptions from the Sync Client
                    //  indicate that something is fundamentally wrong on the Sync Thread.
                    //  In Barq Java this has only been reported during development of new
                    //  features, so throwing an Error seems appropriate to increase visibility.
                    throw Error("[${threadName()}] Error on sync thread : $error")
                }
            }
        )
    }

    actual fun barq_sync_client_config_set_multiplex_sessions(syncClientConfig: BarqSyncClientConfigurationPointer, enabled: Boolean) {
        barqc.barq_sync_client_config_set_multiplex_sessions(syncClientConfig.cptr(), enabled)
    }

    actual fun barq_set_log_callback(callback: LogCallback) {
        barqc.set_log_callback(callback)
    }

    actual fun barq_set_log_level(level: CoreLogLevel) {
        barqc.barq_set_log_level(level.priority)
    }

    actual fun barq_set_log_level_category(category: String, level: CoreLogLevel) {
        barqc.barq_set_log_level_category(category, level.priority)
    }

    actual fun barq_get_log_level_category(category: String): CoreLogLevel =
        CoreLogLevel.valueFromPriority(barqc.barq_get_log_level_category(category).toShort())

    actual fun barq_get_category_names(): List<String> {
        @Suppress("UNCHECKED_CAST")
        val names: Array<String> = barqc.barq_get_log_category_names() as Array<String>
        return names.asList()
    }

    actual fun barq_sync_client_config_set_user_agent_binding_info(
        syncClientConfig: BarqSyncClientConfigurationPointer,
        bindingInfo: String
    ) {
        barqc.barq_sync_client_config_set_user_agent_binding_info(
            syncClientConfig.cptr(),
            bindingInfo
        )
    }

    actual fun barq_sync_client_config_set_user_agent_application_info(
        syncClientConfig: BarqSyncClientConfigurationPointer,
        applicationInfo: String
    ) {
        barqc.barq_sync_client_config_set_user_agent_application_info(
            syncClientConfig.cptr(),
            applicationInfo
        )
    }

    actual fun barq_sync_client_config_set_connect_timeout(syncClientConfig: BarqSyncClientConfigurationPointer, timeoutMs: ULong) {
        barqc.barq_sync_client_config_set_connect_timeout(syncClientConfig.cptr(), timeoutMs.toLong())
    }

    actual fun barq_sync_client_config_set_connection_linger_time(syncClientConfig: BarqSyncClientConfigurationPointer, timeoutMs: ULong) {
        barqc.barq_sync_client_config_set_connection_linger_time(syncClientConfig.cptr(), timeoutMs.toLong())
    }

    actual fun barq_sync_client_config_set_ping_keepalive_period(syncClientConfig: BarqSyncClientConfigurationPointer, timeoutMs: ULong) {
        barqc.barq_sync_client_config_set_ping_keepalive_period(syncClientConfig.cptr(), timeoutMs.toLong())
    }

    actual fun barq_sync_client_config_set_pong_keepalive_timeout(syncClientConfig: BarqSyncClientConfigurationPointer, timeoutMs: ULong) {
        barqc.barq_sync_client_config_set_pong_keepalive_timeout(syncClientConfig.cptr(), timeoutMs.toLong())
    }

    actual fun barq_sync_client_config_set_fast_reconnect_limit(syncClientConfig: BarqSyncClientConfigurationPointer, timeoutMs: ULong) {
        barqc.barq_sync_client_config_set_fast_reconnect_limit(syncClientConfig.cptr(), timeoutMs.toLong())
    }

    actual fun barq_network_transport_new(networkTransport: NetworkTransport): BarqNetworkTransportPointer {
        return LongPointerWrapper(barqc.barq_network_transport_new(networkTransport))
    }

    actual fun barq_sync_config_set_error_handler(
        syncConfig: BarqSyncConfigurationPointer,
        errorHandler: SyncErrorCallback
    ) {
        barqc.sync_set_error_handler(syncConfig.cptr(), errorHandler)
    }

    actual fun barq_sync_config_set_resync_mode(
        syncConfig: BarqSyncConfigurationPointer,
        resyncMode: SyncSessionResyncMode
    ) {
        barqc.barq_sync_config_set_resync_mode(syncConfig.cptr(), resyncMode.nativeValue)
    }

    actual fun barq_sync_config_set_before_client_reset_handler(
        syncConfig: BarqSyncConfigurationPointer,
        beforeHandler: SyncBeforeClientResetHandler
    ) {
        barqc.sync_before_client_reset_handler(syncConfig.cptr(), beforeHandler)
    }

    actual fun barq_sync_config_set_after_client_reset_handler(
        syncConfig: BarqSyncConfigurationPointer,
        afterHandler: SyncAfterClientResetHandler
    ) {
        barqc.sync_after_client_reset_handler(syncConfig.cptr(), afterHandler)
    }

    actual fun barq_sync_session_get(barq: BarqPointer): BarqSyncSessionPointer {
        return LongPointerWrapper(barqc.barq_sync_session_get(barq.cptr()))
    }

    actual fun barq_sync_session_wait_for_download_completion(
        syncSession: BarqSyncSessionPointer,
        callback: SyncSessionTransferCompletionCallback
    ) {
        barqc.barq_sync_session_wait_for_download_completion(
            syncSession.cptr(),
            JVMSyncSessionTransferCompletionCallback(callback)
        )
    }

    actual fun barq_sync_session_wait_for_upload_completion(
        syncSession: BarqSyncSessionPointer,
        callback: SyncSessionTransferCompletionCallback
    ) {
        barqc.barq_sync_session_wait_for_upload_completion(
            syncSession.cptr(),
            JVMSyncSessionTransferCompletionCallback(callback)
        )
    }

    actual fun barq_sync_session_state(syncSession: BarqSyncSessionPointer): CoreSyncSessionState {
        return CoreSyncSessionState.of(barqc.barq_sync_session_get_state(syncSession.cptr()))
    }
    actual fun barq_sync_connection_state(syncSession: BarqSyncSessionPointer): CoreConnectionState {
        return CoreConnectionState.of(barqc.barq_sync_session_get_connection_state(syncSession.cptr()))
    }

    actual fun barq_sync_session_pause(syncSession: BarqSyncSessionPointer) {
        barqc.barq_sync_session_pause(syncSession.cptr())
    }

    actual fun barq_sync_session_resume(syncSession: BarqSyncSessionPointer) {
        barqc.barq_sync_session_resume(syncSession.cptr())
    }

    actual fun barq_sync_session_handle_error_for_testing(
        syncSession: BarqSyncSessionPointer,
        error: ErrorCode,
        errorMessage: String,
        isFatal: Boolean
    ) {
        barqc.barq_sync_session_handle_error_for_testing(
            syncSession.cptr(),
            error.nativeValue,
            errorMessage,
            isFatal
        )
    }

    actual fun barq_sync_session_register_progress_notifier(
        syncSession: BarqSyncSessionPointer,
        direction: ProgressDirection,
        isStreaming: Boolean,
        callback: ProgressCallback,
    ): BarqNotificationTokenPointer {
        return LongPointerWrapper(
            barqc.barq_sync_session_register_progress_notifier_wrapper(
                syncSession.cptr(),
                direction.nativeValue,
                isStreaming,
                callback
            ),
            managed = false
        )
    }

    actual fun barq_sync_session_register_connection_state_change_callback(
        syncSession: BarqSyncSessionPointer,
        callback: ConnectionStateChangeCallback,
    ): BarqNotificationTokenPointer {
        return LongPointerWrapper(
            barqc.barq_sync_session_register_connection_state_change_callback(
                syncSession.cptr(),
                callback
            ),
            managed = false
        )
    }

    @Suppress("LongParameterList")

    actual fun barq_sync_user_new_from_token(
        tenantId: String,
        userId: String,
        accessToken: String
    ): BarqUserPointer {
        return LongPointerWrapper(barqc.barq_sync_user_new_from_token(tenantId, userId, accessToken))
    }

    actual fun barq_sync_user_set_route(user: BarqUserPointer, route: String, verified: Boolean) {
        barqc.barq_sync_user_set_route(user.cptr(), route, verified)
    }

    actual fun barq_sync_user_set_access_token(user: BarqUserPointer, accessToken: String) {
        barqc.barq_sync_user_set_access_token(user.cptr(), accessToken)
    }

    actual fun barq_sync_user_mark_access_token_refresh_required(user: BarqUserPointer) {
        barqc.barq_sync_user_mark_access_token_refresh_required(user.cptr())
    }

    actual fun barq_sync_user_make_sync_config(
        user: BarqUserPointer,
        partition: String
    ): BarqSyncConfigurationPointer {
        return LongPointerWrapper<BarqSyncConfigT>(barqc.barq_sync_user_make_sync_config(user.cptr(), partition)).also { ptr ->
            // Stop the session immediately when the Barq is closed, so the lifecycle of the
            // Sync Client thread is manageable.
            barqc.barq_sync_config_set_session_stop_policy(ptr.cptr(), barq_sync_session_stop_policy_e.BARQ_SYNC_SESSION_STOP_POLICY_IMMEDIATELY)
        }
    }

    actual fun barq_sync_user_make_flexible_sync_config(user: BarqUserPointer): BarqSyncConfigurationPointer {
        return LongPointerWrapper<BarqSyncConfigT>(barqc.barq_sync_user_make_flexible_sync_config(user.cptr()))
    }

    actual fun barq_sync_config_new(user: BarqUserPointer, partition: String): BarqSyncConfigurationPointer {
        return LongPointerWrapper<BarqSyncConfigT>(barqc.barq_sync_config_new(user.cptr(), partition)).also { ptr ->
            // Stop the session immediately when the Barq is closed, so the lifecycle of the
            // Sync Client thread is manageable.
            barqc.barq_sync_config_set_session_stop_policy(ptr.cptr(), barq_sync_session_stop_policy_e.BARQ_SYNC_SESSION_STOP_POLICY_IMMEDIATELY)
        }
    }

    actual fun barq_flx_sync_config_new(user: BarqUserPointer): BarqSyncConfigurationPointer {
        return LongPointerWrapper<BarqSyncConfigT>(barqc.barq_flx_sync_config_new(user.cptr()))
    }

    actual fun barq_config_set_sync_config(barqConfiguration: BarqConfigurationPointer, syncConfiguration: BarqSyncConfigurationPointer) {
        barqc.barq_config_set_sync_config(barqConfiguration.cptr(), syncConfiguration.cptr())
    }

    private fun classInfo(barq: BarqPointer, className: String): barq_class_info_t {
        val found = booleanArrayOf(false)
        val classInfo = barq_class_info_t()
        barqc.barq_find_class(barq.cptr(), className, found, classInfo)
        if (!found[0]) {
            throw IllegalArgumentException("Cannot find class: '$className'. Has the class been added to the Barq schema?")
        }
        return classInfo
    }

    private fun propertyInfo(barq: BarqPointer, classKey: ClassKey, col: String): barq_property_info_t {
        val found = booleanArrayOf(false)
        val pinfo = barq_property_info_t()
        barqc.barq_find_property(barq.cptr(), classKey.key, col, found, pinfo)
        if (!found[0]) {
            val className = barq_get_class(barq, classKey).name
            throw IllegalArgumentException("Cannot find property: '$col' in class '$className'")
        }
        return pinfo
    }

    actual fun barq_query_parse(
        barq: BarqPointer,
        classKey: ClassKey,
        query: String,
        args: BarqQueryArgumentList,
    ): BarqQueryPointer {
        return LongPointerWrapper(
            barqc.barq_query_parse(
                barq.cptr(),
                classKey.key,
                query,
                args.size,
                args.head
            )
        )
    }

    actual fun barq_query_parse_for_results(
        results: BarqResultsPointer,
        query: String,
        args: BarqQueryArgumentList,
    ): BarqQueryPointer {
        return LongPointerWrapper(
            barqc.barq_query_parse_for_results(
                results.cptr(),
                query,
                args.size,
                args.head
            )
        )
    }

    actual fun barq_query_parse_for_list(
        list: BarqListPointer,
        query: String,
        args: BarqQueryArgumentList,
    ): BarqQueryPointer {
        return LongPointerWrapper(
            barqc.barq_query_parse_for_list(
                list.cptr(),
                query,
                args.size,
                args.head
            )
        )
    }

    actual fun barq_query_parse_for_set(
        set: BarqSetPointer,
        query: String,
        args: BarqQueryArgumentList,
    ): BarqQueryPointer {
        return LongPointerWrapper(
            barqc.barq_query_parse_for_set(
                set.cptr(),
                query,
                args.size,
                args.head
            )
        )
    }

    actual fun barq_query_find_first(query: BarqQueryPointer): Link? {
        val value = barq_value_t()
        val found = booleanArrayOf(false)
        barqc.barq_query_find_first(query.cptr(), value, found)
        if (!found[0]) {
            return null
        }
        if (value.type != barq_value_type_e.BARQ_TYPE_LINK) {
            error("Query did not return link but ${value.type}")
        }
        return value.asLink()
    }

    actual fun barq_query_find_all(query: BarqQueryPointer): BarqResultsPointer {
        return LongPointerWrapper(barqc.barq_query_find_all(query.cptr()))
    }

    actual fun barq_query_count(query: BarqQueryPointer): Long {
        val count = LongArray(1)
        barqc.barq_query_count(query.cptr(), count)
        return count[0]
    }

    actual fun barq_query_append_query(
        query: BarqQueryPointer,
        filter: String,
        args: BarqQueryArgumentList,
    ): BarqQueryPointer {
        return LongPointerWrapper(
            barqc.barq_query_append_query(query.cptr(), filter, args.size, args.head)
        )
    }

    actual fun barq_query_get_description(query: BarqQueryPointer): String {
        return barqc.barq_query_get_description(query.cptr())
    }

    actual fun barq_results_get_query(results: BarqResultsPointer): BarqQueryPointer {
        return LongPointerWrapper(barqc.barq_results_get_query(results.cptr()))
    }

    actual fun barq_results_resolve_in(results: BarqResultsPointer, barq: BarqPointer): BarqResultsPointer {
        return LongPointerWrapper(barqc.barq_results_resolve_in(results.cptr(), barq.cptr()))
    }

    actual fun barq_results_count(results: BarqResultsPointer): Long {
        val count = LongArray(1)
        barqc.barq_results_count(results.cptr(), count)
        return count[0]
    }

    actual fun MemAllocator.barq_results_average(
        results: BarqResultsPointer,
        propertyKey: PropertyKey
    ): Pair<Boolean, BarqValue> {
        val struct = allocBarqValueT()
        val found = booleanArrayOf(false)
        barqc.barq_results_average(results.cptr(), propertyKey.key, struct, found)
        return found[0] to BarqValue(struct)
    }

    actual fun MemAllocator.barq_results_sum(
        results: BarqResultsPointer,
        propertyKey: PropertyKey
    ): BarqValue {
        val struct = allocBarqValueT()
        val foundArray = BooleanArray(1)
        barqc.barq_results_sum(results.cptr(), propertyKey.key, struct, foundArray)
        return BarqValue(struct)
    }

    actual fun MemAllocator.barq_results_max(
        results: BarqResultsPointer,
        propertyKey: PropertyKey
    ): BarqValue {
        val struct = allocBarqValueT()
        val foundArray = BooleanArray(1)
        barqc.barq_results_max(results.cptr(), propertyKey.key, struct, foundArray)
        return BarqValue(struct)
    }

    actual fun MemAllocator.barq_results_min(
        results: BarqResultsPointer,
        propertyKey: PropertyKey
    ): BarqValue {
        val struct = allocBarqValueT()
        val foundArray = BooleanArray(1)
        barqc.barq_results_min(results.cptr(), propertyKey.key, struct, foundArray)
        return BarqValue(struct)
    }

    // TODO OPTIMIZE Getting a range
    actual fun MemAllocator.barq_results_get(
        results: BarqResultsPointer,
        index: Long
    ): BarqValue {
        val value = allocBarqValueT()
        barqc.barq_results_get(results.cptr(), index, value)
        return BarqValue(value)
    }

    actual fun barq_results_get_list(results: BarqResultsPointer, index: Long): BarqListPointer =
        LongPointerWrapper(barqc.barq_results_get_list(results.cptr(), index))

    actual fun barq_results_get_dictionary(results: BarqResultsPointer, index: Long): BarqMapPointer =
        LongPointerWrapper(barqc.barq_results_get_dictionary(results.cptr(), index))

    actual fun barq_get_object(barq: BarqPointer, link: Link): BarqObjectPointer {
        return LongPointerWrapper(barqc.barq_get_object(barq.cptr(), link.classKey.key, link.objKey))
    }

    actual fun barq_object_find_with_primary_key(
        barq: BarqPointer,
        classKey: ClassKey,
        transport: BarqValue
    ): BarqObjectPointer? {
        val found = booleanArrayOf(false)
        return nativePointerOrNull(
            barqc.barq_object_find_with_primary_key(
                barq.cptr(),
                classKey.key,
                transport.value,
                found
            )
        )
    }

    actual fun barq_results_delete_all(results: BarqResultsPointer) {
        barqc.barq_results_delete_all(results.cptr())
    }

    actual fun barq_object_delete(obj: BarqObjectPointer) {
        barqc.barq_object_delete(obj.cptr())
    }

    actual fun barq_sync_subscription_id(subscription: BarqSubscriptionPointer): ObjectId {
        val nativeBytes: ShortArray = barqc.barq_sync_subscription_id(subscription.cptr()).bytes
        val byteArray = ByteArray(nativeBytes.size)
        nativeBytes.mapIndexed { index, b -> byteArray[index] = b.toByte() }
        return ObjectId(byteArray)
    }

    actual fun barq_sync_subscription_name(subscription: BarqSubscriptionPointer): String? {
        return barqc.barq_sync_subscription_name(subscription.cptr())
    }

    actual fun barq_sync_subscription_object_class_name(subscription: BarqSubscriptionPointer): String {
        return barqc.barq_sync_subscription_object_class_name(subscription.cptr())
    }

    actual fun barq_sync_subscription_query_string(subscription: BarqSubscriptionPointer): String {
        return barqc.barq_sync_subscription_query_string(subscription.cptr())
    }

    actual fun barq_sync_subscription_created_at(subscription: BarqSubscriptionPointer): Timestamp {
        val ts: barq_timestamp_t = barqc.barq_sync_subscription_created_at(subscription.cptr())
        return TimestampImpl(ts.seconds, ts.nanoseconds)
    }

    actual fun barq_sync_subscription_updated_at(subscription: BarqSubscriptionPointer): Timestamp {
        val ts: barq_timestamp_t = barqc.barq_sync_subscription_updated_at(subscription.cptr())
        return TimestampImpl(ts.seconds, ts.nanoseconds)
    }

    actual fun barq_sync_get_latest_subscriptionset(barq: BarqPointer): BarqSubscriptionSetPointer {
        return LongPointerWrapper(barqc.barq_sync_get_latest_subscription_set(barq.cptr()))
    }

    actual fun barq_sync_on_subscriptionset_state_change_async(
        subscriptionSet: BarqSubscriptionSetPointer,
        destinationState: CoreSubscriptionSetState,
        callback: SubscriptionSetCallback
    ) {
        val jvmWrapper: (Int) -> Any = { value: Int ->
            callback.onChange(CoreSubscriptionSetState.of(value))
        }
        barqc.barq_sync_on_subscription_set_state_change_async(
            subscriptionSet.cptr(),
            destinationState.nativeValue,
            jvmWrapper
        )
    }

    actual fun barq_sync_subscriptionset_version(subscriptionSet: BarqBaseSubscriptionSetPointer): Long {
        return barqc.barq_sync_subscription_set_version(subscriptionSet.cptr())
    }

    actual fun barq_sync_subscriptionset_state(subscriptionSet: BarqBaseSubscriptionSetPointer): CoreSubscriptionSetState {
        return CoreSubscriptionSetState.of(barqc.barq_sync_subscription_set_state(subscriptionSet.cptr()))
    }

    actual fun barq_sync_subscriptionset_error_str(subscriptionSet: BarqBaseSubscriptionSetPointer): String? {
        return barqc.barq_sync_subscription_set_error_str(subscriptionSet.cptr())
    }

    actual fun barq_sync_subscriptionset_size(subscriptionSet: BarqBaseSubscriptionSetPointer): Long {
        return barqc.barq_sync_subscription_set_size(subscriptionSet.cptr())
    }

    actual fun barq_sync_subscription_at(
        subscriptionSet: BarqBaseSubscriptionSetPointer,
        index: Long
    ): BarqSubscriptionPointer {
        return LongPointerWrapper(barqc.barq_sync_subscription_at(subscriptionSet.cptr(), index))
    }

    actual fun barq_sync_find_subscription_by_name(
        subscriptionSet: BarqBaseSubscriptionSetPointer,
        name: String
    ): BarqSubscriptionPointer? {
        val ptr = barqc.barq_sync_find_subscription_by_name(subscriptionSet.cptr(), name)
        return nativePointerOrNull(ptr)
    }

    actual fun barq_sync_find_subscription_by_query(
        subscriptionSet: BarqBaseSubscriptionSetPointer,
        query: BarqQueryPointer
    ): BarqSubscriptionPointer? {
        val ptr = barqc.barq_sync_find_subscription_by_query(subscriptionSet.cptr(), query.cptr())
        return nativePointerOrNull(ptr)
    }

    actual fun barq_sync_subscriptionset_refresh(subscriptionSet: BarqSubscriptionSetPointer): Boolean {
        return barqc.barq_sync_subscription_set_refresh(subscriptionSet.cptr())
    }

    actual fun barq_sync_make_subscriptionset_mutable(
        subscriptionSet: BarqSubscriptionSetPointer
    ): BarqMutableSubscriptionSetPointer {
        return LongPointerWrapper(
            barqc.barq_sync_make_subscription_set_mutable(subscriptionSet.cptr()),
            managed = false
        )
    }

    actual fun barq_sync_subscriptionset_clear(
        mutableSubscriptionSet: BarqMutableSubscriptionSetPointer
    ): Boolean {
        val erased = barqc.barq_sync_subscription_set_size(mutableSubscriptionSet.cptr()) > 0
        barqc.barq_sync_subscription_set_clear(mutableSubscriptionSet.cptr())
        return erased
    }

    actual fun barq_sync_subscriptionset_insert_or_assign(
        mutableSubscriptionSet: BarqMutableSubscriptionSetPointer,
        query: BarqQueryPointer,
        name: String?
    ): Pair<BarqSubscriptionPointer, Boolean> {
        val outIndex = longArrayOf(1)
        val outInserted = BooleanArray(1)
        barqc.barq_sync_subscription_set_insert_or_assign_query(
            mutableSubscriptionSet.cptr(),
            query.cptr(),
            name,
            outIndex,
            outInserted
        )
        @Suppress("UNCHECKED_CAST")
        return Pair(
            barq_sync_subscription_at(
                mutableSubscriptionSet as BarqSubscriptionSetPointer,
                outIndex[0]
            ),
            outInserted[0]
        )
    }

    actual fun barq_sync_subscriptionset_erase_by_name(
        mutableSubscriptionSet: BarqMutableSubscriptionSetPointer,
        name: String
    ): Boolean {
        val erased = BooleanArray(1)
        barqc.barq_sync_subscription_set_erase_by_name(
            mutableSubscriptionSet.cptr(),
            name,
            erased
        )
        return erased[0]
    }

    actual fun barq_sync_subscriptionset_erase_by_query(
        mutableSubscriptionSet: BarqMutableSubscriptionSetPointer,
        query: BarqQueryPointer
    ): Boolean {
        val erased = BooleanArray(1)
        barqc.barq_sync_subscription_set_erase_by_query(
            mutableSubscriptionSet.cptr(),
            query.cptr(),
            erased
        )
        return erased[0]
    }

    actual fun barq_sync_subscriptionset_erase_by_id(
        mutableSubscriptionSet: BarqMutableSubscriptionSetPointer,
        sub: BarqSubscriptionPointer
    ): Boolean {
        val id = barqc.barq_sync_subscription_id(sub.cptr())
        val erased = BooleanArray(1)
        barqc.barq_sync_subscription_set_erase_by_id(
            mutableSubscriptionSet.cptr(),
            id,
            erased
        )
        return erased[0]
    }

    actual fun barq_sync_subscriptionset_commit(
        mutableSubscriptionSet: BarqMutableSubscriptionSetPointer
    ): BarqSubscriptionSetPointer {
        return LongPointerWrapper(barqc.barq_sync_subscription_set_commit(mutableSubscriptionSet.cptr()))
    }

    actual fun barq_sync_set_websocket_transport(
        syncClientConfig: BarqSyncClientConfigurationPointer,
        webSocketTransport: WebSocketTransport
    ) {
        barqc.barq_sync_websocket_new(syncClientConfig.cptr(), webSocketTransport)
    }

    actual fun barq_sync_socket_callback_complete(nativePointer: BarqWebsocketHandlerCallbackPointer, cancelled: Boolean, status: WebsocketCallbackResult, reason: String) {
        barqc.barq_sync_websocket_callback_complete(cancelled, nativePointer.cptr(), status.nativeValue, reason)
    }

    actual fun barq_sync_socket_websocket_connected(nativePointer: BarqWebsocketProviderPointer, protocol: String) {
        barqc.barq_sync_websocket_connected(nativePointer.cptr(), protocol)
    }

    actual fun barq_sync_socket_websocket_error(nativePointer: BarqWebsocketProviderPointer) {
        barqc.barq_sync_websocket_error(nativePointer.cptr())
    }

    actual fun barq_sync_socket_websocket_message(
        nativePointer: BarqWebsocketProviderPointer,
        data: ByteArray
    ): Boolean {
        return barqc.barq_sync_websocket_message(nativePointer.cptr(), data, data.size.toLong())
    }

    actual fun barq_sync_socket_websocket_closed(nativePointer: BarqWebsocketProviderPointer, wasClean: Boolean, errorCode: WebsocketErrorCode, reason: String) {
        barqc.barq_sync_websocket_closed(nativePointer.cptr(), wasClean, errorCode.nativeValue, reason)
    }

    fun <T : CapiT> NativePointer<T>.cptr(): Long {
        return (this as LongPointerWrapper).ptr
    }

    private fun <T : CapiT> nativePointerOrNull(ptr: Long, managed: Boolean = true): NativePointer<T>? {
        return if (ptr != 0L) {
            LongPointerWrapper<T>(ptr, managed)
        } else {
            null
        }
    }

}

fun barq_value_t.asTimestamp(): Timestamp {
    if (this.type != barq_value_type_e.BARQ_TYPE_TIMESTAMP) {
        error("Value is not of type Timestamp: $this.type")
    }
    return TimestampImpl(this.timestamp.seconds, this.timestamp.nanoseconds)
}

fun barq_value_t.asLink(): Link {
    if (this.type != barq_value_type_e.BARQ_TYPE_LINK) {
        error("Value is not of type link: $this.type")
    }
    return Link(ClassKey(this.link.target_table), this.link.target)
}

fun ObjectId.asBarqObjectIdT(): barq_object_id_t {
    return barq_object_id_t().apply {
        val data = ShortArray(OBJECT_ID_BYTES_SIZE)
        val objectIdBytes = this@asBarqObjectIdT.toByteArray()
        (0 until OBJECT_ID_BYTES_SIZE).map {
            data[it] = objectIdBytes[it].toShort()
        }
        bytes = data
    }
}

private class JVMScheduler(dispatcher: CoroutineDispatcher) {
    val scope: CoroutineScope = CoroutineScope(dispatcher)
    val lock = SynchronizableObject()
    var cancelled = false

    fun notifyCore(schedulerPointer: Long) {
        scope.launch {
            lock.withLock {
                if (!cancelled) {
                    barqc.invoke_core_notify_callback(schedulerPointer)
                }
            }
        }
    }

    fun cancel() {
        lock.withLock {
            cancelled = true
        }
    }
}
