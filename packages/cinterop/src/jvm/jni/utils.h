...
public:
    ...
    operator barq::StringData() const
    {
        // To solve the link issue by directly using Table::max_string_size
        static constexpr size_t max_string_size = barq::Table::max_string_size;

        if (m_is_null) {
            return barq::StringData();
        }
        else if (m_size > max_string_size) {
            throw RuntimeError(ErrorCodes::RuntimeError, barq::util::format(
                            "The length of 'String' value in UTF8 encoding is %1 which exceeds the max string length %2.",
                            m_size, max_string_size));
        }
        else {
            return barq::StringData(m_data.get(), m_size);
        }
    }
    ...
    operator barq_string_t() const noexcept
    {
        return barq_string_t {m_data.get(), m_size };
    }
