...
#ifndef BARQ_UTIL_UTF8_HPP
#define BARQ_UTIL_UTF8_HPP
                                                  ...
                                                  Char16* out_end)
{
    ...
    while (in != in_end) {
        ...
        if (BARQ_LIKELY(v1 < 0xF0)) { // Three bytes
            if (BARQ_UNLIKELY(in_end - in < 3)) {
                invalid = 4;
                break; // Incomplete UTF-8 sequence
            }
            uint_fast16_t v2 = uint_fast16_t(traits8::to_int_type(in[1]));
            ...
            if (BARQ_UNLIKELY((v2 & 0xC0) != 0x80 || (v3 & 0xC0) != 0x80)) {
                invalid = true;
                break; // Invalid continuation byte
            }
            uint_fast16_t v = uint_fast16_t(((v1 & 0x0F) << 12) | ((v2 & 0x3F) << 6) | ((v3 & 0x3F) << 0));
            if (BARQ_UNLIKELY(v < 0x800)) {
                invalid = 5;
                break; // Overlong encoding is invalid
            }
            if (BARQ_UNLIKELY(0xD800 <= v && v < 0xE000)) {
                invalid = 6;
        ...
        if (BARQ_UNLIKELY(out + 1 == out_end)) {
            break; // Need space in output buffer for surrogate pair
        }
        if (BARQ_LIKELY(v1 < 0xF8)) { // Four bytes
            if (BARQ_UNLIKELY(in_end - in < 4)) {
                invalid = 7;
                break; // Incomplete UTF-8 sequence
            }
            ...
            if (BARQ_UNLIKELY((v2 & 0xC0) != 0x80 || (v3 & 0xC0) != 0x80 || (v4 & 0xC0) != 0x80)) {
                invalid = 8;
                break; // Invalid continuation byte
            }
            ...
            if (BARQ_UNLIKELY(v < 0x10000)) {
                invalid = 9;
                break; // Overlong encoding is invalid
            }
            if (BARQ_UNLIKELY(0x110000 <= v)) {
                invalid = 19;
                                                 ...
                                                 char* out_end, size_t& error_code)
{
    ...
    while (in != in_end) {
        ...
        if (BARQ_LIKELY(v1 < 0x800)) {
            if (BARQ_UNLIKELY(out_end - out < 2)) {
                error_code = 2;
                break; // Not enough output buffer space
            }
            // UTF-8 layout: 110xxxxx 10xxxxxx
        ...
        if (BARQ_LIKELY(v1 < 0xD800 || 0xE000 <= v1)) {
            if (BARQ_UNLIKELY(out_end - out < 3)) {
                error_code = 3;
                break; // Not enough output buffer space
            }
            // UTF-8 layout: 1110xxxx 10xxxxxx 10xxxxxx
                                                                 ...
                                                                 size_t& error_code)
{
    ...
    while (in != in_end) {
        ...
        if (BARQ_LIKELY(v < 0x80)) {
            if (BARQ_UNLIKELY(int_add_with_overflow_detect(num_out, 1))) {
                error_code = 1;
                break; // Avoid overflow
            }
            in += 1;
        }
        else if (BARQ_LIKELY(v < 0x800)) {
            if (BARQ_UNLIKELY(int_add_with_overflow_detect(num_out, 2))) {
                error_code = 2;
                break; // Avoid overflow
            }
            in += 1;
        }
        else if (BARQ_LIKELY(v < 0xD800 || 0xE000 <= v)) {
            if (BARQ_UNLIKELY(int_add_with_overflow_detect(num_out, 3))) {
                error_code = 3;
                break; // Avoid overflow
            }
            in += 1;
            ...
            if (BARQ_UNLIKELY(int_add_with_overflow_detect(num_out, 4))) {
                error_code = 5;
                break; // Avoid overflow
            }
            in += 2;
