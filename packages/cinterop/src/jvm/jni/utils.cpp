...
static std::string string_to_hex(const std::string& message, barq::StringData& str, const char* in_begin, const char* in_end,
...
jstring to_jstring(JNIEnv* env, barq::StringData str)
{
    if (str.is_null()) {
        return NULL;
    }
...
struct JStringCharsAccessor {
    ...
    ~JStringCharsAccessor()
    {
        ...
        //  https://github.com/barq/barq-java/pull/7232. We should consider making this the
        ...
        : m_env(env)
{
    ...
    BARQ_ASSERT(max_project_size <= std::numeric_limits<size_t>::max() / 4);
    size_t buf_size;
