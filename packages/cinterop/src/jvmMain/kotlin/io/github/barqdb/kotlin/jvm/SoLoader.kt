...
package io.github.barqdb.kotlin.jvm

import io.github.barqdb.kotlin.internal.SDK_VERSION
import java.io.File
...
class SoLoader {
    ...
    private val libraryName = "barqc"

    @Suppress("unused") // Called using reflection. See /packages/jni-swig-stub/barq.i
    fun load() {
        ...
        //
        // See https://github.com/BarqDB/barq-kotlin/issues/1105 for more information.
        try {
    ...
    private fun load(libraryName: String) {
        ...
        // path should be <default user lib dir>/io.github.barqdb.kotlin/libraryVersion]/libbarqffi.so
...
private enum class Platform(
    ...
    MACOS(
        ...
        prefix = "lib",
        suffix = "dylib",
        defaultSystemLocation = "${System.getProperty("user.home")}/Library/Caches/io.github.barqdb.kotlin/"
    ),
    LINUX(
        shortName = "/jni/linux",
        prefix = "lib",
        suffix = "so",
        defaultSystemLocation = "${System.getProperty("user.home")}/.cache/io.github.barqdb.kotlin/"
    ),
    WINDOWS(
