...
// TODO Could be replace by lambda. See barq_app_config_new networkTransportFactory for example.
interface Callback<T : BarqNativePointer> {
...
fun interface SyncErrorCallback {
    fun onSyncError(pointer: BarqSyncSessionPointer, error: SyncError)
}
...
interface SyncBeforeClientResetHandler {
    fun onBeforeReset(barqBefore: FrozenBarqPointer)
}
...
fun interface MigrationCallback {
    fun migrate(
        oldBarq: FrozenBarqPointer,
        newBarq: LiveBarqPointer,
        schema: BarqSchemaPointer
    )
}
...
// The underlying Core implementation can also pass in Barq pointer, but since it is not
