...
import io.github.barqdb.kotlin.internal.interop.BarqWebsocketHandlerCallbackPointer
import io.github.barqdb.kotlin.internal.interop.BarqWebsocketProviderPointer
...
interface WebSocketTransport {
     ...
     */
    fun post(handlerCallback: BarqWebsocketHandlerCallbackPointer)
    ...
    fun createTimer(
        ...
        handlerCallback: BarqWebsocketHandlerCallbackPointer,
    ): CancellableTimer
    ...
    fun runCallback(
        ...
        status: WebsocketCallbackResult = WebsocketCallbackResult.BARQ_ERR_SYNC_SOCKET_SUCCESS,
        reason: String = ""
    ) {
        BarqInterop.barq_sync_socket_callback_complete(
...
interface WebSocketClient {
     ...
     */
    fun send(message: ByteArray, handlerCallback: BarqWebsocketHandlerCallbackPointer)
 ...
 */
class WebSocketObserver(private val webSocketObserverPointer: BarqWebsocketProviderPointer) {
    /**
     * Communicate the negotiated Sync protocol.
     */
    fun onConnected(protocol: String) {
        BarqInterop.barq_sync_socket_websocket_connected(webSocketObserverPointer, protocol)
    }
     ...
     */
    fun onError() {
        BarqInterop.barq_sync_socket_websocket_error(webSocketObserverPointer)
    }
    ...
    fun onNewMessage(data: ByteArray): Boolean {
        return BarqInterop.barq_sync_socket_websocket_message(webSocketObserverPointer, data)
    }
    ...
    fun onClose(wasClean: Boolean, errorCode: WebsocketErrorCode, reason: String) {
        BarqInterop.barq_sync_socket_websocket_closed(
