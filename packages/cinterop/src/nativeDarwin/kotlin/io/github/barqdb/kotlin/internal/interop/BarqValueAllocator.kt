...
class NativeMemAllocator : MemTrackingAllocator {
    ...
    override inline fun allocBarqValueT(): BarqValueT = scope.alloc()

    @Suppress("NOTHING_TO_INLINE")
    override inline fun allocBarqValueList(count: Int): BarqValueList =
        BarqValueList(count, scope.allocArray(count))

    override fun nullTransport(): BarqValue =
        createTransport(null, barq_value_type.BARQ_TYPE_NULL)

    override fun longTransport(value: Long?): BarqValue =
        createTransport(value, barq_value_type.BARQ_TYPE_INT) { integer = it }

    override fun booleanTransport(value: Boolean?): BarqValue =
        createTransport(value, barq_value_type.BARQ_TYPE_BOOL) { boolean = it }

    override fun timestampTransport(value: Timestamp?): BarqValue =
        createTransport(value, barq_value_type.BARQ_TYPE_TIMESTAMP) {
            timestamp.apply {
                seconds = it.seconds
    ...
    override fun floatTransport(value: Float?): BarqValue =
        createTransport(value, barq_value_type.BARQ_TYPE_FLOAT) { fnum = it }

    override fun doubleTransport(value: Double?): BarqValue =
        createTransport(value, barq_value_type.BARQ_TYPE_DOUBLE) { dnum = it }

    override fun decimal128Transport(value: Decimal128?): BarqValue =
        createTransport(value, barq_value_type.BARQ_TYPE_DECIMAL128) {
            decimal128.apply {
                w[0] = it.low
                w[1] = it.high
            }
    ...
    override fun objectIdTransport(value: ByteArray?): BarqValue =
        createTransport(value, barq_value_type.BARQ_TYPE_OBJECT_ID) {
            object_id.apply {
    ...
    override fun uuidTransport(value: ByteArray?): BarqValue =
        createTransport(value, barq_value_type.BARQ_TYPE_UUID) {
            uuid.apply {
    ...
    override fun barqObjectTransport(value: BarqObjectInterop?): BarqValue =
        createTransport(value, barq_value_type.BARQ_TYPE_LINK) {
            barq_wrapper.barq_object_as_link(it.objectPointer.cptr())
                .useContents {
                    link.apply {
    ...
    override fun stringTransport(value: String?): BarqValue =
        createTransport(value, barq_value_type.BARQ_TYPE_STRING) { string.set(scope, it) }

    override fun byteArrayTransport(value: ByteArray?): BarqValue =
        createTransport(value, barq_value_type.BARQ_TYPE_BINARY) { binary.set(scope, it) }

    override fun queryArgsOf(queryArgs: List<BarqQueryArgument>): BarqQueryArgumentList {
        val cArgs = scope.allocArray<barq_query_arg_t>(queryArgs.size)
        ...
        return BarqQueryArgumentList(queryArgs.size.toULong(), cArgs.pointed)
    }
 ...
 */
@Suppress("NOTHING_TO_INLINE")
actual inline fun barqValueAllocator(): MemAllocator = NativeMemAllocator()
 ...
 */
@Suppress("NOTHING_TO_INLINE")
actual inline fun trackingBarqValueAllocator(): MemTrackingAllocator = NativeMemAllocator()
