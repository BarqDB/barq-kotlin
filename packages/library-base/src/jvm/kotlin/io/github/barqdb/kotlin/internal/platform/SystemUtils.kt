...
import io.github.barqdb.kotlin.internal.Constants.FILE_COPY_BUFFER_SIZE
...
public actual fun copyAssetFile(
    ...
    assetFileAsStream(assetFilename).let { inputStream ->
        ...
        val outputFile = File(barqFilePath)
        try {
...
private fun preparePath(directoryPath: String) {
    ...
    if (!dir.exists()) {
        if (!dir.mkdirs()) {
            throw IllegalStateException("Directories for Barq file could not be created: $directoryPath")
        }
    }
    if (dir.isFile) {
