...
import io.github.barqdb.kotlin.internal.platform.currentTime

/**
 * A representation of a Barq timestamp. A timestamp represent a single point in time defined as
 ...
 *
 * Examples:
 * - The UNIX epoch is constructed by `BarqInstant.from(0, 0)`.
 * - Relative times are constructed as follows:
 *      - +1 second is constructed by BarqInstant.from(1, 0)
 *      - +1 nanosecond is constructed by BarqInstant.from(0, 1)
 *      - +1.1 seconds (1100 milliseconds after the epoch) is constructed by BarqInstant.from(1, 100000000)
 *      - -1.1 seconds (1100 milliseconds before the epoch) is constructed by BarqInstant.from(-1, -100000000)
 */
public interface BarqInstant : Comparable<BarqInstant> {

    public companion object {
        internal const val SEC_AS_NANOSECOND: Int = 1_000_000_000

        /**
         * Minimum timestamp that can be stored in Barq.
         */
        public val MIN: BarqInstant = from(Long.MIN_VALUE, -999_999_999)

        /**
         * Maximum timestamp that can be stored in Barq.
         */
        public val MAX: BarqInstant = from(Long.MAX_VALUE, 999_999_999)

        /**
         * Creates a [BarqInstant] that is the [epochSeconds] number of seconds from the UNIX epoch
         ...
         */
        public fun from(epochSeconds: Long, nanosecondAdjustment: Int): BarqInstant {
            ...
            return BarqInstantImpl(s, ns)
        }

        /**
         * Creates a [BarqInstant] describing the number of seconds and nanoseconds from the UNIX epoch
         * `1970-01-01T00:00:00Z` until now.
         */
        public fun now(): BarqInstant {
            return currentTime()
        }
    }
