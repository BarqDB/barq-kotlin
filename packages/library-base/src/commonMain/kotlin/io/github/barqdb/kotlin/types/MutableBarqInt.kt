...
import io.github.barqdb.kotlin.internal.UnmanagedMutableBarqInt

/**
 * A `MutableBarqInt` is a mutable, [Long]-like, numeric quantity.
 *
 * `MutableBarqInt`s are most interesting as members of a managed [BarqObject] object in a
 * synchronized Barq. When managed, the [increment] and [decrement] operators implement a
 ...
 * to [increment] or [decrement]. Although the value of a `MutableBarqInt` will always converge
 ...
 *
 * `MutableBarqInt`s **cannot be primary keys**.
 *
 * `MutableBarqInt`s cannot store `null` values. However, it is possible to declare nullable
 * `MutableBarqInt` class members:
 * ```
 * var counter: MutableBarqInteger? = null
 * ```
 *
 * A reference to a managed `MutableBarqInt` is subject to all of the constraints that apply to the
 * model object from which it was obtained: It can only be mutated within a transaction and it
 * becomes invalid if the Barq backing it is closed. Note that a reference to a managed
 * `MutableBarqInt` retains a reference to the model object to which it belongs. For example in
 * this code:
 * ```
 * val counter: MutableBarqInt = barq.query<User>("user = $0", "Fred")
 *      .first()
 *      .find()!!
 *      .counter
 * ```
 ...
 *
 * It is worth noting that sharing `MutableBarqInt`s across `BarqObject`s results in different
 ...
 * as is the case for all other Barq primitive types. This means that the last two lines in the
 ...
 * ```
 *
 * In addition to the API functions, `MutableBarqInt` is a subclass of [Number]. This allows users
 ...
 * modify `counter` but rather create a new, **unmanaged** `MutableBarqInt` with the updated value.
 * **The only operations that result in a mutated value are [set], [increment] and [decrement]**.
 */
public abstract class MutableBarqInt : Comparable<MutableBarqInt>, Number() {

    /**
     * Gets the `MutableBarqInt` value.
     *
     * @return the value.
     */
    ...
    /**
     * Sets the `MutableBarqInt` value.
     *
     * Calling [set] forcibly sets the `MutableBarqInt` to the provided value. Doing this removes
    ...
    /**
     * Increments the `MutableBarqInt`, adding the value of the argument. Increment/decrement
     ...
     *
     * @param value quantity to be added to the `MutableBarqInt`.
     */
    public abstract fun increment(value: Number)

    /**
     * Decrements the `MutableBarqInt`, subtracting the value of the argument. Increment/decrement
     * operations from all devices are reflected in the new value, which is guaranteed to converge.
     * This operation should not be confused with the [dec] operator, which returns a new value
     * whereas this one modifies the value itself.
     *
     * @param value quantity to be subtracted from the `MutableBarqInt`.
     */
    public abstract fun decrement(value: Number)

    /**
     * Two `MutableBarqInt`s are equal if and only if their [Long] values are equal.
     *
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (other !is MutableBarqInt) return false
    ...
    /**
     * A `MutableBarqInt`'s hash code is, exactly, the hash code of its value.
     *
    ...
    override fun toString(): String = "BarqMutableInt{${get()}}"

    /**
     * `MutableBarqInt`s compare strictly by their [Long] values.
     *
     * @param other the compare target
     * @return -1, 0, or 1, depending on whether this object's value is <, =, or > the target's.
     */
    override fun compareTo(other: MutableBarqInt): Int = get().compareTo(other.get())

    /**
     * Adds the other value to this value.
     * The other value will be converted to [Long] before applying the operator.
     */
    public operator fun plus(other: Number): MutableBarqInt = create(get() + other.toLong())

    /**
     * Subtracts the other value from this value.
     * The other value will be converted to [Long] before applying the operator.
     */
    public operator fun minus(other: Number): MutableBarqInt = create(get() - other.toLong())

    /**
     * Multiplies this value by the other value.
     * The other value will be converted to [Long] before applying the operator.
     */
    public operator fun times(other: Number): MutableBarqInt = create(get() * other.toLong())
     ...
     * The other value will be converted to [Long] before applying the operator.
     */
    public operator fun div(other: Number): MutableBarqInt = create(get() / other.toLong())
     ...
     * The other value will be converted to [Long] before applying the operator.
     */
    public operator fun rem(other: Number): MutableBarqInt = create(get() % other.toLong())

    /**
     * Returns this value incremented by one.
     */
    public operator fun inc(): MutableBarqInt = create(get() + 1)

    /**
     * Returns this value decremented by one.
     */
    public operator fun dec(): MutableBarqInt = create(get() - 1)

    /**
     * Returns this value.
     */
    public operator fun unaryPlus(): MutableBarqInt = create(get().unaryPlus())

    /**
     * Returns the negative of this value.
     */
    public operator fun unaryMinus(): MutableBarqInt = create(get().unaryMinus())
     ...
     */
    public infix fun shl(bitCount: Int): MutableBarqInt = create(get().shl(bitCount))
     ...
     * The shift distance actually used is therefore always in the range `0..63`.
     */
    public infix fun shr(bitCount: Int): MutableBarqInt = create(get().shr(bitCount))
     ...
     * The shift distance actually used is therefore always in the range `0..63`.
     */
    public infix fun ushr(bitCount: Int): MutableBarqInt = create(get().ushr(bitCount))

    /**
     * Performs a bitwise AND operation between the two values.
     */
    public infix fun and(other: Number): MutableBarqInt = create(get().and(other.toLong()))

    /**
     * Performs a bitwise OR operation between the two values.
     */
    public infix fun or(other: Number): MutableBarqInt = create(get().or(other.toLong()))

    /**
     * Performs a bitwise XOR operation between the two values.
     */
    public infix fun xor(other: Number): MutableBarqInt = create(get().xor(other.toLong()))

    /**
     * Inverts the bits in this value.
     */
    public fun inv(): MutableBarqInt = create(get().inv())

    public companion object {
        /**
         * Creates a new, unmanaged [MutableBarqInt] with the specified initial value.
         *
         * @param value initial value.
         */
        public fun create(value: Long): MutableBarqInt = UnmanagedMutableBarqInt(value)
    }
}
