...
import io.github.barqdb.kotlin.internal.fromCoreLogLevel
import io.github.barqdb.kotlin.internal.interop.CoreLogLevel
import io.github.barqdb.kotlin.internal.interop.LogCallback
...
import io.github.barqdb.kotlin.internal.platform.createDefaultSystemLogger
import io.github.barqdb.kotlin.internal.toCoreLogLevel
import io.github.barqdb.kotlin.log.BarqLog.add
import io.github.barqdb.kotlin.log.BarqLog.addDefaultSystemLogger

/**
 * Global logger class used by all Barq components.
 *
 ...
 *
 * Custom loggers can be added by registering a class implementing [BarqLogger] using [add].
 *
...
public object BarqLog {
    ...
    private var systemLoggerInstalled: BarqLogger? = null
    // Kotlin Multiplatform is currently lacking primitives like CopyOnWriteArrayList. We could
    // use `io.github.barqdb.kotlin.internal.interop.SynchronizableObject`, but it would require locking
    ...
    private var loggers: MutableList<BarqLogger> = mutableListOf()
     ...
     * @param category target log category, [LogCategory.Barq] by default.
     */
    public fun setLevel(level: LogLevel, category: LogCategory = LogCategory.Barq) {
        BarqInterop.barq_set_log_level_category(category.toString(), level.toCoreLogLevel())
     ...
     */
    public fun getLevel(category: LogCategory = LogCategory.Barq): LogLevel {
        return BarqInterop.barq_get_log_level_category(category.toString()).fromCoreLogLevel()
    }
    ...
    init {
        ...
        BarqInterop.barq_set_log_callback(
            object : LogCallback {
     ...
     */
    public fun add(logger: BarqLogger) {
     ...
     */
    public fun remove(logger: BarqLogger): Boolean {
        loggersMutex.withLock {
            val updatedLoggers = MutableList(loggers.size) { loggers[it] }
    ...
    public fun addDefaultSystemLogger(): Boolean {
        loggersMutex.withLock {
            if (systemLoggerInstalled == null) {
                val systemLogger = createDefaultSystemLogger(Barq.DEFAULT_LOG_TAG)
                val updatedLoggers = MutableList(loggers.size) { loggers[it] }
