...
/**
 * An **internal Barq configuration** that holds internal properties from a
 * [io.github.barqdb.Configuration]. This is needed to make "agnostic" configurations from a base-sync
 * point of view.
 */
...
public interface InternalConfiguration : Configuration {
    public val mapOfKClassWithCompanion: Map<KClass<out BaseBarqObject>, BarqObjectCompanion>
    ...
    // Temporary work-around for https://github.com/BarqDB/barq-kotlin/issues/724
     ...
     * Each pointer should only be used to open _one_ barq. If you want to open multiple barqs
     * with the same [Configuration], this method should be called for each one of them.
     */
    public fun createNativeConfiguration(): BarqConfigurationPointer

    /**
     * This function is a way `BarqImpl` can defer how the Barq is opened to either a local
     * or sync code path. Synced and Local Barqs will differ depending on
     ...
     * by both `BarqConfigurationImpl` and `SyncConfigurationImpl`.
     *
     * @param barq instance of the Barq that is being created.
     * @returns a pair of (LiveBarqPointer, FileCreated)
     */
    public suspend fun openBarq(barq: BarqImpl): Pair<FrozenBarqReference, Boolean>

    /**
     * This function is a way `BarqImpl` can defer how the Barq is initialized once opened.
     * Synced and Local Barqs will differ depending on whether `Configuration.initialData` or
     * `SyncConfiguration.initialSubscriptions` are set.
     *
     * @param barq instance of the Barq that is being created.
     * @param barqFileCreated `true` if the Barq file was just created, `false` if it already existed.
     */
    public suspend fun initializeBarqData(barq: BarqImpl, barqFileCreated: Boolean)
