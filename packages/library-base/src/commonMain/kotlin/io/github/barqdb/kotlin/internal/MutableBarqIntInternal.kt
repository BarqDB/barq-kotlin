...
import io.github.barqdb.kotlin.internal.BarqObjectHelper.NOT_IN_A_TRANSACTION_MSG
...
internal class ManagedMutableBarqInt(
    private val obj: BarqObjectReference<out BaseBarqObject>,
    private val propertyKey: PropertyKey,
    private val converter: BarqValueConverter<Long>
) : MutableBarqInt() {
    ...
    override fun set(value: Number) = operationInternal("Cannot set", value) {
        inputScope {
            with(converter) {
                val convertedValue = publicToBarqValue(value.toLong())
                BarqInterop.barq_set_value(
                    obj.objectPointer,
                    propertyKey,
                    convertedValue,
                    false
    ...
    override fun increment(value: Number) = operationInternal("Cannot increment/decrement", value) {
        BarqInterop.barq_object_add_int(
            obj.objectPointer,
            propertyKey,
            value.toLong()
        )
...
internal class UnmanagedMutableBarqInt(
