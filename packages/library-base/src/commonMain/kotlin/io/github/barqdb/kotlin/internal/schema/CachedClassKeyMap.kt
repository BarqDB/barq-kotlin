...
import io.github.barqdb.kotlin.internal.interop.SCHEMA_NO_VALUE
...
public interface ClassMetadata {
    public val clazz: KClass<out TypedBarqObject>?
    ...
    public val isEmbeddedBarqObject: Boolean
...
public interface PropertyMetadata {
    ...
    public val accessor: KProperty1<BaseBarqObject, Any?>?
...
public class CachedSchemaMetadata(
    private val dbPointer: BarqPointer,
    companions: Collection<BarqObjectCompanion>
) : SchemaMetadata {
    ...
    init {
        classMapByName = BarqInterop.barq_get_class_keys(dbPointer).map<ClassKey, Pair<String, CachedClassMetadata>> {
            val classInfo = BarqInterop.barq_get_class(dbPointer, it)
            // FIXME OPTIMIZE
            val className = classInfo.name
            val companion: BarqObjectCompanion? = companions.singleOrNull { it.io_github_barqdb_kotlin_className == className }
...
public class CachedClassMetadata(
    dbPointer: BarqPointer,
    override val className: String,
    override val classKey: ClassKey,
    companion: BarqObjectCompanion?
) : ClassMetadata {
    // TODO OPTIMIZE We should theoretically be able to lazy load these, but it requires locking
    //  and 'by lazy' initializers can throw
    //  kotlin.native.concurrent.InvalidMutabilityException: Frozen during lazy computation
    ...
    override val clazz: KClass<out TypedBarqObject>? = companion?.io_github_barqdb_kotlin_class

    init {
        val classInfo = BarqInterop.barq_get_class(dbPointer, classKey)
        BarqInterop.barq_get_class_properties(
            dbPointer,
            classInfo.key,
            ...
            properties = interopProperties.map { propertyInfo: PropertyInfo ->
                CachedPropertyMetadata(
                    propertyInfo,
                    companion?.io_github_barqdb_kotlin_fields?.get(propertyInfo.name)?.second
                )
            }
        }
        ...
        isEmbeddedBarqObject = classInfo.isEmbedded
...
public class CachedPropertyMetadata(
    ...
    override val accessor: KProperty1<BaseBarqObject, Any?>? = null
) : PropertyMetadata {
