...
/**
 * This sealed class describe the possible events that can be observed on a [BarqSingleQuery] flow.
 *
 * It extends the sealed interface [ObjectChange] by adding the new event [PendingObject] on top of
 * its hierarchy. See [BarqSingleQuery.asFlow] for more information on how these events are emitted.
 *
 ...
 * ```
 */
public sealed interface SingleQueryChange<O : BaseBarqObject> {
    /**
 ...
 */
public interface PendingObject<O : BaseBarqObject> : SingleQueryChange<O>

/**
 * This sealed interface describe the possible changes that can be observed to a Barq Object.
 *
 ...
 * barq.filter<Person>().first().asFlow()
 ...
 */
public sealed interface ObjectChange<O : BaseBarqObject> : SingleQueryChange<O> {
    /**
...
/**
 * Initial event to be observed on a [BarqObject] or [EmbeddedBarqObject] flow. It contains a
 ...
 */
public interface InitialObject<O : BaseBarqObject> : ObjectChange<O> {
    override val obj: O
}

/**
 * [BarqObject] or [EmbeddedBarqObject] flow event that describes that an update has been performed on
 * to the observed object. It provides a reference to the object and a list of the changed field
 * names.
 */
public interface UpdatedObject<O : BaseBarqObject> : ObjectChange<O> {
    override val obj: O
...
/**
 * This interface describes the event is emitted deleted on a [BarqObject] or [EmbeddedBarqObject]
 * flow. The flow will terminate after emitting this event.
 */
public interface DeletedObject<O : BaseBarqObject> : ObjectChange<O>
