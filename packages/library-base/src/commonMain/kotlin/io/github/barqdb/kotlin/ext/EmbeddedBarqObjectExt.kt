...
import io.github.barqdb.kotlin.internal.interop.BarqObjectT
import io.github.barqdb.kotlin.internal.barqObjectReference
 ...
 * be a [BarqObject] or another [EmbeddedBarqObject].
 *
 * If known, the type parameter can be used to cast it to the parent type. The other approach is to cast
 * it to the generic [TypedBarqObject] and then switch over its possible types:
 *
 * ```
 * val parent: TypedBarqObject = child.parent()
 * when(parent) {
 *  is Parent1 -> TODO()
 *  is Parent2 -> TODO()
 ...
 */
public fun <T : TypedBarqObject> EmbeddedBarqObject.parent(parentClass: KClass<T>): T {
    if (!this.isManaged()) {
        throw IllegalStateException("Unmanaged embedded objects don't support parent access.")
    }

    return with(this.barqObjectReference!!) {
        BarqInterop.barq_object_get_parent(
            objectPointer
        ) { classKey: ClassKey, objectPointer: NativePointer<BarqObjectT> ->
            val sourceClassMetadata = owner.schemaMetadata[classKey] ?: throw IllegalArgumentException("Parent class not defined in the Barq schema: ${parentClass. qualifiedName}")
            ...
            BarqObjectReference(
                ...
                .also { barqObject: T ->
                    if (!parentClass.isInstance(barqObject)) {
...
/**
 * Returns a [TypedBarqObject] that represents the parent that hosts the embedded object.
 *
 * Reified convenience wrapper for [EmbeddedBarqObject.parent].
 */
public inline fun <reified T : TypedBarqObject> EmbeddedBarqObject.parent(): T = parent(T::class)

/**
 * Defines a backlink that represents a one-to-one inverse relationship between an [EmbeddedBarqObject] and a
 * [TypedBarqObject].
 *
 * You cannot directly modify the backlink itself, it must be done on the pointing object. Usage example:
 *
 * ```
 * class Town : EmbeddedBarqObject {
 *  val county: County by backlinks(County::towns)
 * }
 *
 * class County : BarqObject {
 *  val towns: BarqList<Town> = barqListOf()
 * }
 * ```
 *
 * Because an [EmbeddedBarqObject] class might be referenced by different [TypedBarqObject] than
...
public fun <T : TypedBarqObject> EmbeddedBarqObject.backlinks(
 ...
 * [EmbeddedBarqObject] and a [TypedBarqObject].
 *
 * Reified convenience wrapper for [EmbeddedBarqObject.backlinks].
 */
public inline fun <reified T : TypedBarqObject> EmbeddedBarqObject.backlinks(
    sourceProperty: KProperty1<T, *>
