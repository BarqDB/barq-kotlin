...
public class ContextLogger(public val context: String? = null) {
    ...
    private inline fun checkPriority(
        ...
        if (level.priority >= BarqLog.sdkLogLevel.priority) {
            block()
        }
    }
    ...
    private inline fun doLog(level: LogLevel, throwable: Throwable?) {
        checkPriority(level) {
            BarqLog.doLog(SdkLogCategory, level, throwable, null)
        }
    }
    ...
    private inline fun doLog(
        ...
        checkPriority(level) {
            BarqLog.doLog(SdkLogCategory, level, throwable, message(), *args)
        }
    }
}
