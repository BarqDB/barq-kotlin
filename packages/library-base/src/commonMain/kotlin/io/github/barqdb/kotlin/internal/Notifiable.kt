...
internal interface Notifiable<T : CoreNotifiable<T, C>, C> {
    ...
    /**
     * Should return the live reference in [liveBarq] that the [SuspendableNotifier] will register
     * notifications for with Core, or `null` if the entity has been deleted.
     */
    public fun coreObservable(liveBarq: LiveBarq): CoreNotifiable<T, C>?
 ...
 * @param C the type of change events emitted for the T entity.
 */
// TODO Public due to being a transitive dependency from BarqObjectReference
public abstract class ChangeFlow<T, C>(private val producerScope: ProducerScope<C>) {
     ...
     * longer present in the [SuspendableNotifier]'s live barq.
     ...
     */
    internal fun emit(frozenRef: T?, change: BarqChangesPointer? = null) {
    ...
    internal abstract fun update(frozenRef: T, change: BarqChangesPointer): C?
...
internal interface CoreNotifiable<T, C> : Notifiable<T, C>, Observable<T, C>, Versioned, KeyPathFlowable<C>
    ...
    public fun thaw(liveBarq: BarqReference): T?
    public fun registerForNotification(keyPaths: BarqKeyPathArrayPointer?, callback: Callback<BarqChangesPointer>): BarqNotificationTokenPointer
    public fun freeze(frozenBarq: BarqReference): T?

    // Default implementation as all Observables are just thawing themselves.
    override fun notifiable(): Notifiable<T, C> = this
    override fun coreObservable(liveBarq: LiveBarq): CoreNotifiable<T, C>? = thaw(liveBarq.barqReference)
