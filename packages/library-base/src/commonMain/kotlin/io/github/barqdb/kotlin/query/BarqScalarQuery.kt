 ...
 * results provided by some query operations, e.g. [BarqQuery.count] or [BarqQuery.sum].
 */
public interface BarqScalarQuery<T> {
    /**
     ...
     *
     * @throws UnsupportedOperationException if called on a query issued on a [MutableBarq].
     */
    public fun asFlow(): Flow<T>
}

/**
 * Similar to [BarqScalarQuery.find] but it receives a [block] in which the scalar result from the
 * query is provided.
 *
 ...
 */
public fun <T, R> BarqScalarQuery<T>.find(block: (T) -> R): R = find().let(block)

/**
 * Similar to [BarqSingleQuery.find] but it receives a [block] in which the [BarqObject] or
 * [EmbeddedBarqObject] from the query is provided.
 *
 * @param T the type of the query
 * @param R the type returned by [block]
 * @return whatever [block] returns
 */
public fun <T : BaseBarqObject, R> BarqSingleQuery<T>.find(block: (T?) -> R): R = find().let(block)
