...
inline jboolean jni_check_exception(JNIEnv *jenv = get_env()) {
    // FIXME https://github.com/BarqDB/barq-kotlin/issues/665 This function is catching and swallowing
...
inline jboolean jni_check_exception_for_callback(JNIEnv *jenv = get_env()) {
    if (jenv->ExceptionCheck()) {
        ...
        barq_register_user_code_callback_error(jenv->NewGlobalRef(exception));
        return false;
    }
    return true;
}
...
inline jobject create_java_exception(JNIEnv *jenv, barq_error_t error) {
    // Invoke CoreErrorConverter.asThrowable() to retrieve an exception instance that
    // maps to the core error.
    static const JavaClass& error_type_class = barq::_impl::JavaClassGlobalDef::core_error_converter();
...
barq_changed_callback(void* userdata) {
    ...
    static JavaMethod java_callback_method(env, java_callback_class, "invoke",
                                           "()Ljava/lang/Object;");
    // TODO Align exceptions handling https://github.com/BarqDB/barq-kotlin/issues/665
...
bool migration_callback(void *userdata, barq_t *old_barq, barq_t *new_barq,
                        const barq_schema_t *schema) {
    auto env = get_env(true);
    static JavaClass java_callback_class(env, "io/github/barqdb/kotlin/internal/interop/MigrationCallback");
    static JavaMethod java_callback_method(env, java_callback_class, "migrate",
                                           "(Lio/github/barqdb/kotlin/internal/interop/NativePointer;Lio/github/barqdb/kotlin/internal/interop/NativePointer;Lio/github/barqdb/kotlin/internal/interop/NativePointer;)V");
    // These barq/schema pointers are only valid for the duration of the
    ...
    env->CallVoidMethod(static_cast<jobject>(userdata), java_callback_method,
                        wrap_pointer(env, reinterpret_cast<jlong>(old_barq), false),
                        wrap_pointer(env, reinterpret_cast<jlong>(new_barq), false),
...
//  changes as barq_collection_changes_t.
barq_notification_token_t *
register_results_notification_cb(barq_results_t *results,
    ...
    return barq_results_add_notification_callback(
            results,
            ...
            reinterpret_cast<barq_key_path_array_t*>(key_path_array_ptr),
            // change callback
            [](void *userdata, const barq_collection_changes_t *changes) {
...
barq_on_object_change_func_t get_on_object_change() {
    auto jenv = get_env(true);
    static JavaMethod on_change_method(jenv, JavaClassGlobalDef::notification_callback(),
                                       "onChange", "(J)V");
    return [](barq_userdata_t userdata, const barq_object_changes_t* changes) {
        // TODO API-NOTIFICATION Consider catching errors and propagate to error callback
        //  like the C-API error callback below
        //  https://github.com/BarqDB/barq-kotlin/issues/889
        auto jenv = get_env(true);
...
barq_on_collection_change_func_t get_on_collection_change() {
    auto jenv = get_env(true);
    static JavaMethod on_change_method(jenv, JavaClassGlobalDef::notification_callback(),
                                       "onChange", "(J)V");
    return [](barq_userdata_t userdata, const barq_collection_changes_t* changes) {
        // TODO API-NOTIFICATION Consider catching errors and propagate to error callback
        //  like the C-API error callback below
        //  https://github.com/BarqDB/barq-kotlin/issues/889
        auto jenv = get_env(true);
...
barq_on_dictionary_change_func_t get_on_dictionary_change() {
    auto jenv = get_env(true);
    static JavaMethod on_change_method(jenv, JavaClassGlobalDef::notification_callback(),
                                       "onChange", "(J)V");
    return [](barq_userdata_t userdata, const barq_dictionary_changes_t* changes) {
        // TODO API-NOTIFICATION Consider catching errors and propagate to error callback
        //  like the C-API error callback below
        //  https://github.com/BarqDB/barq-kotlin/issues/889
        auto jenv = get_env(true);
...
register_notification_cb(
    ...
    switch (collection_type) {
        case BARQ_COLLECTION_TYPE_NONE:
            return barq_object_add_notification_callback(
                    reinterpret_cast<barq_object_t*>(collection_ptr),
                    user_data, // Use the callback as user data
                    user_data_free,
                    (key_path_array_ptr == 0) ? NULL : reinterpret_cast<barq_key_path_array_t*>(key_path_array_ptr),
                    get_on_object_change()
            );
        case BARQ_COLLECTION_TYPE_LIST: return barq_list_add_notification_callback(
                    reinterpret_cast<barq_list_t*>(collection_ptr),
                    user_data, // Use the callback as user data
                    user_data_free,
                    reinterpret_cast<barq_key_path_array_t*>(key_path_array_ptr),
                    get_on_collection_change()
            );
        case BARQ_COLLECTION_TYPE_SET: return barq_set_add_notification_callback(
                    reinterpret_cast<barq_set_t*>(collection_ptr),
                    user_data, // Use the callback as user data
                    user_data_free,
                    reinterpret_cast<barq_key_path_array_t*>(key_path_array_ptr),
                    get_on_collection_change()
            );
        case BARQ_COLLECTION_TYPE_DICTIONARY: return barq_dictionary_add_notification_callback(
                    reinterpret_cast<barq_dictionary_t*>(collection_ptr),
                    user_data, // Use the callback as user data
                    user_data_free,
                    reinterpret_cast<barq_key_path_array_t*>(key_path_array_ptr),
...
public:
    CustomJVMScheduler(jobject dispatchScheduler) : m_id(std::this_thread::get_id()) {
        JNIEnv *jenv = get_env();
        jclass jvm_scheduler_class = jenv->FindClass("io/github/barqdb/kotlin/internal/interop/JVMScheduler");
    ...
    void notify(barq_work_queue_t* work_queue) {
        ...
        // shutdown property. See https://github.com/BarqDB/barq-core/issues/6429
...
//      _Java_io.github.barqdb.kotlin.internal_interop_barqcJNI_invoke_1core_1notify_1callback in barqc.cpp.o
...
void invoke_core_notify_callback(int64_t work_queue) {
    barq_scheduler_perform_work(reinterpret_cast<barq_work_queue_t *>(work_queue));
}
...
barq_create_scheduler(jobject dispatchScheduler) {
    if (dispatchScheduler) {
        ...
        auto scheduler = barq_scheduler_new(
                jvmScheduler,
                [](void *userdata) {
                ...
                [](void *userdata, barq_work_queue_t* work_queue) { static_cast<CustomJVMScheduler *>(userdata)->notify(work_queue); },
...
jobject convert_to_jvm_app_error(JNIEnv* env, const barq_app_error_t* error) {
    static JavaMethod app_error_constructor(env,
                                                JavaClassGlobalDef::app_error(),
                                                "newInstance",
                                                "(IIILjava/lang/String;Ljava/lang/String;)Lio/github/barqdb/kotlin/internal/interop/sync/AppError;",
                                                true);
    env->PushLocalFrame(3);
...
jobject core_connection_state(JNIEnv* env, barq_sync_connection_state_e state) {
    static JavaMethod connection_state_constructor(env,
                                            JavaClassGlobalDef::connection_state(),
                                            "of",
                                            "(I)Lio/github/barqdb/kotlin/internal/interop/sync/CoreConnectionState;",
                                            true);
    env->PushLocalFrame(1);
...
void app_complete_void_callback(void *userdata, const barq_app_error_t *error) {
    auto env = get_env(true);
    static JavaMethod java_notify_onerror(env, JavaClassGlobalDef::app_callback(), "onError",
                                          "(Lio/github/barqdb/kotlin/internal/interop/sync/AppError;)V");
...
void app_complete_result_callback(void* userdata, void* result, const barq_app_error_t* error) {
    auto env = get_env(true);
    static JavaMethod java_notify_onerror(env, JavaClassGlobalDef::app_callback(), "onError",
                                          "(Lio/github/barqdb/kotlin/internal/interop/sync/AppError;)V");
    static JavaMethod java_notify_onsuccess(env, JavaClassGlobalDef::app_callback(), "onSuccess",
    ...
    static JavaClass native_pointer_class(env, "io/github/barqdb/kotlin/internal/interop/LongPointerWrapper");
    ...
    if (env->ExceptionCheck()) {
        ...
        void* cloned_result = barq_clone(result);
...
// Legacy api-key callbacks removed: Barq uses token auth and no
// longer defines barq_app_user_apikey_t.

void app_string_callback(barq_userdata_t userdata, const char *serialized_ejson_response,
                         const barq_app_error_t *error) {
    auto env = get_env(true);
...
// app_apikey_list_callback removed together with the api-key provider surface.

bool barq_should_compact_callback(void* userdata, uint64_t total_bytes, uint64_t used_bytes) {
    auto env = get_env(true);
    static JavaClass java_should_compact_class(env, "io/github/barqdb/kotlin/internal/interop/CompactOnLaunchCallback");
...
bool barq_data_initialization_callback(void* userdata, barq_t*) {
    auto env = get_env(true);
    static JavaClass java_data_init_class(env, "io/github/barqdb/kotlin/internal/interop/DataInitializationCallback");
...
static void send_request_via_jvm_transport(JNIEnv *jenv, jobject network_transport, const barq_http_request_t request, jobject j_response_callback) {
    static JavaMethod m_send_request_method(jenv,
                                            JavaClassGlobalDef::network_transport_class(),
                                            "sendRequest",
                                            "(Ljava/lang/String;Ljava/lang/String;Ljava/util/Map;Ljava/lang/String;Lio/github/barqdb/kotlin/internal/interop/sync/ResponseCallback;)V");

    // Prepare request fields to be consumable by JVM
    std::string method;
    switch (request.method) {
        case barq_http_request_method::BARQ_HTTP_REQUEST_METHOD_GET:
            method = "get";
            break;
        case barq_http_request_method::BARQ_HTTP_REQUEST_METHOD_POST:
            method = "post";
            break;
        case barq_http_request_method::BARQ_HTTP_REQUEST_METHOD_PATCH:
            method = "patch";
            break;
        case barq_http_request_method::BARQ_HTTP_REQUEST_METHOD_PUT:
            method = "put";
            break;
        case barq_http_request_method::BARQ_HTTP_REQUEST_METHOD_DELETE:
            method = "delete";
            break;
    }
    ...
    for (int i = 0; i < map_size; i++) {
        ...
        barq_http_header_t header_pair = request.headers[i];
...
void complete_http_request(void* request_context, jobject j_response) {
    ...
    barq_http_transport_complete_request(request_context, &response);
}
...
static void network_request_lambda_function(void* userdata,
                                            const barq_http_request_t request,
    ...
    try {
        ...
        static jmethodID response_callback_constructor = jenv->GetMethodID(response_callback_class,
                                                                           "<init>",
                                                                           "(Lio/github/barqdb/kotlin/internal/interop/sync/NetworkTransport;J)V");
        push_local_frame(jenv, 1);
        ...
        barq_http_response_t response_error;
        // FIXME: validate we propagate the custom codes as an actual exception to the user
        // see: https://github.com/BarqDB/barq-kotlin/issues/451
        response_error.custom_status_code = -4;
        response_error.num_headers = 0;
        response_error.body_size = 0;

        barq_http_transport_complete_request(request_context, &response_error);
    }
}

barq_http_transport_t* barq_network_transport_new(jobject network_transport) {
    auto jenv = get_env(false); // Always called from JVM
    return barq_http_transport_new(&network_request_lambda_function,
...
static void websocket_post_func(barq_userdata_t userdata,
                                barq_sync_socket_post_callback_t* barq_callback) {
    ...
    WebsocketFunctionHandlerCallback* lambda = new WebsocketFunctionHandlerCallback([barq_callback=std::move(barq_callback)](bool cancelled, int status, const char* reason) {
        barq_sync_socket_post_complete(barq_callback,
                                        cancelled ? barq_sync_socket_callback_result::BARQ_ERR_SYNC_SOCKET_OPERATION_ABORTED : barq_sync_socket_callback_result::BARQ_ERR_SYNC_SOCKET_SUCCESS,
                                        "");
    });
    ...
    static JavaMethod post_method(jenv, JavaClassGlobalDef::sync_websocket_transport(), "post",
                                                     "(Lio/github/barqdb/kotlin/internal/interop/NativePointer;)V");
...
static barq_sync_socket_timer_t websocket_create_timer_func(
        barq_userdata_t userdata, uint64_t delay_ms,
        barq_sync_socket_timer_callback_t *barq_callback) {
    ...
    WebsocketFunctionHandlerCallback *lambda = new WebsocketFunctionHandlerCallback(
            [barq_callback = std::move(barq_callback)](bool cancel, int status,
                                                         const char *reason) {
                if (cancel) {
                    barq_sync_socket_timer_canceled(barq_callback);
                } else {
                    barq_sync_socket_timer_complete(barq_callback,
                                                     barq_sync_socket_callback_result::BARQ_ERR_SYNC_SOCKET_SUCCESS,
                                                     "");
                }
            });
    jobject lambda_callback_pointer_wrapper = wrap_pointer(jenv,reinterpret_cast<jlong>(lambda));
    ...
    static JavaMethod create_timer_method (jenv, JavaClassGlobalDef::sync_websocket_transport(), "createTimer",
                                                     "(JLio/github/barqdb/kotlin/internal/interop/NativePointer;)Lio/github/barqdb/kotlin/internal/interop/sync/CancellableTimer;");
    jobject websocket_transport = static_cast<jobject>(userdata);
    ...
    jenv->DeleteLocalRef(lambda_callback_pointer_wrapper);
    return reinterpret_cast<barq_sync_socket_timer_t>(jenv->NewGlobalRef(cancellable_timer));
}

static void websocket_cancel_timer_func(barq_userdata_t userdata,
    ...
    if (timer_userdata != nullptr) {
        ...
        static JavaClass cancellable_timer_class(jenv, "io/github/barqdb/kotlin/internal/interop/sync/CancellableTimer");
...
static barq_sync_socket_websocket_t websocket_connect_func(
        barq_userdata_t userdata, barq_websocket_endpoint_t endpoint,
        barq_websocket_observer_t* barq_websocket_observer) {

    auto jenv = get_env(false);

    jobject observer_pointer = wrap_pointer(jenv,reinterpret_cast<jlong>(barq_websocket_observer));

    static JavaClass websocket_observer_class(jenv, "io/github/barqdb/kotlin/internal/interop/sync/WebSocketObserver");
    ...
    static JavaMethod connect_method(jenv, JavaClassGlobalDef::sync_websocket_transport(), "connect",
                                                        "(Lio/github/barqdb/kotlin/internal/interop/sync/WebSocketObserver;Ljava/lang/String;Ljava/lang/String;JZJLjava/lang/String;)Lio/github/barqdb/kotlin/internal/interop/sync/WebSocketClient;");
    jobject websocket_transport = static_cast<jobject>(userdata);
    ...
    barq_sync_socket_websocket_t global_websocket_ref = reinterpret_cast<barq_sync_socket_websocket_t>(jenv->NewGlobalRef(websocket_client));
...
static void websocket_async_write_func(barq_userdata_t userdata,
                                 barq_sync_socket_websocket_t websocket_userdata,
                                 const char* data, size_t size,
                                 barq_sync_socket_write_callback_t* barq_callback) {
    auto jenv = get_env(false);

    WebsocketFunctionHandlerCallback* lambda = new WebsocketFunctionHandlerCallback([barq_callback=std::move(barq_callback)](bool cancelled, int status, const char* reason) {
        barq_sync_socket_write_complete(barq_callback,
                                         cancelled ? barq_sync_socket_callback_result::BARQ_ERR_SYNC_SOCKET_OPERATION_ABORTED: barq_sync_socket_callback_result::BARQ_ERR_SYNC_SOCKET_SUCCESS,
                                         "");
    });
    jobject lambda_callback_pointer_wrapper = wrap_pointer(jenv,reinterpret_cast<jlong>(lambda));
    ...
    static jmethodID write_method = jenv->GetMethodID(JavaClassGlobalDef::sync_websocket_transport(), "write",
                                                      "(Lio/github/barqdb/kotlin/internal/interop/sync/WebSocketClient;[BJLio/github/barqdb/kotlin/internal/interop/NativePointer;)V");
    jobject websocket_transport = static_cast<jobject>(userdata);
...
static void barq_sync_websocket_free(barq_userdata_t userdata,
                                      barq_sync_socket_websocket_t websocket_userdata) {
...
static void barq_sync_userdata_free(barq_userdata_t userdata) {
    if (userdata != nullptr) {
        auto jenv = get_env(false);
...
void barq_sync_websocket_connected(int64_t observer_ptr, const char* protocol) {
    barq_sync_socket_websocket_connected(reinterpret_cast<barq_websocket_observer_t*>(observer_ptr), protocol);
}

void barq_sync_websocket_error(int64_t observer_ptr) {
    barq_sync_socket_websocket_error(reinterpret_cast<barq_websocket_observer_t*>(observer_ptr));
}
...
bool barq_sync_websocket_message(int64_t observer_ptr, jbyteArray data, size_t size) {
    ...
    bool close_websocket = !barq_sync_socket_websocket_message(reinterpret_cast<barq_websocket_observer_t*>(observer_ptr), charData.get(), size);
...
void barq_sync_websocket_closed(int64_t observer_ptr, bool was_clean, int error_code, const char* reason) {
    barq_sync_socket_websocket_closed(reinterpret_cast<barq_websocket_observer_t*>(observer_ptr), was_clean, static_cast<barq_web_socket_errno_e>(error_code), reason);
}

barq_sync_socket_t* barq_sync_websocket_new(int64_t sync_client_config_ptr, jobject websocket_transport) {
    auto jenv = get_env(false); // Always called from JVM
    barq_sync_socket_t* socket_provider = barq_sync_socket_new(jenv->NewGlobalRef(websocket_transport), /*userdata*/
                                  barq_sync_userdata_free/*userdata_free*/,
                                  ...
                                  barq_sync_websocket_free/*websocket_free_func*/);
    jni_check_exception(jenv);

    barq_sync_client_config_set_sync_socket(reinterpret_cast<barq_sync_client_config_t*>(sync_client_config_ptr)/*config*/, socket_provider);
    barq_release(socket_provider);
...
auto jenv = get_env(false);
barq_set_log_callback([](void *userdata, const char *category, barq_log_level_e level, const char *message) {
...
jobject convert_to_jvm_sync_error(JNIEnv* jenv, const barq_sync_error_t& error) {

    static JavaMethod sync_error_constructor(jenv,
                                             JavaClassGlobalDef::sync_error(),
                                             "<init>",
    "(IILjava/lang/String;Ljava/lang/String;Ljava/lang/String;ZZZ[Lio/github/barqdb/kotlin/internal/interop/sync/CoreCompensatingWriteInfo;Ljava/lang/Throwable;)V");
    ...
    for (int i = 0; i < error.user_info_length; i++) {
        barq_sync_error_user_info_t user_info = error.user_info_map[i];
    ...
    for (int index = 0; index < error.compensating_writes_length; index++) {
        barq_sync_error_compensating_write_info_t& compensating_write_info = error.compensating_writes[index];
...
void sync_set_error_handler(barq_sync_config_t* sync_config, jobject error_handler) {
    barq_sync_config_set_error_handler(sync_config,
                                        [](void* userdata, barq_sync_session_t* session, const barq_sync_error_t error) {
                                            auto jenv = get_env(true);
                                            ...
                                            static JavaMethod sync_error_method(jenv,
                                                                                ...
                                                                                "onSyncError",
                                                                                "(Lio/github/barqdb/kotlin/internal/interop/NativePointer;Lio/github/barqdb/kotlin/internal/interop/sync/SyncError;)V");
...
void transfer_completion_callback(void* userdata, barq_error_t* error) {
    auto env = get_env(true);
...
void barq_subscriptionset_changed_callback(void* userdata, barq_flx_sync_subscription_set_state_e state) {
    auto env = get_env(true);
    env->PushLocalFrame(1);
...
void barq_async_open_task_callback(void* userdata, barq_thread_safe_reference_t* barq, const barq_async_error_t* error) {
    auto env = get_env(true);
    ...
    if (error) {
        barq_error_t err;
        barq_get_async_error(error, &err);
...
before_client_reset(void* userdata, barq_t* before_barq) {
    ...
    static JavaMethod java_before_callback_function(env,
                                                    ...
                                                    "(Lio/github/barqdb/kotlin/internal/interop/NativePointer;)V");
    env->PushLocalFrame(1);
    jobject before_pointer = wrap_pointer(env, reinterpret_cast<jlong>(before_barq), false);
...
after_client_reset(void* userdata, barq_t* before_barq,
    ...
    static JavaMethod java_after_callback_function(env,
                                                   ...
                                                   "onAfterReset",
                                                   "(Lio/github/barqdb/kotlin/internal/interop/NativePointer;Lio/github/barqdb/kotlin/internal/interop/NativePointer;Z)V");
    env->PushLocalFrame(2);
    jobject before_pointer = wrap_pointer(env, reinterpret_cast<jlong>(before_barq), false);
    // Reuse the scheduler from the beforeBarq, otherwise Core will attempt to recreate a new one,
    // which will fail on platforms that hasn't defined a default scheduler factory.
    barq_scheduler_t scheduler = barq_scheduler(before_barq->get()->scheduler());
    barq_t* after_barq_ptr = barq_from_thread_safe_reference(after_barq, &scheduler);

    jobject after_pointer = wrap_pointer(env, reinterpret_cast<jlong>(after_barq_ptr), false);
    env->CallVoidMethod(static_cast<jobject>(userdata), java_after_callback_function, before_pointer, after_pointer, did_recover);
    barq_close(after_barq_ptr);
    bool result = jni_check_exception_for_callback(env);
    env->PopLocalFrame(NULL);
    return result;
}
...
sync_before_client_reset_handler(barq_sync_config_t* config, jobject before_handler) {
    // TODO use typemap patterns in barq.i
    auto jenv = get_env(true);
    auto before_func = reinterpret_cast<barq_sync_before_client_reset_func_t>(before_client_reset);
    void* user_data = static_cast<jobject>(jenv->NewGlobalRef(before_handler));
    barq_free_userdata_func_t free_func = [](void *userdata) {
        get_env(true)->DeleteGlobalRef(static_cast<jobject>(userdata));
    };
    barq_sync_config_set_before_client_reset_handler(config, before_func, user_data, free_func);
}
...
sync_after_client_reset_handler(barq_sync_config_t* config, jobject after_handler) {
    // TODO use typemap patterns in barq.i
    auto jenv = get_env(true);
    auto after_func = reinterpret_cast<barq_sync_after_client_reset_func_t>(after_client_reset);
    void* user_data = static_cast<jobject>(jenv->NewGlobalRef(after_handler));
    barq_free_userdata_func_t free_func = [](void *userdata) {
        get_env(true)->DeleteGlobalRef(static_cast<jobject>(userdata));
    };
    barq_sync_config_set_after_client_reset_handler(config, after_func, user_data, free_func);
}
...
barq_sync_session_connection_state_change_callback(void *userdata, barq_sync_connection_state_e old_state, barq_sync_connection_state_e new_state) {
    ...
    static JavaMethod java_callback_method(env, JavaClassGlobalDef::connection_state_change_callback(), "onChange", "(Lio/github/barqdb/kotlin/internal/interop/sync/CoreConnectionState;Lio/github/barqdb/kotlin/internal/interop/sync/CoreConnectionState;)V");
...
barq_sync_session_register_progress_notifier_wrapper(
        barq_sync_session_t* session, barq_sync_progress_direction_e direction, bool is_streaming, jobject callback
) {
    auto jenv = get_env(true);
    jlong jresult = 0;
    barq_sync_session_connection_state_notification_token_t *result =
            barq_sync_session_register_progress_notifier(
                    session,
                    barq_sync_session_progress_notifier_callback,
                    direction,
                    is_streaming,
    ...
    *(barq_sync_session_connection_state_notification_token_t **)&jresult = result;
    return jresult;
}
...
barq_create_generic_scheduler() {
    return new barq_scheduler_t { barq::util::Scheduler::make_dummy() };
}
...
jobjectArray barq_get_log_category_names() {
    ...
    size_t namesCount = barq_get_category_names(0, nullptr);

    const char** category_names = new const char*[namesCount];
    barq_get_category_names(namesCount, category_names);
