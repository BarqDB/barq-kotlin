...
import io.github.barqdb.kotlin.entities.backlink.Child
import io.github.barqdb.kotlin.entities.backlink.EmbeddedChild
import io.github.barqdb.kotlin.entities.backlink.MissingSourceProperty
import io.github.barqdb.kotlin.entities.backlink.Parent
import io.github.barqdb.kotlin.entities.backlink.Parent2
import io.github.barqdb.kotlin.entities.backlink.Recursive
...
class BacklinksTests {
    ...
    fun unmanaged_throws() {
        ...
        parent.child = child
        parent.childSet = barqSetOf(child)
        parent.childList = barqListOf(child)
        parent.childDictionary = barqDictionaryOf("A" to child)
    ...
    fun managed_childrenDictionaryWithNullValues() {
        barq.writeBlocking {
            val child = this.copyToBarq(Child())

            val parents = (1..5).map {
                this.copyToBarq(Parent(it))
            }
    ...
    @Test
    fun recursive() {
        val recursive = barq.writeBlocking {
            val recursive = this.copyToBarq(Recursive())
    ...
    fun dynamic() {
        ...
        barq.asDynamicBarq().query(Child::class.simpleName!!)
            .first()
            .find { dynamicObject ->
    ...
    fun dynamicMissingProperty_throws() {
        barq.writeBlocking {
            this.copyToBarq(Recursive())
        }
        barq.asDynamicBarq().let { dynamicBarq ->
            dynamicBarq.query("Recursive")
                .first()
                .find { child ->
                    assertNotNull(child)
    ...
    fun dynamicWrongProperty_throws() {
        ...
        barq.asDynamicBarq().let { dynamicBarq ->
            dynamicBarq.query("Recursive")
                ...
                .find { child ->
                    assertNotNull(child)
                    assertFailsWithMessage<IllegalArgumentException>("Trying to access property 'name' as an object reference but schema type is 'class io.github.barqdb.kotlin.types.BarqUUID'") {
        ...
        barq.asDynamicBarq().let { dynamicBarq ->
            dynamicBarq.query("Recursive")
                ...
                .find { child ->
                    assertNotNull(child)
                    assertFailsWithMessage<IllegalArgumentException>("Trying to access property 'uuidSet' as an object reference but schema type is 'BarqSet<class io.github.barqdb.kotlin.types.BarqUUID>'") {
        ...
        barq.asDynamicBarq().let { dynamicBarq ->
            dynamicBarq.query("Recursive")
                ...
                .find { child ->
                    assertNotNull(child)
                    assertFailsWithMessage<IllegalArgumentException>("Trying to access property 'uuidList' as an object reference but schema type is 'BarqList<class io.github.barqdb.kotlin.types.BarqUUID>'") {
        ...
        barq.asDynamicBarq().let { dynamicBarq ->
            dynamicBarq.query("Recursive")
                ...
                .find { child ->
                    assertNotNull(child)
                    assertFailsWithMessage<IllegalArgumentException>("Trying to access property 'uuidDictionary' as an object reference but schema type is 'BarqDictionary<class io.github.barqdb.kotlin.types.BarqUUID>'") {
    ...
    fun classNotInSchema() {
        tmpDir = PlatformUtils.createTempDir()
        val configuration =
            BarqConfiguration.Builder(setOf(Child::class))
                .directory(tmpDir)
                .build()
    ...
    fun fieldNotInClass() {
        tmpDir = PlatformUtils.createTempDir()
        val configuration =
            BarqConfiguration.Builder(setOf(MissingSourceProperty::class))
                .directory(tmpDir)
                .build()
    ...
    fun deleteSourceObject() {
        ...
        listOf(
            ...
            // Deleting the object should not affect as it happens in a different Barq version.
            linkingObjects.forEach {
                assertEquals(5, it.size)
            }
        }
    ...
    @Test
    fun closingBarqInvalidatesBacklinks() {
        val child = barq.writeBlocking {
            val child = this.copyToBarq(Child())
        ...
        listOf(
            ...
            // Closing the Barq instance should make backlinks inaccessible
            linkingObjects.forEach {
    ...
    @Test
    fun linkingNull() {
        val parent = barq.writeBlocking { copyToBarq(Parent()) }
    ...
    fun linkingObjects_namedQueries() {
        ...
        assertEquals(
            1,
            barq.query<Child>("@links.Parent.child.id == $0", parent.id).count().find()
        )
    }
    ...
    fun linkingObjects_namedQueriesRecursive() {
        ...
        assertEquals(
            1,
            barq.query<Recursive>("@links.Recursive.recursiveField.name == $0", recursive.name).count().find()
        )
    }

    @Test
    fun linkingEmbeddedObjects_unnamedLinkQueries() {
        val parent1 = barq.writeBlocking {
            copyToBarq(
        ...
        val parent2 = barq.writeBlocking {
            copyToBarq(
        ...
        assertEquals(
            1,
            barq.query<EmbeddedChild>("@links.Parent.embeddedChild.id == $0", parent1.id).count().find()
        )
        assertEquals(
            0,
            barq.query<EmbeddedChild>("@links.Parent.embeddedChild.id == $0", parent2.id).count().find()
        )
        assertEquals(
            1,
            barq.query<EmbeddedChild>("@links.Parent2.embeddedChild.id == $0", parent2.id).count().find()
        )
        assertEquals(
            0,
            barq.query<EmbeddedChild>("@links.Parent2.embeddedChild.id == $0", parent1.id).count().find()
        )
    }
    ...
    fun linkingEmbeddedObjects_namedLinkQueries() {
        ...
        assertEquals(
            1,
            barq.query<EmbeddedChild>("parentViaBacklinks.id == $0", parent1.id).count().find()
        )
        assertEquals(
            0,
            barq.query<EmbeddedChild>("parentViaBacklinks.id == $0", parent2.id).count().find()
        )
        assertEquals(
            1,
            barq.query<EmbeddedChild>("parent2ViaBacklinks.id == $0", parent2.id).count().find()
        )
        assertEquals(
            0,
            barq.query<EmbeddedChild>("parent2ViaBacklinks.id == $0", parent1.id).count().find()
        )
    }
    ...
    fun deleteBacklinkResults() {
        ...
        assertEquals(0, barq.query<Recursive>().count().find())
    }
}
