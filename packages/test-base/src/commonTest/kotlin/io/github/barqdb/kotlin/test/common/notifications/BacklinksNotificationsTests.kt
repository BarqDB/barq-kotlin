...
class BacklinksNotificationsTests : BarqEntityNotificationTests {
    ...
    override fun asFlow() {
        runBlocking {
            val targetObject = barq.write {
                val target = copyToBarq(Sample())
                copyToBarq(
                    Sample().apply {
    ...
    fun verifyChangeEvents() {
        runBlocking {
            ...
            barq.write {
                copyToBarq(Sample().apply { nullableObject = findLatest(target) })
            }
    ...
    override fun deleteEntity() {
        runBlocking {
            listOf(
                barq.write { copyToBarq(Sample()) }.let { Pair(it, it.objectBacklinks) },
                barq.write { copyToBarq(Sample()) }.let { Pair(it, it.listBacklinks) },
                barq.write { copyToBarq(Sample()) }.let { Pair(it, it.setBacklinks) }
            ).forEach { (target: Sample, results: BarqResults<Sample>) ->
                val c = Channel<ResultsChange<*>?>(capacity = 1)
    ...
    override fun asFlowOnDeletedEntity() {
        runBlocking {
            val sample = barq.write { copyToBarq(Sample()) }
    ...
    override fun closeBarqInsideFlowThrows(): Unit = TODO("Not correctly implemented yet")
    ...
    override fun keyPath_topLevelProperty() = runBlocking<Unit> {
        ...
        val result: BarqResults<Sample> = barq.query<Sample>("stringField = 'child'").find()
    ...
    override fun keyPath_defaultDepth() = runBlocking<Unit> {
        ...
        val results = barq.query<Sample>("stringField = 'BottomChild'").find()
        val observer = async {
            // Default keypath
            results.asFlow().collect {
                c.trySend(it)
