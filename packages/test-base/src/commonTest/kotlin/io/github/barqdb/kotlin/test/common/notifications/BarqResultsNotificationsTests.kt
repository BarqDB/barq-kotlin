...
import io.github.barqdb.kotlin.test.common.utils.KeyPathFlowableTests
...
class BarqResultsNotificationsTests : FlowableTests, KeyPathFlowableTests {
    ...
    fun setup() {
        ...
        configuration = BarqConfiguration.Builder(schema = setOf(Sample::class) + listTestSchema)
            .directory(tmpDir)
            .build()
        ...
        runBlocking {
            ...
            barq.writeBlocking {
                (dataset1 + dataset3).forEach { element ->
                    delete(
                        query<BarqListContainer>("stringField = $0", element.stringField)
                            .first()
                            .find()!!
                    )
                }
    ...
    override fun cancelAsFlow() {
        runBlocking {
            ...
            barq.write {
                copyToBarq(Sample().apply { stringField = "Baz" })
            }
    ...
    override fun keyPath_nestedProperty() = runBlocking<Unit> {
        ...
        val results = barq.query<Sample>("stringField != 'child'").find()
    ...
    override fun keyPath_defaultDepth() = runBlocking<Unit> {
        ...
        val results = barq.query<Sample>("intField = 1").find()
        assertEquals(1, results.size)
        val observer = async {
            // Default keypath
