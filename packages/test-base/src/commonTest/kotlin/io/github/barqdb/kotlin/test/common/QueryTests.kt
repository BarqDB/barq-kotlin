...
import io.github.barqdb.kotlin.internal.query.AggregatorQueryType
...
class QueryTests {
    ...
    fun setup() {
        ...
        val configuration = BarqConfiguration.Builder(schema = setOf(QuerySample::class))
            .directory(tmpDir)
            .build()
    ...
    fun query_missingArgumentThrows() {
        assertFailsWithMessage<IllegalArgumentException>("Request for argument at index 0 but no arguments are provided") {
            barq.query<QuerySample>("stringField = $0")
        }
    }
    ...
    fun query_wrongArgumentTypeThrows() {
        assertFailsWithMessage<IllegalArgumentException>("Cannot compare argument \$0 with value 'true' to a string") {
            barq.query<QuerySample>("stringField = $0", true)
        }
    }

    @Test
    fun description() {
        val desc = barq.query<QuerySample>("stringField = $0", "search-term").description()
    ...
    fun query_typesAreConvertedCorrectly() {
        barq.writeBlocking {
            val objectWithDefaults = copyToBarq(QuerySample().apply { stringField = "DEFAULTS" })
            copyToBarq(
                QuerySample().apply {
                    ...
                    nullableBarqObject = objectWithDefaults
                }
            )
        }
        assertEquals(2, barq.query<QuerySample>().find().size)

        fun <T> checkQuery(property: KMutableProperty1<QuerySample, T>, value: T) {
            barq.query<QuerySample>("${property.name} = $0", value)
                .find()
                .single()
                .run {
        ...
        for (type: BarqStorageType in BarqStorageType.values()) {
            when (type) {
                ...
                BarqStorageType.OBJECT -> {
                    val child = barq.query<QuerySample>("stringField = 'DEFAULTS'").find().first()
                    assertEquals(
                        "NONDEFAULTS",
                        barq.query<QuerySample>("nullableBarqObject = $0", child).find()
                            .single().stringField
                    )
                }
                ...
                BarqStorageType.UUID -> {
                    checkQuery(
                        ...
                        BarqUUID.from("46423f1b-ce3e-4a7e-812f-004cf9c42d77")
                    )
                }
                BarqStorageType.BINARY -> {
                    val value = byteArrayOf(43)
                    barq.query<QuerySample>("binaryField = $0", value)
                        .find()
                        .single()
                        .run {
                ...
                BarqStorageType.ANY -> {
                    checkQuery(QuerySample::barqAnyField, BarqAny.create("Hello"))
                }
    ...
    fun find_barq() {
        ...
        barq.writeBlocking {
            ...
            copyToBarq(QuerySample(stringField = stringValue))
        ...
        // Filter by string
        barq.query<QuerySample>("stringField = $0", stringValue)
            .find { results -> assertEquals(1, results.size) }

        // Filter by string that doesn't match
        barq.query<QuerySample>("stringField = $0", "invalid string")
            .find { results -> assertEquals(0, results.size) }
    }

    @Test
    fun find_mutableBarq() {
        val stringValue = "some string"
    ...
    fun composedQuery() {
        ...
        barq.writeBlocking {
            copyToBarq(QuerySample(intField = 1, stringField = joe))
            copyToBarq(QuerySample(intField = 2, stringField = sylvia))
            copyToBarq(QuerySample(intField = 3, stringField = stacy))
            copyToBarq(QuerySample(intField = 4, stringField = ruth))
            copyToBarq(QuerySample(intField = 5, stringField = bob))
        }

        // 5, 4, 3
        barq.query<QuerySample>("intField > 1")
            .query("intField > 2")
    ...
    fun composedQuery_withDescriptor() {
        ...
        barq.writeBlocking {
            copyToBarq(QuerySample(intField = value1, stringField = joe))
            copyToBarq(QuerySample(intField = value2, stringField = sylvia))
            copyToBarq(QuerySample(intField = value3, stringField = stacy))
            copyToBarq(QuerySample(intField = value4, stringField = ruth))
            copyToBarq(QuerySample(intField = value5, stringField = bob))
        }
    ...
    fun composedQuery_withDescriptors() {
        ...
        barq.writeBlocking {
            copyToBarq(QuerySample(intField = value1, stringField = sylvia))
    ...
    fun sort_singleField() {
        ...
        barq.writeBlocking {
            values.forEach { (intValue, stringValue) ->
                copyToBarq(QuerySample(intField = intValue, stringField = stringValue))
            }
        }
    ...
    fun sort_barqAny() {
        barq.writeBlocking {
            QUERY_BARQ_ANY_VALUES.forEach {
                copyToBarq(QuerySample().apply { barqAnyField = it })
            }
        }

        // No filter, multiple sortings
        barq.query<QuerySample>()
            .sort(QuerySample::barqAnyField.name to Sort.DESCENDING)
            .find { results ->
                val managedBarqAny = assertNotNull(results.first().barqAnyField)
                val expected = BARQ_ANY_MAX.asBarqObject(QuerySample::class)
                val actual = managedBarqAny.asBarqObject(QuerySample::class)
                ...
                assertEquals(BARQ_ANY_MIN, results[results.size - 2].barqAnyField)
                assertEquals(null, results.last().barqAnyField)
            }
    }
    ...
    fun distinct() {
        ...
        val value2 = 2
        barq.writeBlocking {
            copyToBarq(QuerySample(intField = value1))
            copyToBarq(QuerySample(intField = value1)) // repeated intentionally
            copyToBarq(QuerySample(intField = value2))
        }
    ...
    fun distinct_multipleFields() {
        ...
        val value2 = 2
        barq.writeBlocking {
            copyToBarq(QuerySample(intField = value1, longField = value1.toLong()))
            // Mixing values for different fields in this object
            copyToBarq(QuerySample(intField = value1, longField = value2.toLong()))
            // Intentionally inserting the same values as specified in the previous object
            copyToBarq(QuerySample(intField = value1, longField = value2.toLong()))
            copyToBarq(QuerySample(intField = value2, longField = value2.toLong()))
        }
    ...
    fun distinct_throwsIfInvalidProperty() {
        val query = barq.query<QuerySample>()
        assertFailsWith<IllegalArgumentException> {
    ...
    fun limit() {
        ...
        barq.writeBlocking {
            copyToBarq(QuerySample(intField = value1))
            copyToBarq(QuerySample(intField = value2))
            copyToBarq(QuerySample(intField = value3))
        }
    ...
    // --------------------------------------------------------------------------------
    // TODO - Aggregators - average - https://github.com/BarqDB/barq-kotlin/issues/646
    // --------------------------------------------------------------------------------
    ...
    fun count_asFlow_cancel() {
        runBlocking {
            ...
            // Write one object
            barq.write {
                copyToBarq(QuerySample().apply { stringField = "Bar" })
            }
            ...
            barq.write {
                copyToBarq(QuerySample().apply { stringField = "Baz" })
            }
    ...
    fun sum_find_emptyTable() {
        ...
        // Iterate over all properties - exclude BarqInstant
    ...
    fun sum_find_allNullValues() {
        val assertions = { propertyDescriptor: PropertyDescriptor ->
            barq.writeBlocking {
                ...
                copyToBarq(getInstance(propertyDescriptor, QuerySample()))
                copyToBarq(getInstance(propertyDescriptor, QuerySample()))
                sumQuery.find { sumValue ->
                    if (sumValue is Number) {
                        assertEquals(0, sumValue.toInt())
        ...
        // Iterate over nullable properties containing both null and non-null values - exclude BarqInstant
    ...
    @Test
    fun sum_throwsIfBarqInstant() {
        assertFailsWith<IllegalArgumentException> {
            barq.query<QuerySample>()
                .sum(QuerySample::timestampField.name, BarqInstant::class)
        }

        assertFailsWith<IllegalArgumentException> {
            barq.query<QuerySample>()
                .sum(QuerySample::nullableTimestampField.name, BarqInstant::class)
        }
    }

    @Test
    fun sum_throwsIfInvalidProperty() {
        // Numeric to something else, only test Int or Short or Long for BARQ_PROPERTY_TYPE_INT columns
        ...
        // Max of a non-numerical BarqList
        assertFailsWithMessage<IllegalArgumentException>("Conversion not possible between") {
            barq.query<QuerySample>()
        ...
        // Max of a BarqAny column
        assertFailsWithMessage<IllegalArgumentException>("BarqAny properties cannot be aggregated") {
            barq.query<QuerySample>()
                .sum<Int>(QuerySample::barqAnyField.name)
        }
    }
    ...
    fun sum_find_shortOverflow() {
        ...
        barq.writeBlocking {
            for (i in 1..iterations) {
                copyToBarq(QuerySample().apply { shortField = Short.MAX_VALUE })
            }
        }

        // We get an inaccurate short sum if we want the result as a Short
        barq.query(QuerySample::class)
    ...
    fun sum_find_byteOverflow() {
        ...
        barq.writeBlocking {
            for (i in 1..iterations) {
                copyToBarq(QuerySample().apply { byteField = Byte.MAX_VALUE })
            }
        }
    ...
    fun max_find() {
        val assertions = { propertyDescriptor: PropertyDescriptor ->
            ...
            barq.writeBlocking {
                ...
                if (expectedMax is BarqAny) {
                    // The MAX of a BarqAny column is BarqObject, compare by field
                    val expected = expectedMax.asBarqObject(QuerySample::class)
                    val actual = (queryMax as BarqAny).asBarqObject(QuerySample::class)
                    assertEquals(expected.stringField, actual.stringField)
                } else {
            ...
            barq.query(QuerySample::class)
                ...
                .find { maxValue ->
                    ...
                    if (expectedMax is BarqAny) {
                        val expected = expectedMax.asBarqObject(QuerySample::class)
                        val actual = (maxValue as BarqAny).asBarqObject(QuerySample::class)
                        assertEquals(expected.stringField, actual.stringField)
                    } else {
    ...
    fun max_throwsIfInvalidProperty() {
        ...
        assertFailsWithMessage<IllegalArgumentException>("BarqAny properties cannot be aggregated") {
            barq.query<QuerySample>()
                .max<Int>(QuerySample::barqAnyField.name)
        }
    }
    ...
    fun min_throwsIfInvalidProperty() {
        ...
        assertFailsWithMessage<IllegalArgumentException>("BarqAny properties cannot be aggregated") {
            barq.query<QuerySample>()
                .min<Int>(QuerySample::barqAnyField.name)
        }
    }
    ...
    fun first_find() {
        ...
        barq.query<QuerySample>("intField > $0", value1)
            .first()
            .find { first ->
                assertNotNull(first)
                assertEquals(value2, first.intField)
    ...
    fun list_query() {
        barq.writeBlocking {
            copyToBarq(
                QuerySample().apply {
                    intListField = barqListOf(1, 2, 3)
                    floatListField = barqListOf(1f, 2f, 3f)
                    doubleListField = barqListOf(1.0, 2.0, 3.0)
                    decimal128ListField = barqListOf(
                        Decimal128("1"),
                        Decimal128("2"),
                        Decimal128("3")
                    )
                    nullableIntListField = barqListOf(1, 2, 3)
                    nullableFloatListField = barqListOf(1f, 2f, 3f)
                    nullableDoubleListField = barqListOf(1.0, 2.0, 3.0)
                    nullableDecimal128ListField = barqListOf(
                        Decimal128("1"),
                        Decimal128("2"),
                        Decimal128("3")
                    )
        ...
        listOf("@sum", "@min", "@max", "@avg")
            .forEach { aggregator ->
                listOf(
                    ...
                    barq.query<QuerySample>("$field.$aggregator < 7")
                        .find { assertEquals(1, it.size) }
                    barq.query<QuerySample>("$field.$aggregator > 42")
    ...
    fun set_query() {
        barq.writeBlocking {
            copyToBarq(
                QuerySample().apply {
                    intSetField = barqSetOf(1, 2, 3)
                    floatSetField = barqSetOf(1f, 2f, 3f)
                    doubleSetField = barqSetOf(1.0, 2.0, 3.0)
                    decimal128SetField = barqSetOf(
                        Decimal128("1"),
                        Decimal128("2"),
                        Decimal128("3")
                    )
                    nullableIntSetField = barqSetOf(1, 2, 3)
                    nullableFloatSetField = barqSetOf(1f, 2f, 3f)
                    nullableDoubleSetField = barqSetOf(1.0, 2.0, 3.0)
                    nullableDecimal128SetField = barqSetOf(
                        Decimal128("1"),
                        Decimal128("2"),
                        Decimal128("3")
                    )
    ...
    fun dictionary_query() {
        barq.writeBlocking {
            copyToBarq(
                QuerySample().apply {
                    intDictionaryField = barqDictionaryOf("A" to 1, "B" to 2, "C" to 3)
                    floatDictionaryField = barqDictionaryOf("A" to 1f, "B" to 2f, "C" to 3f)
                    doubleDictionaryField = barqDictionaryOf("A" to 1.0, "B" to 2.0, "C" to 3.0)
                    decimal128DictionaryField = barqDictionaryOf(
                        "A" to Decimal128("1"),
                        "B" to Decimal128("2"),
                        "C" to Decimal128("3")
                    )
                    nullableIntDictionaryField = barqDictionaryOf("A" to 1, "B" to 2, "C" to 3)
                    nullableFloatDictionaryField = barqDictionaryOf("A" to 1f, "B" to 2f, "C" to 3f)
                    nullableDoubleDictionaryField = barqDictionaryOf("A" to 1.0, "B" to 2.0, "C" to 3.0)
                    nullableDecimal128DictionaryField = barqDictionaryOf(
                        "A" to Decimal128("1"),
                        "B" to Decimal128("2"),
                        "C" to Decimal128("3")
                    )
    ...
    fun query_inListArgument_mixedCollectionTypes() {
        barq.writeBlocking {
            (0..15).forEach { copyToBarq(QuerySample().apply { intField = it }) }
        }
    ...
    fun query_inListArgument_boolean() {
        ...
        barq.query<QuerySample>("booleanField IN $0", listOf(true, false)).find().run {
    ...
    fun query_inListArgument_string() {
        barq.writeBlocking {
            copyToBarq(QuerySample().apply { stringField = "1" })
            copyToBarq(QuerySample().apply { stringField = "2" })
        }
        barq.query<QuerySample>("stringField IN $0", listOf("1", "2")).find().run {
            assertEquals(2, size)
        }
    }
    ...
    fun query_inListArgument_escapedString() {
        val values = mutableSetOf("\"Barq\"", "'Barq'", "Barqs'", "😀")
        barq.writeBlocking {
            values.forEach {
                copyToBarq(QuerySample().apply { stringField = it })
            }
        }
        assertEquals(values.size, barq.query<QuerySample>().find().size)
        barq.query<QuerySample>(
            "stringField IN $0",
            values
        ).find().run {
    ...
    fun query_inListArgument_links() {
        val (even, odd) = barq.writeBlocking {
            val even = copyToBarq(QuerySample().apply { stringField = "EVEN" })
            copyToBarq(QuerySample().apply { nullableBarqObject = even })
            val odd = copyToBarq(QuerySample().apply { stringField = "ODD" })
            copyToBarq(QuerySample().apply { nullableBarqObject = odd })
            even to odd
        }
        assertEquals(4, barq.query<QuerySample>().find().size)
        barq.query<QuerySample>("nullableBarqObject IN $0", listOf(even, odd)).find().run {
            assertEquals(2, size)
            forEach {
    ...
    fun playground_multiThreadScenario() {
        val channel = TestChannel<BarqResults<QuerySample>>()
        var query: BarqQuery<QuerySample>? = null
        val scope = singleThreadDispatcher("1")
        val intValue = 666

        runBlocking {
            barq.writeBlocking {
                copyToBarq(QuerySample(intField = intValue))
            }

            // Create a query - the query itself is parsed but nothing else is done
            query = barq.query(QuerySample::class, "intField == $0", intValue)
    ...
    fun fullTextSearch() {
        barq.writeBlocking {
            copyToBarq(QuerySample(1).apply { fulltextField = "The quick brown fox jumped over the lazy dog." })
            copyToBarq(QuerySample(2).apply { fulltextField = "I sat at the cafè, drinking a BIG cup of coffee." })
            copyToBarq(QuerySample(3).apply { fulltextField = "Rødgrød med fløde." })
            copyToBarq(QuerySample(4).apply { fulltextField = "full-text-search is hard to implement!" })
            copyToBarq(QuerySample(5).apply { fulltextField = "Trying to search for an emoji, like 😊, inside a text string is not supported." })
        }

        assertEquals(1, barq.query<QuerySample>("fulltextField TEXT 'quick dog'").find().size) // words at different locations
        assertEquals(0, barq.query<QuerySample>("fulltextField TEXT 'brown -fox'").find().size) // exclusion
        assertEquals(2, barq.query<QuerySample>("fulltextField TEXT 'fo*'").find().size) // token prefix search is supported.
        assertEquals(1, barq.query<QuerySample>("fulltextField TEXT 'cafe big'").find().size) // case- and diacritics-insensitive
        assertEquals(1, barq.query<QuerySample>("fulltextField TEXT 'rødgrød'").find().size) // Latin-1 supplement
        assertEquals(0, barq.query<QuerySample>("fulltextField TEXT '😊'").find().size) // Searching outside supported chars return nothing
    }

    @Test
    fun fullTextSearch_invalidArguments() {
        assertFailsWith<IllegalArgumentException> {
            barq.query<QuerySample>("fulltextField TEXT ''").find().size
        }
    }
    ...
    fun fullTextSearch_onNonFullTextFieldThrows() {
        assertFailsWithMessage<IllegalStateException>("Column has no fulltext index") {
            barq.query<QuerySample>("stringField TEXT 'foo'")
        }
    }
    ...
    private fun MutableBarq.saveData(propertyDescriptor: PropertyDescriptor) {
        copyToBarq(getInstance(propertyDescriptor, QuerySample(), 0))
        copyToBarq(getInstance(propertyDescriptor, QuerySample(), 1))

        // Add all values for BarqAny manually
        if (propertyDescriptor.clazz == BarqAny::class) {
            for (i in 2 until QUERY_BARQ_ANY_VALUES.size) {
                copyToBarq(getInstance(propertyDescriptor, QuerySample(), i))
            }
        }
    }
     ...
     * used to populate the barq instance should be used.
     */
    private fun <C> getInstance(
        ...
        when (property.returnType.classifier as KClass<*>) {
            ...
            BarqInstant::class -> setProperty(
                instance,
                property as KMutableProperty1<C, BarqInstant?>,
                values as List<BarqInstant?>,
                index
            )
            BarqAny::class -> setProperty(
                instance,
                property as KMutableProperty1<C, BarqAny?>,
                values as List<BarqAny?>,
                index
            )
    ...
    private fun expectedAggregate(clazz: KClass<*>, type: AggregatorQueryType): Any =
        when (clazz) {
            ...
            BarqAny::class -> when (type) {
                AggregatorQueryType.MIN -> BARQ_ANY_MIN
                AggregatorQueryType.MAX -> BARQ_ANY_MAX
                AggregatorQueryType.SUM -> BARQ_ANY_SUM
            }
            else -> throw IllegalArgumentException("Only numerical properties and timestamps are allowed.")
        }
    ...
    private fun asFlowAggregatorAssertions(
        ...
        runBlocking {
            ...
            when (propertyDescriptor.clazz) {
                BarqAny::class -> when (type) {
                    AggregatorQueryType.MIN -> {
                        val actualValue = (receivedAggregate as BarqAny)
                            .asBoolean()
                        val expectedValue = (expectedAggregate as BarqAny)
                            .asBoolean()
                        assertEquals(expectedValue, actualValue)
                    }
                    AggregatorQueryType.MAX -> {
                        val actualValue = (receivedAggregate as BarqAny)
                            .asBarqObject<QuerySample>()
                            .stringField
                        val expectedValue = (expectedAggregate as BarqAny)
                            .asBarqObject<QuerySample>()
                            .stringField
            ...
            // NOTE - if the aggregated value is a BarqObject the value WILL BE EMITTED
            barq.writeBlocking {
            ...
            when (propertyDescriptor.clazz) {
                BarqAny::class -> when (type) {
                    ...
                    // MAX for BarqAny is BarqObject so emitted objects will not be the same
                    // even though they may the same at a structural level
                    AggregatorQueryType.MAX -> {
                        val receivedRepeatedAggregate = channel.receiveOrFail()
                        val actualString = (receivedRepeatedAggregate as BarqAny)
                            .asBarqObject<QuerySample>()
                            .stringField
                        val expectedString = (expectedAggregate as BarqAny)
                            .asBarqObject<QuerySample>()
                            .stringField
    ...
    private fun asFlowCancel(
        ...
        runBlocking {
            ...
            if (propertyDescriptor.clazz == BarqAny::class) {
                when (type) {
                    AggregatorQueryType.SUM -> {
                        val expectedBarqAnyValue =
                            (propertyDescriptor.values as List<BarqAny?>)[0]
                        if (expectedBarqAnyValue?.type != BarqAny.Type.INT) {
                            throw IllegalArgumentException("Wrong BarqAny value to compare. Expected 'INT' but was '${expectedBarqAnyValue?.type}'.")
                        }
                        val asLong = expectedBarqAnyValue.asLong()
                        ...
                        assertEquals(expectedBarqAnyValue, receivedAggregate)
                    }
                }
            } else {
    ...
    fun keyPath_usingWildCards() = runBlocking<Unit> {
        val channel = Channel<ResultsChange<QuerySample>>(1)
        val observer = async {
            barq.query<QuerySample>("stringField = 'parent'")
        ...
        val obj = barq.write {
            copyToBarq(
                QuerySample().apply {
                    stringField = "parent"
                    nullableBarqObject = QuerySample().apply {
                        stringField = "child"
                    }
                }
            )
        ...
        barq.write {
            findLatest(obj)!!.nullableBarqObject!!.stringField = "update"
        }
        channel.receiveOrFail().let { resultsChange ->
            assertIs<UpdatedResults<*>>(resultsChange)
            assertEquals(1, resultsChange.list.size)
            assertEquals("update", resultsChange.list.first().nullableBarqObject!!.stringField)
        }
        observer.cancel()
        channel.close()
    }
    ...
    private fun cleanUpBetweenProperties() = barq.writeBlocking {
    ...
    private fun getDescriptor(
        ...
        BarqInstant::class -> PropertyDescriptor(
        ...
        BarqAny::class -> PropertyDescriptor(
            QuerySample::barqAnyField,
            BarqAny::class,
            QUERY_BARQ_ANY_VALUES
        )
    ...
    // TODO figure out whether we need to test aggregators on BarqList<Number> fields
    //  see https://github.com/BarqDB/barq-core/issues/5137
}
...
/**
 * Use this and not [io.github.barqdb.kotlin.entities.Sample] as that class has default initializers that make
 * aggregating operations harder to assert.
 */
class QuerySample() : BarqObject {
...
internal val QUERY_BARQ_ANY_OBJECT = BarqAny.create(
    QuerySample().apply { stringField = "foo" },
    QuerySample::class
)

// Use this for QUERY tests as this file does exhaustive testing on all BarqAny types
internal val QUERY_BARQ_ANY_VALUES = BARQ_ANY_PRIMITIVE_VALUES + QUERY_BARQ_ANY_OBJECT

internal val BARQ_ANY_SUM = Decimal128("82") // sum of numerics present in PRIMITIVE_BARQ_ANY_VALUES = -12+13+14+15+16L+17F+18.0+Decimal128("1")
internal val BARQ_ANY_MIN = BarqAny.create(false) // Boolean is the "lowest" type when comparing Mixed types
internal val BARQ_ANY_MAX = QUERY_BARQ_ANY_OBJECT // BarqObject is "highest" when comparing Mixed types
