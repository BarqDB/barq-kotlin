...
package io.github.barqdb.kotlin.test.compiler.set
...
class SetTests : CollectionTests(
    ...
    @Test
    fun `unsupported type in set - EmbeddedBarqObject fails`() {
        val result = compileFromSource(
            SourceFile.kotlin(
                "unsupportedEmbeddedBarqObjectSet.kt",
        ...
        assertTrue(result.messages.contains("BarqSet does not support embedded barq objects element types"))
    }
    ...
    // - BarqObject fails
    @Test
    fun `nullable BarqObject set - fails`() {
        val result = createFileAndCompile(
            "nullableBarqObjectSet.kt",
            getTestCodeForCollection(
                collectionType = CollectionType.SET,
        ...
        assertEquals(KotlinCompilation.ExitCode.COMPILATION_ERROR, result.exitCode)
        assertTrue(result.messages.contains("BarqSet does not support nullable barq objects element types"))
    }

    // - nullable Embedded objects fail
    @Test
    fun `nullable EmbeddedBarqObject - fails`() {
        val result = compileFromSource(
            SourceFile.kotlin(
                "unsupportedEmbeddedBarqObjectSet.kt",
                getTestCodeForCollection(
