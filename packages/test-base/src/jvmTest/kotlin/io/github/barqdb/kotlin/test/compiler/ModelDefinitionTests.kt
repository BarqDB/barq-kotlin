...
import io.github.barqdb.kotlin.compiler.Registrar
...
class ModelDefinitionTests {
    ...
    fun `no_zero_arg_constructor_should_fail`() {
        val result = Compiler.compileFromSource(
            ...
            source = SourceFile.kotlin(
                ...
                """
                        ...
                        class NoZeroArgCtor(var name: String) : BarqObject

                        val configuration =
                            BarqConfiguration.create(schema = setOf(NoZeroArgCtor::class))
                """.trimIndent()
            )
        )
        ...
        assertTrue(result.messages.contains("sources/no_zero_arg_ctor.kt:4:1 [Barq] Cannot find primary zero arg constructor"), result.messages)
    }
        ...
        val result = Compiler.compileFromSource(
            ...
            source = SourceFile.kotlin(
                ...
                """
                        ...
                        class MultipleConstructors(var firstName: String, var lastName: String, var age: Int) : BarqObject {
    ...
    fun `data_class_with_default_constructor`() {
        val result = Compiler.compileFromSource(
            ...
            source = SourceFile.kotlin(
                ...
                """
                        import io.github.barqdb.kotlin.types.BarqObject
                        data class Foo(val name: String) : BarqObject
                """.trimIndent()
            )
        )
    ...
    fun `enum_class`() {
        val result = Compiler.compileFromSource(
            ...
            source = SourceFile.kotlin(
                ...
                """
                        import io.github.barqdb.kotlin.types.BarqObject
                        enum class Foo : BarqObject { NORTH, SOUTH }
                """.trimIndent()
            )
        )
    ...
    fun `model_class_with_unsupported_type`() {
        ...
        assertTrue(result.messages.contains("Barq does not support persisting properties of this type."), "Error was: ${result.messages}")
    }
    ...
    fun `persisted_properties_with_val_should_fail`() {
        val result = Compiler.compileFromSource(
            ...
            source = SourceFile.kotlin(
                ...
                """
                        import io.github.barqdb.kotlin.types.BarqObject
                        class Person(val name: String) : BarqObject
                """.trimIndent()
            )
        )
