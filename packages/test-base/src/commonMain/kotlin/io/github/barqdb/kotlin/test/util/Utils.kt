 ...
 */
suspend fun <T : BarqObject> T.update(block: T.() -> Unit): T {
    @Suppress("invisible_reference", "invisible_member")
    val barq = ((this as io.github.barqdb.kotlin.internal.BarqObjectInternal).`io_github_barqdb_kotlin_objectReference`!!.owner).owner as Barq
    return barq.write {
...
// Expose a try-with-resource pattern for Barqs
inline fun Barq.use(action: (Barq) -> Unit) {
    try {
        action(this)
    } finally {
        this.close()
...
// Expose a try-with-resource pattern for Barqs, but with support for Coroutines
suspend fun Barq.useInContext(action: suspend (Barq) -> Unit) {
    try {
        action(this)
    } finally {
        this.close()
...
// Convert Kotlinx-datatime Instant to BarqInstant
fun Instant.toBarqInstant(): BarqInstant {
    val s: Long = this.epochSeconds
    val ns: Int = this.nanosecondsOfSecond

    // Both Instant and BarqInstant uses positive numbers above epoch
    // Below epoch:
    //  - BarqInstant uses both negative seconds and nanonseconds
    //  - Instant uses negative seconds but ONLY positive nanoseconds
    return if (s >= 0) {
        BarqInstant.from(s, ns)
    } else {
        val adjustedSeconds = s + 1
        val adjustedNanoSeconds = ns - 1_000_000_000
        BarqInstant.from(adjustedSeconds, adjustedNanoSeconds)
    }
}
