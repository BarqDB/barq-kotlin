...
import io.github.barqdb.kotlin.test.platform.PlatformUtils.createTempDir
import io.github.barqdb.kotlin.test.platform.PlatformUtils.deleteTempDir
...
class MemoryTests {
    ...
    @Test
    @Ignore // Investigate https://github.com/BarqDB/barq-kotlin/issues/327
    fun garbageCollectorShouldFreeNativeResources() {
        ...
        val referenceHolder = mutableListOf<Sample>();
        {
            ...
            // TODO use Barq.delete once this is implemented
            barq.writeBlocking {
                delete(query<Sample>())
            }
        ...
        // After releasing all the 'barq_object_create' reference the Barq should be closed and the
        ...
        assertEquals(
            "",
            runSystemCommand(amountOfMemoryMappedInProcessCMD),
            "Freeing the references should close the Barq so no memory mapped allocation should be present"
        )
    }
    ...
    fun closeShouldFreeMemory() {
        ...
        // as done in Darwin SystemUtils.kt and initialized lazily, so do a full barq-lifecycle
        ...
        val referenceHolder = mutableListOf<Sample>();
        {
            ...
            referenceHolder.addAll(elements)
            barq.close() // force closing will free the native memory even though we still have reference to barq_object open.
        }()
