...
dependencies {
    kotlinCompilerPluginClasspath("io.github.barqdb.kotlin:plugin-compiler:${Barq.version}")
    kotlinNativeCompilerPluginClasspath("io.github.barqdb.kotlin:plugin-compiler-shaded:${Barq.version}")
...
// Substitute maven coordinate dependencies of pattern 'io.github.barqdb.kotlin:<name>:${Barq.version}'
...
configurations.all {
    resolutionStrategy.dependencySubstitution {
        rootProject.allprojects
            ...
            .forEach { subproject: Project ->
                substitute(module("io.github.barqdb.kotlin:${subproject.name}:${Barq.version}")).using(
                    project(":${subproject.name}")
                )
            }
    }

    // Ensure that androidUnitTest uses the Barq JVM variant rather than Android.
    ...
    //
    // See https://github.com/BarqDB/barq-kotlin/issues/1404 for more details.
    if (name.endsWith("UnitTestRuntimeClasspath")) {
        resolutionStrategy.dependencySubstitution {
            substitute(module("io.github.barqdb.kotlin:library-base:${Barq.version}")).using(
                module("io.github.barqdb.kotlin:library-base-jvm:${Barq.version}")
            )
            substitute(module("io.github.barqdb.kotlin:cinterop:${Barq.version}")).using(
                module("io.github.barqdb.kotlin:cinterop-jvm:${Barq.version}")
            )
        }
    }
}
    ...
    sourceSets {
        val commonMain by getting {
            dependencies {
                ...
                implementation("io.github.barqdb.kotlin:library-base:${Barq.version}")
                implementation("io.github.barqdb.kotlin:library-sync:${Barq.version}")
...
android {
    namespace = "io.github.barqdb.sync.testapp"
...
kotlin {
    ...
    sourceSets {
        ...
        val androidUnitTest by getting {
            dependencies {
                // Barq dependencies must be converted to -jvm variants here.
    ...
    sourceSets {
        val jvmMain by getting {
            dependencies {
                ...
                implementation("io.github.barqdb.kotlin:plugin-compiler:${Barq.version}")
    ...
    sourceSets {
        ...
        if (HOST_OS.isMacOs()) {
            ...
            val nativeDarwinTest by creating {
                ...
                // e: java.lang.IllegalStateException: IrPropertyPublicSymbolImpl for io.github.barqdb.kotlin.test.sync.util/TEST_METHODS|-1310682179529671403[0] is already bound: PROPERTY name:TEST_METHODS visibility:public modality:FINAL [val]
...
// - 'syncTestUrl` defines the root URL for the sync server. Default is `http://localhost:9090`
// - 'syncTestAppNamePrefix' is added as a differentiator for all apps created by tests. This makes
...
buildkonfig {
    packageName = "io.github.barqdb.kotlin.test.sync"
