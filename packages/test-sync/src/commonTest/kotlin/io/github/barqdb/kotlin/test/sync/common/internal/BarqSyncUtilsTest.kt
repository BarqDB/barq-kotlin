...
package io.github.barqdb.kotlin.test.sync.common.internal
...
import io.github.barqdb.kotlin.internal.interop.UnknownCodeDescription
...
import io.github.barqdb.kotlin.sync.internal.convertAppError
import io.github.barqdb.kotlin.sync.internal.convertSyncError
...
class BarqSyncUtilsTest {
    ...
    fun convertSyncErrorCode_unmappedErrorCode2() {
        val syncException = convertSyncError(
            SyncError(
                CoreError(
                    categoriesNativeValue = ErrorCategory.BARQ_ERR_CAT_CLIENT_ERROR.nativeValue,
    ...
    fun convertSyncErrorCode_unknownNativeErrrorCode() {
        val syncException = convertSyncError(
            SyncError(
                CoreError(
                    categoriesNativeValue = ErrorCategory.BARQ_ERR_CAT_CLIENT_ERROR.nativeValue,
                    errorCodeNativeValue = ErrorCode.BARQ_ERR_UNKNOWN.nativeValue,
                    messageNativeValue = "Placeholder message"
                )
            )
        )
    ...
    fun convertAppError_unmappedErrorCode() {
        val appException = convertAppError(
            AppError(
                categoryFlags = ErrorCategory.BARQ_ERR_CAT_CUSTOM_ERROR.nativeValue,
    ...
    fun convertAppError_unmappedErrorCategoryAndErrorCode_linkServerLog() {
        val appException = convertAppError(
            AppError(
                ...
                message = "Placeholder message",
                httpStatusCode = UNMAPPED_ERROR_CODE,
                linkToServerLog = "http://barq.io"
            )
        )

        assertEquals(
            "[$UNMAPPED_CATEGORY_CODE][Unknown($UNMAPPED_ERROR_CODE)] Placeholder message. Server log entry: http://barq.io",
            appException.message
        )
    }
    ...
    fun convertAppError_unmappedErrorCategoryAndErrorCode_noMessage_linkServerLog() {
        ...
        assertEquals(
            "[$UNMAPPED_CATEGORY_CODE][Unknown($UNMAPPED_ERROR_CODE)] Server log entry: http://barq.io",
            appException.message
        )
    }
}
